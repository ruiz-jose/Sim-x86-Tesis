...
\hypertarget{implementaciuxf3n-del-ciclo-de-instrucciuxf3n-mov-al-bl-en-xdevs}{%
\section{Implementación del ciclo de instrucción MOV AL, BL en xDEVS}\label{implementaciuxf3n-del-ciclo-de-instrucciuxf3n-mov-al-bl-en-xdevs}}

La instrucción \textbf{MOV AL, BL} transfiere el contenido del registro fuente \textbf{BL} al registro destino \textbf{AL}.\\
En el simulador \textbf{VonSim8}, esta operación se modela como una secuencia de \emph{eventos discretos} coordinados por la \textbf{Unidad de Control (UC)}, que emite señales de lectura y escritura sobre los registros involucrados.

A continuación se describe la implementación de este ciclo en el entorno \textbf{xDEVS}, donde cada componente se representa como un \textbf{modelo atómico}, y su interacción como un \textbf{modelo acoplado jerárquico}.
...
El ciclo de instrucción completo se organiza en dos fases generales, representadas como transiciones de estado dentro del formalismo xDEVS:

\begin{itemize}
\tightlist
\item
  \textbf{Fase de captación (Fetch)} --- común a todas las instrucciones.\\
\item
  \textbf{Fase de ejecución (Execute)} --- específica del tipo de instrucción (en este caso, MOV).
\end{itemize}
...
\hypertarget{modelos-atuxf3micos-involucrados}{%
\subsubsection{Modelos Atómicos involucrados}\label{modelos-atuxf3micos-involucrados}}

Durante la ejecución, intervienen los siguientes modelos atómicos del entorno xDEVS:
...
\midrule
\textbf{UC (Unidad de Control)} & Atómico & Decodifica IR, genera señales de control para lectura/escritura.\\
\addlinespace[8pt]
\textbf{IR (Registro de Instrucción)} & Atómico & Contiene el código binario de la instrucción.\\
\addlinespace[8pt]
\textbf{AL (Registro Acumulador bajo)} & Atómico & Registro destino.\\
\addlinespace[8pt]
\textbf{BL (Registro base bajo)} & Atómico & Registro fuente.\\
\addlinespace[8pt]
\textbf{BUS (Bus de datos)} & Atómico o variable compartida & Transporta el dato desde el origen al destino.\\
\addlinespace[8pt]
\bottomrule
\end{tabular}}
\end{table}
...
\hypertarget{flujo-de-eventos-y-microoperaciones-nivel-rtl}{%
\subsubsection{Flujo de eventos y microoperaciones (nivel RTL)}\label{flujo-de-eventos-y-microoperaciones-nivel-rtl}}

El proceso se descompone en microoperaciones que describen la propagación de señales y los cambios de estado en los modelos atómicos.
Cada paso se corresponde con una transición DEVS (\(\delta_{\text{ext}}\), \(\delta_{\text{int}}\), \(\lambda\)), tal como se muestra en la tabla siguiente:
...
\midrule
\textbf{1} & UC: decodificar(IR) & La UC lee el valor del IR y reconoce el opcode MOV con operandos AL, BL. & $\delta_{\text{ext}}$ (UC) recibe evento del IR; actualiza su estado (S) con la instrucción actual.\\
\addlinespace[8pt]
\textbf{2} & UC → BL.enable\_out = 1 & La UC activa la señal de lectura del registro fuente. & $\lambda$ (UC) genera evento de salida hacia BL.\\
\addlinespace[8pt]
\textbf{3} & BUS ← BL & El registro BL coloca su valor en el bus de datos. & $\lambda$ (BL) emite valor actual al BUS.\\
\addlinespace[8pt]
