...
\item
  Teclado y pantalla.
\item
  Un PIO, que puede conectarse a llaves, LEDs o una impresora.
\item
...
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\fontsize{10}{12}\selectfont
\begin{tabular}[t]{>{\centering\arraybackslash}p{4cm}|>{\centering\arraybackslash}p{2cm}>{\centering\arraybackslash}p{6cm}}
\toprule
\multicolumn{1}{>{\centering\arraybackslash}p{4cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Módulo}}} & \multicolumn{1}{>{\centering\arraybackslash}p{2cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Dirección}}} & \multicolumn{1}{>{\centering\arraybackslash}p{6cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Nombre}}}\\
\midrule
\textbf{Timer} & 10h & CONT\\
\textbf{Timer} & 11h & COMP\\
\textbf{PIC} & 20h & EOI\\
\addlinespace[10pt]
\textbf{PIC} & 21h & IMR\\
\textbf{PIC} & 22h & IRR\\
...
\textbf{PIO} & 32h & CA\\
\textbf{PIO} & 33h & CB\\
\textbf{Handshake} & 40h & DATA\\
\textbf{Handshake} & 41h & STATE\\
\bottomrule
\end{tabular}}
\end{table}
...
El \emph{programmed input-output} (PIO) es un módulo que hace de interfaz para conectar dispositivos genéricos al CPU. Está basado en el PPI 8255 de Intel en su modo ``0'', pero con algunas modificaciones para simplificar su funcionamiento.
...
\tightlist
\item
  \texttt{PA} (dirección \texttt{30h} de la {[}memoria E/S{]}),
\item
  \texttt{PB} (dirección \texttt{31h} de la {[}memoria E/S{]}),
\item
  \texttt{CA} (dirección \texttt{32h} de la {[}memoria E/S{]}),
\item
  y \texttt{CB} (dirección \texttt{33h} de la {[}memoria E/S{]}).
\end{itemize}

El valor del puerto A se encuentra en el registro \texttt{PA} y su configuración en el registro \texttt{CA}. El registro \texttt{CA} también es de 8 bits y le indica al PIO el modo cada bit: un \texttt{0} si es de salida y un \texttt{1} si es de entrada. Por ejemplo, para \texttt{CA\ =\ 00001111b}, los cuatro bits más significativos son de salida y los cuatro menos significativos son de entrada. El puerto B funciona idénticamente.

El PIO puede estar conectado a {[}Leds y llaves{]} o a una {[}impresora{]}.
...
\hypertarget{llaves}{%
\subsubsection{Llaves}\label{llaves}}

Las llaves o interruptores están conectados al puerto PA/CA del PIO y funcionan como dispositivos de entrada. Al cambiar su estado, este se refleja en el puerto PA, siempre que el PIO esté configurado correctamente. Sin embargo, si se modifica directamente el valor de PA, los cambios no se reflejarán en las llaves, ya que estas no se actualizan automáticamente.
...
\hypertarget{impresora-con-pio}{%
\subsubsection{Impresora con PIO}\label{impresora-con-pio}}

Una opción es conectar la impresora al {[}PIO{]}. La conexión queda así:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{PA = \_\_\_\_ \_\_SB}
\NormalTok{PB = DDDD DDDD}
\end{Highlighting}
\end{Shaded}

En \texttt{PB} se almacenará carácter a imprimir, expresado en ASCII. En \texttt{PA}, los seis bits más significativos no hacen nada. El bit menos significativo es la flag \emph{busy}. Y el que queda es el \emph{strobe}.

El \emph{strobe} es el bit que le indica a la impresora que queremos imprimir. Las impresoras Centronics toman el valor en \texttt{PB} cuando detectan un flanco ascendente en el \emph{strobe}. Es decir, una transición de \texttt{S=0} a \texttt{S=1}.
...
\tightlist
\item
  verificar que el \emph{buffer} no esté lleno (flag \emph{busy}),
\item
  escribir el carácter en \texttt{PB},
\item
  poner el \emph{strobe} en 0,
\item
  poner el \emph{strobe} en 1.
\end{enumerate}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{; Imprime el string "hola" en la impresora usando PIO}

\NormalTok{dato db "hola", 0         ; String a imprimir, terminado en 0}

\NormalTok{PA EQU 30h                ; Puerto A: strobe y busy}
\NormalTok{PB EQU 31h                ; Puerto B: datos}
\NormalTok{CA EQU 32h                ; Control A}
\NormalTok{CB EQU 33h                ; Control B}

\NormalTok{; {-}{-}{-} Configuración de PIO {-}{-}{-}}
\NormalTok{mov al, 0                 ; PA como salida (strobe)}
\NormalTok{out CA, al}
\NormalTok{mov al, 0                 ; PB como salida (datos)}
\NormalTok{out CB, al}

\NormalTok{; {-}{-}{-} Inicializar strobe en 0 {-}{-}{-}}
\NormalTok{in  al, PA}
\NormalTok{and al, 11111101b         ; Bit 1 (strobe) a 0}
\NormalTok{out PA, al}

\NormalTok{mov bl, offset dato       ; BL apunta al inicio del string}

\NormalTok{sondeo:}
\NormalTok{    in  al, PA}
\NormalTok{    and al, 00000001b     ; ¿Buffer lleno? (busy)}
\NormalTok{    jz  imprimir\_cadena   ; Si busy es 0, salta a imprimir}
\NormalTok{    jmp sondeo            ; Si busy es 1, sigue esperando}

\NormalTok{imprimir\_cadena:}
\NormalTok{    mov al, [bl]          ; Siguiente carácter}
\NormalTok{    cmp al, 0             ; ¿Fin del string?}
\NormalTok{    jz  fin}

\NormalTok{    out PB, al            ; Enviar carácter a PB}

\NormalTok{    ; {-}{-}{-} Generar flanco ascendente en strobe {-}{-}{-}}
\NormalTok{    in  al, PA}
\NormalTok{    or  al, 00000010b     ; Strobe a 1}
\NormalTok{    out PA, al}
\NormalTok{    in  al, PA}
\NormalTok{    and al, 11111101b     ; Strobe a 0}
\NormalTok{    out PA, al}

\NormalTok{    inc bl                ; Siguiente carácter}
\NormalTok{    jmp sondeo}
