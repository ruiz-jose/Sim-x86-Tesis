...
\toprule
\multicolumn{1}{>{\centering\arraybackslash}p{1.5cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Paso}}} & \multicolumn{1}{>{\centering\arraybackslash}p{3.5cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Microoperación}}} & \multicolumn{1}{>{\centering\arraybackslash}p{8cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Descripción}}} & \multicolumn{1}{>{\centering\arraybackslash}p{7cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Funciones DEVS}}}\\
\midrule
...
\textbf{3} & UC $\rightarrow$ MEM$\,$; $\,$IP $\leftarrow$ IP+1 & La UC activa la señal mem\_read para iniciar la lectura de la instrucción y, de forma concurrente, genera una señal de incremento sobre el IP +1 . & \textbf{$\lambda(UC) \rightarrow \delta_{ext}(MEM)$; $\lambda_{UC} \rightarrow \delta_{ext}(IP)$}\\
...
\begin{table}[!h]
\centering
\caption{\label{tab:microoperacionesMOV}Secuencia de microoperaciones y eventos DEVS para la Fase de ejecución}
\centering
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\fontsize{10}{12}\selectfont
...
\midrule
1 & UC: decodificar(IR) & La Unidad de Control (UC) interpreta el opcode MOV y determina los operandos AL y BL. & $\delta_{\text{ext}}(UC)$ recibe evento del IR y actualiza su estado con la instrucción actual.\\
2 & UC $\rightarrow$ REG\_BANK.enable\_out(BL) = 1 & El Banco de Registros habilita la salida del registro BL bajo control de la UC. & $\lambda_{UC}$ genera evento hacia REG\_BANK(BL).\\
3 & BUS $\leftarrow$ BL & El valor almacenado en BL se transfiere al bus compartido. & $\lambda_{BL}$ emite su valor al bus.\\
