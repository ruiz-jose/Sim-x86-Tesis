...
La simulación de la instrucción MOV AL, BL constituye un caso base para validar la arquitectura del simulador VonSim8, diseñado bajo el modelo Von Neumann, en el cual una única memoria unificada almacena tanto las instrucciones como los datos.
En este esquema, el bus principal es compartido, lo que genera el clásico cuello de botella de Von Neumann y obliga a que la Unidad de Control (UC) sincronice cuidadosamente los accesos a memoria y al bus.

La instrucción MOV AL, BL, que transfiere el contenido del registro fuente BL al registro destino AL, se modela como una secuencia de eventos discretos y temporizados que representan su ejecución a nivel de Transferencia entre Registros (RTL), utilizando el formalismo xDEVS (eXtended Discrete Event System Specification).
En este modelo, cada componente funcional del procesador ---registros, UC, memoria y bus--- se define como un modelo atómico, mientras que sus interacciones se estructuran mediante un modelo acoplado jerárquico, lo que permite simular comportamientos paralelos, asincrónicos y temporizados.
...
Los componentes que intervienen en el ciclo completo de instrucción ---tanto en la fase de captación (Fetch) como en la de ejecución (Execute)--- se presentan en la siguiente tabla.\\
Cada uno se modela como un \textbf{modelo atómico o acoplado} dentro del entorno xDEVS, con entradas, salidas y retardos propios.
...
\addlinespace
\textbf{REG\_BANK (Banco de Registros)} & Banco de registros que agrupa a AL, BL, CL y DL. Cada registro es un modelo atómico interno que puede leer o escribir a través del bus. El banco gestiona las señales de control (\texttt{enable\_in}, \texttt{enable\_out}) y coordina el acceso a los registros. & \texttt{entradas: enable\_in[n], enable\_out[n]; salida: data\_bus; (n = AL, BL, CL, DL)} & \textbf{$\tau_{regbank} = t_{regbank}$}\\
...
El ciclo de instrucción se compone de dos fases principales, controladas por la Unidad de Control (UC) y modeladas mediante transiciones de estado y funciones temporizadas en el entorno xDEVS:
...
La fase de captación (Fetch) tiene como objetivo leer la instrucción almacenada en la memoria unificada utilizando el valor del Instruction Pointer (IP) y cargarla en el Instruction Register (IR) para su posterior decodificación.
En VonSim8, esta fase modela el uso exclusivo del bus y la memoria compartida, reproduciendo la restricción estructural característica de la arquitectura Von Neumann.

La secuencia de eventos principales es:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  UC → IP: la UC inicia la lectura de la dirección actual del IP.
\item
  IP → MAR: el IP coloca la dirección en el bus y la transfiere al MAR.
\item
  UC → MEM: la UC activa la señal mem\_read y garantiza acceso exclusivo al bus.
\item
  MEM → MBR: la memoria devuelve la instrucción tras un retardo \(\tau_{mem}\).
\item
  MBR → IR: el MBR transfiere la instrucción al IR.
\item
  UC → IR → UC: el IR notifica a la UC que la instrucción está lista para decodificación.
\end{enumerate}
...
El REG\_BANK es un modelo acoplado que contiene cuatro modelos atómicos: AL, BL, CL y DL.
Cada registro posee sus propias entradas (enable\_in, enable\_out, data\_in, data\_out), pero el banco actúa como intermediario, gestionando la habilitación del registro fuente y del registro destino según las señales de la UC.

Microoperaciones del ciclo MOV AL, BL:
...
Estas microoperaciones se ejecutan de manera ordenada y temporizada, según las funciones de transición del formalismo xDEVS, lo que permite representar el flujo de control y la sincronización de eventos entre modelos atómicos.

\hypertarget{conclusiuxf3n}{%
\subsection{Conclusión}\label{conclusiuxf3n}}

El modelado con xDEVS permite describir de manera precisa, modular y extensible el comportamiento del ciclo de instrucción MOV AL, BL en el simulador VonSim8. La simulación refleja cómo la Unidad de Control coordina los accesos a memoria y al bus compartido, respetando la exclusividad de acceso característica de la arquitectura Von Neumann.

Este enfoque facilita el análisis de métricas como el CPI, la latencia de memoria o el tiempo de retención del bus, reforzando el valor pedagógico y analítico de VonSim8, al vincular los conceptos teóricos de la arquitectura clásica con un entorno de simulación formal y configurable.
