...
Esta sección describe los componentes que integran la Unidad Central de Procesamiento (CPU) del simulador VonSim8, destacando su correspondencia con elementos presentes en arquitecturas reales, con el fin de reforzar su valor didáctico. La descripción de cada componente se organiza en función de su rol funcional y su relevancia didáctica.

El diseño de los registros fue concebido para apoyar la comprensión de los modos de direccionamiento y del ciclo de instrucción, considerados ejes fundamentales en la enseñanza de la arquitectura de computadoras. Se describe en la siguiente tabla \ref{tab:arq}:
...
\addlinespace[10pt]
\textbf{} & 2 registros dedicados a la transferencia de información entre la CPU y la memoria principal: MAR (Memory Address Register), que almacena direcciones de memoria, y MBR (Memory Buffer Register), que guarda los datos a transferir.\\
\addlinespace[10pt]
...
\addlinespace[10pt]
\textbf{Instrucciones} & Las instrucciones presentan una longitud variable (1, 2 o 3 bytes), y admiten múltiples modos de direccionamiento: registro a registro, directo, indirecto e inmediato.\\
\addlinespace[10pt]
\bottomrule
\end{tabular}}
\end{table}
...
El repertorio de instrucciones fue diseñado como una simplificación deliberada de la arquitectura x86, con el propósito de optimizar los procesos de enseñanza y aprendizaje en el contexto educativo. En una etapa inicial del proceso de enseñanza, se incluyen únicamente aquellas instrucciones esenciales que permiten introducir progresivamente los contenidos básicos de la asignatura Arquitectura de Computadoras. Este enfoque gradual facilita la comprensión de los conceptos fundamentales sin abrumar al estudiante con la complejidad inherente a un repertorio completo de instrucciones.
La Tabla \ref{tab:setreducido} presenta un repertorio reducido de instrucciones, que incluye las operaciones más comunes y relevantes para el aprendizaje inicial. Este repertorio se centra en las instrucciones de transferencia y procesamiento de datos, así como en las instrucciones de control de flujo, permitiendo al estudiante familiarizarse con los conceptos básicos de la arquitectura x86 sin la complejidad adicional de un repertorio completo.
...
\begin{itemize}
\item
  \texttt{00}: Indica que el operando es un registro de propósito general.
\item
