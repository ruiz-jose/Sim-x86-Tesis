...
Tomando como punto de partida los objetivos didácticos definidos en el capítulo introductorio (\ref{intro}), esta sección presenta los requisitos funcionales y pedagógicos que guiaron el diseño del simulador. Dichos requisitos no solo responden a principios técnicos y educativos, sino que también fueron validados en colaboración con docentes especializados en la enseñanza de Arquitectura de Computadoras.
...
\tightlist
\item
  Permitir la ejecución paso a paso y la ejecución completa de programas escritos en lenguaje ensamblador. Esta funcionalidad favorece una comprensión analítica del comportamiento de cada instrucción, promoviendo el desarrollo de habilidades en el trazado y depuración de código bajo un enfoque de bajo nivel, lo cual es esencial para interiorizar el vínculo entre software y hardware.
\end{itemize}
...
\tightlist
\item
  Implementar una visualización interactiva del flujo de datos utilizando el nivel de transferencia entre registros (Register Transfer Level, RTL). Este modelo describe, con precisión temporal y estructural, el desplazamiento de datos entre registros, buses y unidades funcionales internas del procesador durante cada etapa del ciclo de instrucción. Stallings \protect\hyperlink{ref-stallings_computer_2021}{{[}14{]}} propone utilizar el nivel de transferencia entre registros (RTL) para representar el ciclo de instrucción, desde la captura (\emph{fetch}) hasta la ejecución (\emph{execute}). Este enfoque permite visualizar el recorrido de los datos y la activación de señales de control en cada etapa, lo cual facilita la comprensión del funcionamiento interno del procesador.
\end{itemize}
...
\tightlist
\item
  Incluir indicadores clave como tiempo de ciclo, tiempo de CPU y ciclos por instrucción (CPI), generados automáticamente a partir de la ejecución de los programas. Estos datos deben mostrarse en una interfaz dedicada para facilitar su análisis. Su incorporación responde a la necesidad de promover el desarrollo de habilidades de análisis de rendimiento, como señalan Hennessy y Patterson \protect\hyperlink{ref-hennessy2017computer}{{[}13{]}}.
\end{itemize}
