...
\item
  ...
  \tightlist
  \item
    Visualizar el flujo de datos mediante una representación en el nivel de transferencia entre registros (Register Transfer Level, RTL), que describe cómo los datos se mueven entre componentes internos como registros y buses durante cada etapa del ciclo de instrucción. Es común en los de Stallings \protect\hyperlink{ref-stallings_computer_2021}{{[}14{]}} la utilización de RTL para representar cada etapa del ciclo de instrucción, desde la captura (`fetch') hasta la ejecución (`execute'). Este enfoque facilita la comprensión del procesador, ya que permite visualizar el recorrido de los datos entre registros y la activación de señales de control en cada etapa.
  \end{itemize}

  A continuación se ilustra un ciclo de instrucción típico del repertorio x86, empleando como ejemplo la operación MOV AL, BL, que transfiere el contenido del registro BL al registro AL. La Figura \ref{fig:cicloinstruccion} representa las etapas del ciclo de instrucción, destacando el flujo de datos y las señales de control activadas en cada fase. Esta representación gráfica, basada en el modelo RTL (Register Transfer Level), permite comprender el funcionamiento interno del procesador desde la recuperación hasta la ejecución de la instrucción.
...
Los requisitos identificados fueron analizados en conjunto con docentes para determinar qué elementos de la arquitectura x86 debían ser representados, simplificados o adaptados. Este análisis guió la selección de una arquitectura reducida de 8 bits y la definición de los módulos funcionales del simulador.
...
Una vez establecidos los requisitos funcionales y didácticos de la herramienta, se procede a traducirlos en un diseño conceptual que organice su estructura interna. Esta arquitectura operativa se estructura en bloques funcionales, cada uno con responsabilidades claramente definidas.
...
\subsection{Características del CPU}\label{caracteruxedsticas-del-cpu}}
