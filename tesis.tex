...
\hypertarget{funciones-del-repertorio-de-instrucciones}{%
\subsection{Funciones del repertorio de instrucciones}\label{funciones-del-repertorio-de-instrucciones}}

El repertorio de instrucciones define las operaciones que un procesador puede realizar y cómo se ejecutan. Sus funciones principales incluyen {[}\protect\hyperlink{ref-stallings_computer_2013}{14}{]}:

\begin{itemize}
\item
  \textbf{Control de la CPU}: Instrucciones de control que gestionan la secuencia de ejecución, interrupciones y llamadas a subrutinas, asegurando que el procesador funcione de manera ordenada y eficiente.
\item
  \textbf{Interacción con la memoria}: Las instrucciones permiten que la CPU cargue y almacene datos desde y hacia la memoria principal, un aspecto crítico para la ejecución de programas y la optimización del rendimiento global del sistema.
\item
  \textbf{Gestión de datos}: Las operaciones aritméticas, lógicas y de comparación son esenciales para la manipulación de datos. Un diseño adecuado del repertorio puede simplificar y acelerar estas operaciones, mejorando tanto el rendimiento como el consumo energético del sistema.
\item
  \textbf{Optimización del rendimiento}: Un repertorio de instrucciones eficiente minimiza el número de instrucciones necesarias para completar una tarea, lo que contribuye a un uso más eficaz de los recursos del sistema, tanto en términos de energía como de tiempo de procesamiento.
\end{itemize}

\hypertarget{modos-de-direccionamiento}{%
\subsection{Modos de direccionamiento}\label{modos-de-direccionamiento}}

Los modos de direccionamiento son fundamentales para definir cómo una CPU accede a los datos para ejecutar una instrucción {[}\protect\hyperlink{ref-hennessy_computer_2012}{13}{]}, {[}\protect\hyperlink{ref-stallings_computer_2013}{14}{]}. Algunos modos comunes incluyen:

\begin{enumerate}
\def\labelenumi{\alph{enumi})}
\tightlist
\item
  \textbf{Inmediato}: El operando está embebido en la propia instrucción, proporcionando acceso rápido a valores constantes pequeños.
\item
  \textbf{Directo}: La dirección de memoria del operando está incluida en la instrucción. Es sencillo, pero limita el rango de direcciones accesibles directamente.
\item
  \textbf{Indirecto}: La dirección de memoria apunta a otra dirección donde se encuentra el operando real, permitiendo un acceso más amplio, pero a un costo de tiempo.
\item
  \textbf{Registro}: Utiliza un registro en lugar de una dirección de memoria, acelerando el acceso a datos.
\item
  \textbf{Registro Indirecto}: Similar al modo indirecto, pero con la dirección almacenada en un registro.
\item
  \textbf{Con Desplazamiento}: Combina una dirección base con un desplazamiento, útil en estructuras de datos como arrays.
\item
  \textbf{Pila}: Calcula la dirección del operando en función de la ubicación actual en la pila, usado comúnmente en llamadas a subrutinas.
\end{enumerate}
...
El formato de las instrucciones determina cómo se estructuran las instrucciones en términos de longitud, número de operandos y campos adicionales como el código de operación (\textbf{opcode}) {[}\protect\hyperlink{ref-hennessy_computer_2012}{13}{]}, {[}\protect\hyperlink{ref-tanenbaum_structured_2013}{24}{]}. El formato tiene un impacto directo en la complejidad del diseño del procesador:

\begin{itemize}
\item
  \textbf{Longitud de la instrucción}: Las instrucciones pueden ser de longitud fija o variable. Las instrucciones de longitud fija son más fáciles de decodificar, pero pueden desperdiciar espacio en casos donde no se necesite la flexibilidad de una instrucción larga.
\item
  \textbf{Cantidad de operandos}: Dependiendo de la arquitectura, las instrucciones pueden manejar desde cero hasta tres o más operandos, impactando tanto la flexibilidad como la complejidad del procesador.
\item
  \textbf{Campos de instrucción}: Un campo común es el \textbf{opcode}, que indica la operación a realizar, mientras que los operandos pueden estar representados en campos adicionales. Otros campos pueden incluir modos de direccionamiento o flags de condición.
\end{itemize}

\hypertarget{filosofuxedas-cisc-y-risc}{%
\section{Filosofías CISC y RISC}\label{filosofuxedas-cisc-y-risc}}

Las arquitecturas del repertorio de instrucciones son un componente crucial en el diseño de procesadores. Dos de las filosofías de diseño más influyentes en este ámbito son \textbf{CISC (Complex Instruction Set Computing)} y \textbf{RISC (Reduced Instruction Set Computing)}. Mientras que CISC busca minimizar la cantidad de instrucciones necesarias para realizar una tarea, RISC simplifica el conjunto de instrucciones para optimizar la velocidad y la eficiencia energética. Esta sección profundiza estos dos enfoques y sus implicaciones en el diseño de procesadores {[}\protect\hyperlink{ref-hennessy_computer_2012}{13}{]}, {[}\protect\hyperlink{ref-patterson_computer_2014}{23}{]}.

\hypertarget{cisc}{%
\subsection{CISC}\label{cisc}}

En las arquitecturas \textbf{CISC}, como la x86, el objetivo principal es reducir la cantidad de instrucciones necesarias para realizar operaciones complejas. Esto se logra implementando instrucciones que pueden realizar varias operaciones en un solo ciclo de instrucción, lo cual reduce el código que debe escribir un programador. Sin embargo, esta ventaja tiene un costo: la \textbf{decodificación} y \textbf{ejecución} de instrucciones CISC requiere un hardware más complejo. Además, las instrucciones de longitud variable, comunes en CISC, pueden incrementar el tiempo de decodificación, generando cuellos de botella en el pipeline.

Por ejemplo, los procesadores \textbf{x86} han evolucionado hacia una arquitectura híbrida, utilizando microcódigo para descomponer las instrucciones CISC en operaciones más simples, similares a las de una arquitectura RISC. Esto mejora la eficiencia de ejecución en algunos contextos, pero sigue siendo una arquitectura más costosa en términos de diseño y energía {[}\protect\hyperlink{ref-patterson_computer_2014}{23}{]}.

\hypertarget{risc}{%
\subsection{RISC}\label{risc}}

Por otro lado, las arquitecturas \textbf{RISC} como \textbf{ARM} y \textbf{MIPS} se centran en un conjunto de instrucciones más reducido y de longitud fija, lo que simplifica la decodificación y permite ejecutar instrucciones en un solo ciclo de reloj. Esta simplicidad también permite implementar técnicas avanzadas de \textbf{pipelining} y \textbf{predicción de ramas} con mayor facilidad, mejorando el rendimiento en operaciones básicas. A nivel de hardware, RISC facilita la \textbf{optimización} y el uso de energía de manera más eficiente que CISC, lo que es especialmente importante en dispositivos móviles y embebidos {[}\protect\hyperlink{ref-hennessy_computer_2012}{13}{]}.

La filosofía de diseño RISC ha permitido que procesadores como \textbf{ARM} dominen el mercado de dispositivos móviles, donde la eficiencia energética es crítica. El enfoque de RISC en operaciones simples y repetitivas, con un CPI (ciclos por instrucción) bajo, ha sido un factor clave para esta adopción {[}\protect\hyperlink{ref-hennessy_computer_2012}{13}{]}.
