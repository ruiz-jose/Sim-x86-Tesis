...
Con el propósito de validar la arquitectura del simulador VonSim8 y analizar su comportamiento interno, se modela la instrucción MOV AL, BL como un caso de estudio representativo del flujo básico de ejecución a nivel de hardware. VonSim8 se fundamenta en la arquitectura de Von Neumann, caracterizada por la utilización de un bus único compartido para instrucciones y datos, lo cual introduce el clásico cuello de botella estructural. En este contexto, la Unidad de Control (UC) debe sincronizar con precisión los accesos concurrentes a memoria y al bus.

Esta implementación aplica el formalismo DEVS (Discrete EVents System Specification), introducido en el capítulo \ref{devs}, para representar la ejecución a nivel de Transferencia entre Registros (RTL) como una secuencia de eventos discretos temporizados. La simulación se construye sobre la plataforma xDEVS, utilizando la biblioteca xdevs.py (Python), como se justificó en la sección \ref{xdevs}.

De acuerdo con el formalismo DEVS, cada componente funcional del procesador ---como los registros, la Unidad de Control (UC), la memoria y el bus--- se modela como un modelo atómico, mientras que las interacciones entre ellos se describen mediante un modelo acoplado jerárquico. Esta estructura permite capturar la dinámica de concurrencia y la propagación de eventos en distintos niveles de abstracción.
...
La tabla \ref{tab:componentesvonsim8} resume los modelos y componentes principales del simulador VonSim8, detallando sus funciones, entradas/salidas y retardos asociados.
...
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\fontsize{10}{12}\selectfont
\begin{tabular}[t]{>{\raggedright\arraybackslash}p{3.5cm}|>{\raggedright\arraybackslash}p{7cm}>{\raggedright\arraybackslash}p{6cm}>{\raggedright\arraybackslash}p{2.5cm}}
\toprule
\multicolumn{1}{>{\centering\arraybackslash}p{3.5cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Modelo}}} & \multicolumn{1}{>{\centering\arraybackslash}p{7cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Descripción}}} & \multicolumn{1}{>{\centering\arraybackslash}p{6cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Entradas / Salidas}}} & \multicolumn{1}{>{\centering\arraybackslash}p{2.5cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Retardo}}}\\
\midrule
\textbf{IP (Instruction Pointer)} & Contiene la dirección de la próxima instrucción a ejecutar. Envía su valor al MAR. & \texttt{salida: addr\_out; entrada: ip\_write, branch\_target} & \textbf{$\tau_{ip} = t_{ip}$}\\
...
\def\labelenumi{\arabic{enumi}.}
\item
  Fase de captación (Fetch): lectura de la instrucción desde la memoria.
\item
...
La fase de captación (Fetch) tiene como objetivo obtener la instrucción almacenada en la memoria unificada, utilizando el valor del Instruction Pointer (IP) como dirección base, y transferirla al Instruction Register (IR) para su posterior decodificación. Este proceso marca el inicio del ciclo de instrucción y establece la secuencia temporal de acceso a memoria en la arquitectura Von Neumann. En el simulador VonSim8, esta fase reproduce la restricción estructural propia de la arquitectura Von Neumann, al modelar el uso exclusivo y secuencial del bus y de la memoria compartida durante la captación de la instrucción.

La tabla \ref{tab:microcaptacionfetch} ilustra la secuencia de eventos principales y microoperaciones que implementan esta fase.

\begin{table}[!h]
\centering
\caption{\label{tab:microcaptacionfetch}Secuencia de microoperaciones y eventos DEVS para la Fase de Captación (Fetch)}
\centering
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\fontsize{10}{12}\selectfont
\begin{tabular}[t]{>{\raggedright\arraybackslash}p{1.5cm}|>{\raggedright\arraybackslash}p{3.5cm}>{\raggedright\arraybackslash}p{8cm}>{\raggedright\arraybackslash}p{7cm}}
\toprule
\multicolumn{1}{>{\centering\arraybackslash}p{1.5cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Paso}}} & \multicolumn{1}{>{\centering\arraybackslash}p{3.5cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Microoperación}}} & \multicolumn{1}{>{\centering\arraybackslash}p{8cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Descripción}}} & \multicolumn{1}{>{\centering\arraybackslash}p{7cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Modelos y funciones DEVS}}}\\
\midrule
...
\textbf{3} & UC $\rightarrow$ MEM & La UC activa la señal mem\_read para iniciar la lectura de la instrucción y, de forma concurrente, genera una señal de incremento sobre el IP +1 . & \textbf{$\lambda(UC) \rightarrow \delta_{ext}(MEM);\lambda(UC) \rightarrow \delta_{ext}(IP)$}\\
\textbf{4} & MEM $\rightarrow$ MBR & La memoria unificada entrega la instrucción solicitada tras su retardo interno. & \textbf{$\delta_{int}(MEM) \rightarrow \lambda(MEM) \rightarrow \delta_{ext}(MBR)$}\\
\textbf{5} & MBR $\rightarrow$ IR & El MBR transfiere la instrucción al registro de instrucción. & \textbf{$\lambda(MBR) \rightarrow \delta_{ext}(IR)$}\\
\addlinespace
\textbf{6} & IR $\rightarrow$ UC & El IR genera un evento indicando que la instrucción está lista para decodificación. & \textbf{$\lambda(IR) \rightarrow \delta_{ext}(UC)$}\\
\bottomrule
\end{tabular}}
\end{table}

Durante este proceso, el bus permanece bloqueado para otros accesos hasta completar la lectura de la instrucción.
...
Durante la fase de ejecución, la UC decodifica el contenido del IR y activa las señales necesarias para que el Banco de Registros (REG\_BANK) permita la transferencia del dato desde BL hacia AL a través del bus compartido.

El REG\_BANK es un modelo acoplado que contiene cuatro modelos atómicos: AL, BL, CL y DL. Cada registro posee sus propias entradas (enable\_in, enable\_out, data\_in, data\_out), pero el banco actúa como intermediario, gestionando la habilitación del registro fuente y del registro destino según las señales de la UC.

La tabla \ref{tab:microoperacionesMOV} ilustra las microoperaciones que implementan esta instrucción.

\begin{table}[!h]
\centering
\caption{\label{tab:microoperacionesMOV}Secuencia de microoperaciones para MOV AL, BL utilizando el Banco de Registros}
\centering
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\fontsize{10}{12}\selectfont
\begin{tabular}[t]{llll}
\toprule
Paso & Microoperación & Descripción & Modelos y funciones xDEVS\\
\midrule
1 & UC: decodificar(IR) & La UC analiza el opcode MOV y determina los operandos AL y BL. & $\delta_{\text{ext}}(UC)$ recibe evento del IR y actualiza su estado con la instrucción actual.\\
2 & UC $\rightarrow$ REG\_BANK.enable\_out(BL) = 1 & El Banco de Registros activa la salida del registro BL. & $\lambda_{UC}$ genera evento hacia REG\_BANK(BL).\\
3 & BUS $\leftarrow$ BL & El valor del registro BL se transmite por el bus. & $\lambda_{BL}$ emite su valor al bus.\\
4 & UC $\rightarrow$ REG\_BANK.enable\_in(AL) = 1 & La UC habilita la entrada del registro AL dentro del Banco de Registros. & $\lambda_{UC}$ activa señal de escritura hacia REG\_BANK(AL).\\
5 & AL $\leftarrow$ BUS & El registro AL captura el dato del bus y actualiza su valor interno. & $\delta_{\text{ext}}(AL)$ recibe el dato desde el bus y actualiza su estado $S$.\\
\bottomrule
\end{tabular}}
\end{table}

Las microoperaciones se ejecutan de manera secuencial y cronometrada conforme a las funciones de transición (\(\delta_{int}, \delta_{ext}\)) y de salida (\(\lambda\)) definidas en el formalismo DEVS, lo que posibilita representar con precisión el flujo de control y la sincronización de eventos entre los modelos atómicos.

\hypertarget{conclusiuxf3n}{%
\subsection{Conclusión}\label{conclusiuxf3n}}

En síntesis, el modelado en la plataforma xDEVS (usando xdevs.py) permite describir de manera precisa, modular y extensible el comportamiento temporal de la instrucción MOV AL, BL en el simulador VonSim8. La simulación refleja cómo la Unidad de Control gestiona los accesos secuenciales al bus y a la memoria compartida, respetando la restricción estructural de la arquitectura de Von Neumann.

Además de representar las dependencias estructurales, este enfoque de modelado permite instrumentar métricas de rendimiento tales como el número de ciclos por instrucción (CPI), la latencia promedio de memoria y el tiempo de retención del bus, proporcionando una base cuantitativa para la validación del simulador.

Desde una perspectiva pedagógica, el modelo facilita la visualización explícita de las dependencias funcionales entre los componentes internos del procesador y permite comprender el impacto del bus compartido sobre el rendimiento global del sistema.
