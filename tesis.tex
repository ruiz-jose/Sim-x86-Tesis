...
La Figura \ref{fig:flujoCicloInstruccion} muestra el flujo general de este ciclo, que se divide en dos etapas principales: Captación (fetch) y ejecución.
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{\texttt{MAR} \(\leftarrow\) \texttt{IP}}: La UC transfiere el contenido del \textbf{Puntero de Instrucción} (\texttt{IP}) al \textbf{Registro de Direcciones de Memoria} (\texttt{MAR}). El \texttt{IP} contiene la dirección de la próxima instrucción a ejecutar, por lo que este paso prepara el sistema para acceder a esa ubicación en la memoria.
\item
  \textbf{\texttt{MBR} \(\leftarrow\) \texttt{read(Memoria{[}MAR{]})} \textbar{} \texttt{IP} \(\leftarrow\) \texttt{IP} + 1}: La UC activa la señal de lectura para obtener la instrucción en la dirección especificada por el \texttt{MAR}. El dato leído se almacena en el \textbf{Registro de Datos de Memoria} (\texttt{MBR}) a través del bus de datos. Simultáneamente, el \texttt{IP} se incrementa en uno para apuntar a la siguiente instrucción u operando.
\item
  \textbf{\texttt{IR} \(\leftarrow\) \texttt{MBR}}: Finalmente, el contenido del \texttt{MBR} se transfiere al \textbf{Registro de Instrucciones} (\texttt{IR}). En este punto, la instrucción ha sido captada y está lista para ser ejecutada.
\end{enumerate}
...
\textbf{Instrucciones con destino registro (Rx)}

Modo de direccionamiento: Entre registros (\texttt{Rx}, \texttt{Ry})
4. Las instrucciones posibles son:
...
- CMP: \textbf{\texttt{Rx} - \texttt{Ry}; update(Flags)} (Solo actualiza los flags, no almacena el resultado)
...
8. Luego se realiza la operación:
...
Si es \texttt{ADD} o \texttt{SUB}, el resultado se escribe en memoria:
9. ADD/SUB: \textbf{\texttt{write(Memoria{[}MAR{]})} \(\leftarrow\) \texttt{MBR}} CMP no realiza este paso.

Modo de direccionamiento: Indirecto (\texttt{{[}BL{]}}, \texttt{Ry})
4. \textbf{\texttt{MAR} \(\leftarrow\) \texttt{IP}}: Se obtiene la dirección del operando destino.
5. \textbf{\texttt{MBR} \(\leftarrow\) \texttt{read(Memoria{[}MAR{]})} \textbar{} \texttt{IP} \(\leftarrow\) \texttt{IP} + 1}: Se lee la dirección de memoria y se almacena en MBR; el IP se incrementa.
6. \textbf{\texttt{MAR} \(\leftarrow\) \texttt{BL}}: El contenido de BL, que es la dirección del operando destino, se transfiere a MAR.
Si la instrucción es MOV:
7. MOV: \textbf{\texttt{MBR} \(\leftarrow\) \texttt{Ry}}: El contenido del registro Ry se mueve a MBR.
8. MOV: \textbf{\texttt{write(Memoria{[}MAR{]})} \(\leftarrow\) \texttt{MBR}}: El dato de MBR se escribe en la memoria.

Si la instrucción es ADD, SUB, CMP:
7. ADD/SUB/CMP: \textbf{\texttt{MBR} \(\leftarrow\) \texttt{read(Memoria{[}MAR{]})}}: Se lee el dato del operando destino en la memoria y se almacena en MBR.
8. Luego se realiza la operación:
- ADD: \textbf{\texttt{MBR} \(\leftarrow\) \texttt{MBR} + \texttt{Ry} \textbar{} update(Flags)}
- SUB: \textbf{\texttt{MBR} \(\leftarrow\) \texttt{MBR} - \texttt{Ry} \textbar{} update(Flags)}
- CMP: \textbf{\texttt{MBR} - \texttt{Ry}; update(Flags)}
...
Modo de direccionamiento: Inmediato (\texttt{{[}Dirección{]}}, \texttt{Dato})
4. \textbf{\texttt{MAR} \(\leftarrow\) \texttt{IP}}: Se obtiene la dirección del operando destino.
5. \textbf{\texttt{MBR} \(\leftarrow\) \texttt{read(Memoria{[}MAR{]})} \textbar{} \texttt{IP} \(\leftarrow\) \texttt{IP} + 1}: Se lee la dirección de memoria y se almacena en MBR; el IP se incrementa.
6. \textbf{\texttt{MAR} \(\leftarrow\) \texttt{IP};\texttt{ri} \(\leftarrow\) \texttt{MBR}}: El IP se transfiere a MAR para obtener el dato inmediato y la dirección de destino se guarda en un registro intermedio (ri).
7. \textbf{\texttt{MBR} \(\leftarrow\) \texttt{read(Memoria{[}MAR{]})} \textbar{} \texttt{IP} \(\leftarrow\) \texttt{IP} + 1}: Se lee el dato inmediato y se guarda en MBR; el IP se incrementa.

Si la instrucción es MOV:
8. MOV:\textbf{\texttt{MAR} \(\leftarrow\) \texttt{ri}}: La dirección de destino se transfiere de ri a MAR.
9. MOV: \textbf{\texttt{write(Memoria{[}MAR{]})} \(\leftarrow\) \texttt{MBR}}

Si la instrucción es ADD, SUB, CMP:
8. ADD/SUB/CMP: \textbf{\texttt{MAR} \(\leftarrow\) \texttt{ri} \textbar{} \texttt{id} \(\leftarrow\) \texttt{MBR}}:el contenido de ri se transfiere al registro MAR y simultaneamente se transfiere el valor de MBR a un registro intermedio id
9. \textbf{\texttt{MBR} \(\leftarrow\) \texttt{read(Memoria{[}MAR{]})}}
10. Luego se realiza la operación:
...
Si es \texttt{ADD} o \texttt{SUB}, el resultado se escribe en memoria:
11. ADD/SUB: \textbf{\texttt{write(Memoria{[}MAR{]})} \(\leftarrow\) \texttt{MBR}} CMP no realiza este paso.

Modo de direccionamiento: Inmediato (\texttt{{[}BL{]}}, \texttt{Dato})
4. \textbf{\texttt{MAR} \(\leftarrow\) \texttt{IP}}: Se obtiene la dirección del operando destino.
5. \textbf{\texttt{MBR} \(\leftarrow\) \texttt{read(Memoria{[}MAR{]})} \textbar{} \texttt{IP} \(\leftarrow\) \texttt{IP} + 1}: Se lee la dirección de memoria y se almacena en MBR; el IP se incrementa.
6. \textbf{\texttt{MAR} \(\leftarrow\) \texttt{BL}}: El contenido de BL, que es la dirección de destino, se transfiere a MAR.

Si la instrucción es MOV:
7. MOV: \textbf{\texttt{write(Memoria{[}MAR{]})} \(\leftarrow\) \texttt{MBR}}

Si la instrucción es ADD, SUB, CMP:\\
6. ADD/SUB/CMP: \textbf{\texttt{MAR} \(\leftarrow\) \texttt{BL} \textbar{} \texttt{id} \(\leftarrow\) \texttt{MBR}}:El contenido de BL, que es la dirección de destino, se transfiere a MAR y simultaneamente el dato inmediato del MBR se transfiere a un registro temporal (id).
7. ADD/SUB/CMP: \textbf{\texttt{MBR} \(\leftarrow\) \texttt{read(Memoria{[}MAR{]})}}: Se lee el operando destino de la memoria.
8. Luego se realiza la operación:\\
- ADD: \textbf{\texttt{MBR} \(\leftarrow\) \texttt{MBR} + \texttt{id}; update(Flags)}
- SUB: \textbf{\texttt{MBR} \(\leftarrow\) \texttt{MBR} - \texttt{id}; update(Flags)}
- CMP: \textbf{\texttt{MBR} - \texttt{id}; update(Flags)}
...
4. \textbf{\texttt{MAR} \(\leftarrow\) \texttt{IP}}: El IP se transfiere al MAR.
5. \textbf{\texttt{MBR} \(\leftarrow\) \texttt{read(Memoria{[}MAR{]})}; \texttt{IP} \(\leftarrow\) \texttt{IP} + 1}: Se lee la dirección de destino y se almacena en el MBR; el IP se incrementa.

Si es salto incondicional \texttt{JMP} :
6. JMP: \textbf{\texttt{IP} \(\leftarrow\) \texttt{MBR}}

Si es salto condicional \texttt{Jxx} :
6. Jxx: Si la condición del flag \texttt{xx} es verdadera, \textbf{\texttt{IP} \(\leftarrow\) \texttt{MBR}}. Si no, se continúa con la siguiente instrucción.
