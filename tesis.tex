...
\textbar{} Componente \textbar{} Tipo \textbar{} Descripción \textbar{}
\textbar{} --------------------------------- \textbar{} ----------------------------- \textbar{} ------------------------------------------------------------------ \textbar{}
\textbar{} \textbf{UC} (Unidad de Control) \textbar{} Atómico \textbar{} Decodifica \texttt{IR}, genera señales de control para lectura/escritura. \textbar{}
\textbar{} \textbf{IR} (Registro de Instrucción) \textbar{} Atómico \textbar{} Contiene el código binario de la instrucción. \textbar{}
\textbar{} \textbf{AL} (Registro Acumulador bajo) \textbar{} Atómico \textbar{} Registro destino. \textbar{}
\textbar{} \textbf{BL} (Registro base bajo) \textbar{} Atómico \textbar{} Registro fuente. \textbar{}
\textbar{} \textbf{BUS} (Bus de datos) \textbar{} Atómico o variable compartida \textbar{} Transporta el dato desde el origen al destino. \textbar{}

\hypertarget{definiciuxf3n-devs-simplificada-de-cada-modelo}{%
\subsubsection{Definición DEVS simplificada de cada modelo}\label{definiciuxf3n-devs-simplificada-de-cada-modelo}}

\textbf{Modelo Atómico: Registro (ej. AL o BL)}

El modelo atómico del \textbf{Registro} se define como:

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.0209}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.1257}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.3979}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.4555}}@{}}
\toprule\noalign{}
...
Descripción
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Modelos y funciones DEVS
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
1 & \texttt{UC:\ decodificar(IR)} & La UC lee el valor del IR y reconoce el opcode \texttt{MOV} con operandos \texttt{AL,\ BL}. & \texttt{/delta\_ext} (UC) recibe evento del IR; actualiza su estado (\texttt{S}) con la instrucción actual. \\
2 & \texttt{UC\ →\ BL.enable\_out\ =\ 1} & La UC activa la señal de lectura del registro fuente. & \texttt{/lambda} (UC) genera evento de salida hacia BL. \\
3 & \texttt{BUS\ ←\ BL} & El registro BL coloca su valor en el bus de datos. & \texttt{/lambda} (BL) emite valor actual al BUS. \\
4 & \texttt{UC\ →\ AL.enable\_in\ =\ 1} & La UC habilita la escritura en AL. & \texttt{/lambda} (UC) envía evento de control a AL. \\
5 & \texttt{AL\ ←\ BUS} & El registro AL captura el dato del bus y actualiza su estado interno. & \texttt{/delta\_ext} (AL) recibe valor del BUS y actualiza su estado \texttt{S}. \\
\end{longtable}

\hypertarget{diagrama-de-interacciuxf3n-resumen-conceptual}{%
\subsubsection{Diagrama de interacción (resumen conceptual)}\label{diagrama-de-interacciuxf3n-resumen-conceptual}}

\hypertarget{resultados-de-simulaciuxf3n-esperados-en-vonsim8}{%
\subsubsection{Resultados de simulación esperados (en VonSim8)}\label{resultados-de-simulaciuxf3n-esperados-en-vonsim8}}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.5300}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Tiempo (t)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\bottomrule\noalign{}
\endlastfoot
t & Captación completada & IR & Contiene \texttt{MOV\ AL,\ BL} \\
t & Decodificación & UC & Genera señales de control \\
t & Lectura de BL & BL & Emite valor al bus \\
t & Escritura en AL & AL & Guarda el valor de BL \\
t & Fin de instrucción & UC & Pasa al siguiente ciclo (fetch siguiente instrucción) \\
\end{longtable}
