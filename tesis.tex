...
\item
  ...
  \begin{table}[!h]
   ...
   \addlinespace[10pt]
   \textbf{Avanzada} & CALL, RET, IN, OUT, INT, IRET, POP, PUSH, INC, DEC, AND, OR, SHL, SHR, CLI, STI & Manejo de periféricos e interrupciones\\
   \addlinespace[10pt]
   \bottomrule
   \end{tabular}}
   \end{table}
...
\addlinespace[10pt]
\textbf{Registros} & 4 registros de propósito general de 8 bits (`AL`, `BL`, `CL` y `DL`).\\
\addlinespace[10pt]
\textbf{} & 6 registros de propósito específico:\begin{itemize} \item `IP` (Instruction Pointer) \item `IR` (Instruction Register) \item `SP` (Stack Pointer) \item `RS` (registro de estado) \item `MAR` (Memory Address Register) \item `MBR` (Memory Buffer Register) \end{itemize}\\
\addlinespace[10pt]
\textbf{} & Los registros de propósito general pueden ser accedidos y modificados por el programador. Los específicos son gestionados por la CPU.\\
\addlinespace[10pt]
\textbf{} & Memoria principal de 256 bytes, direccionada por un bus de direcciones de 8 bits. Cada posición de memoria almacena un byte. La memoria se organiza en celdas de 16 bits, con una dirección inicial de `0x00` y una dirección final de `0xFF`.\\
\addlinespace[10pt]
\addlinespace
\textbf{Memoria} & Sistema de buses compuesto por un bus de datos y un bus de direcciones, ambos de 8 bits, lo cual limita el espacio de direccionamiento a 256 posiciones de memoria. Un bus de control de 4 bits que gestiona las señales de control entre la CPU, la memoria y los dispositivos periféricos. Conformado por rd (read), wr (write)\\
\addlinespace[10pt]
Puertos & 0- 8 bits de direcciones de memoria (bus de direcciones, con su respectivo buffer `MAR`)
- 8 bits de datos (bus de datos, con su respectivo buffer `MBR`)
- 1 bit para la señal de lectura `rd`
- 1 bit para la señal de escritura `wr`
- 1 bit para indicar si la escritura es a un módulo de entrada/salida o a memoria principal `IO/M`
- 1 bit para la petición de interrupción `INTR`
\textbf{- 1 bit para la señal de acknowledge de interrupción `INTA`}\\
\addlinespace[10pt]
\textbf{Instrucciones} & El repertorio de instrucciones incluye operaciones aritméticas, lógicas, de transferencia de datos y control de flujo. Las instrucciones poseen una longitud variable (1, 2 o 3 bytes) y admiten diversos modos de direccionamiento: registro a registro, directo, indirecto e inmediato.\\
\addlinespace[10pt]
\bottomrule
\end{tabular}}
\end{table}
...
\#\#\#\# Destino registro
* MOV \texttt{Rx}, \texttt{Ry} (Entre registros)
4. \textbf{\texttt{Rx} \(\leftarrow\) \texttt{Ry}}: El valor almacenado en Ry se copia en el registro Rx.

\begin{itemize}
\tightlist
\item
  MOV \texttt{Rx}, \texttt{{[}Dirección{]}} (Directo)
  ...
  \tightlist
  \item
    \textbf{\texttt{MAR} \(\leftarrow\) \texttt{IP}}: El contenido de IP se transfiere al registro MAR.
  \item
  ...
  \item
    \textbf{\texttt{MAR} \(\leftarrow\) \texttt{MBR}}: El valor de MBR se transfiere a MAR.
  \item
    \textbf{\texttt{MBR} \(\leftarrow\) \texttt{read(Memoria{[}MAR{]})}}: Se lee el dato de la memoria en la nueva dirección de MAR y se almacena en MBR.
  \item
...
\item
  ...
  \item
    ...
    \end{enumerate}
  \item
    \textbf{\texttt{ri} \(\leftarrow\) \texttt{MBR}}: El valor de MBR se transfiere a un registro intermedio (ri).\\
  \item
    \begin{enumerate}
    \def\labelenumii{\arabic{enumii}.}
    \setcounter{enumii}{7}
...
\#\#\#\# Destino registro
* ADD \texttt{Rx}, \texttt{Ry} (Entre registros)
4. \textbf{\texttt{Rx} \(\leftarrow\) \texttt{Rx} + \texttt{Ry}; update(Flags)}
...
\item
  ...
  \item
    ...
    \end{enumerate}
  \item
    \textbf{\texttt{ri} \(\leftarrow\) \texttt{MBR}}
  \item
    \begin{enumerate}
    \def\labelenumii{\arabic{enumii}.}
    \setcounter{enumii}{7}
