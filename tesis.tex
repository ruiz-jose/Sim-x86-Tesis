...
Por ejemplo, los procesadores \textbf{x86} que ha evolucionado hacia un enfoque híbrido. Utiliza microcódigo para descomponer las instrucciones complejas en operaciones más simples, parecidas a las de un procesador RISC. Aunque esta estrategia mejora la eficiencia de ejecución en algunos casos, el diseño sigue siendo más costoso en términos de consumo energético y complejidad {[}\protect\hyperlink{ref-patterson_computer_2014}{23}{]}.
...
La tabla \ref{tab:comparativaciscrisc} resume las principales diferencias entre estas filosofías.
...
\caption{\label{tab:hitosx86}Hitos procesadores x86}\tabularnewline
\toprule
...
\begin{longtable}[]{@{}rll@{}}
\caption{\label{tab:evolucionx86}Línea de Tiempo de la Evolución de la Arquitectura x86}\tabularnewline
\toprule
...
La arquitectura x86 destaca por su complejidad y flexibilidad, reflejada en un repertorio de instrucciones extenso y de longitud variable. Esto contrasta con arquitecturas RISC, donde predominan instrucciones de longitud fija y decodificación sencilla {[}\protect\hyperlink{ref-hennessy_computer_2012}{13}{]}. Aunque esta flexibilidad permite adaptaciones a diversas aplicaciones, también implica mayores desafíos en términos de implementación y eficiencia operativa.
...
En el caso de la arquitectura x86, los programadores pueden elegir entre diversos ensambladores, como TASM (Turbo Assembler) {[}\protect\hyperlink{ref-tasm}{28}{]}, MASM (Microsoft Macro Assembler) {[}\protect\hyperlink{ref-masm}{29}{]} y NASM (Netwide Assembler) {[}\protect\hyperlink{ref-nasm}{30}{]}. Aunque cada ensamblador tiene características y sintaxis particulares, todos comparten el objetivo fundamental de convertir las instrucciones ensamblador en código binario ejecutable por los procesadores x86 {[}\protect\hyperlink{ref-hyde2010art}{31}{]}.
...
Estas características transforman la experiencia de aprendizaje, al vincular teoría con práctica y promover un enfoque activo en la comprensión de conceptos clave de arquitectura de computadoras. Al adoptar DEVS como parte del entorno educativo, se potencia la capacidad de los estudiantes para abordar problemas complejos y explorar soluciones innovadoras {[}\protect\hyperlink{ref-calvo_valdes_simulador_2010}{34}{]}.
...
1. \textbf{Representación global de la estructura y funcionamiento de la computadora:}
...
\begin{longtable}[]{@{}ll@{}}
\caption{(\#tab:bloques\_funcionales)Bloque funcionales principales}\tabularnewline
\toprule
...
Unidad Central de Procesamiento (CPU)\strut
\end{minipage} & \begin{minipage}[t]{0.62\columnwidth}\raggedright
Simulación de registros, Unidad de Control (UC) y ALU.\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.32\columnwidth}\raggedright
Memoria\strut
\end{minipage} & \begin{minipage}[t]{0.62\columnwidth}\raggedright
Estructura de memoria y simulación de operaciones de lectura y escritura.\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.32\columnwidth}\raggedright
Sistema de entrada/salida (E/S)\strut
\end{minipage} & \begin{minipage}[t]{0.62\columnwidth}\raggedright
Interacción con periféricos y manejo de interrupciones.\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.32\columnwidth}\raggedright
Bus de datos, direcciones y control\strut
\end{minipage} & \begin{minipage}[t]{0.62\columnwidth}\raggedright
Modelado de la comunicación entre componentes.\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}
...
\begin{longtable}[]{@{}ll@{}}
\caption{\label{tab:arq}Componentes del CPU x86}\tabularnewline
\toprule
...
- von Neumann, con memoria compartida para datos e instrucciones.\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.10\columnwidth}\raggedright
Registros\strut
\end{minipage} & \begin{minipage}[t]{0.84\columnwidth}\raggedright
- 4 registros de propósito general (\texttt{AL}, \texttt{BL}, \texttt{DL} y \texttt{DL}) de 8 bits cada uno.\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.10\columnwidth}\raggedright
\strut
\end{minipage} & \begin{minipage}[t]{0.84\columnwidth}\raggedright
- 3 registros de propósito específico: \texttt{IP} (Instruction Pointer), \texttt{IR}(Instruction Register) y \texttt{SP}(Stack Pointer).\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.10\columnwidth}\raggedright
\strut
\end{minipage} & \begin{minipage}[t]{0.84\columnwidth}\raggedright
- 1 registro de estado (\texttt{RS}) para las banderas.\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.10\columnwidth}\raggedright
\strut
\end{minipage} & \begin{minipage}[t]{0.84\columnwidth}\raggedright
- 2 registros intermedios (\texttt{MAR}: buffer de direcciones y \texttt{MBR}: buffer de datos) para comunicación entre CPU y memoria.\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.10\columnwidth}\raggedright
Memoria\strut
\end{minipage} & \begin{minipage}[t]{0.84\columnwidth}\raggedright
- Memoria de 256 posiciones, con una capacidad de un byte por posición.\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.10\columnwidth}\raggedright
Buses\strut
\end{minipage} & \begin{minipage}[t]{0.84\columnwidth}\raggedright
...
Instrucciones\strut
\end{minipage} & \begin{minipage}[t]{0.84\columnwidth}\raggedright
- Longitud variable (1, 2 y 3 bytes).\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.10\columnwidth}\raggedright
\strut
\end{minipage} & \begin{minipage}[t]{0.84\columnwidth}\raggedright
- Soporta un conjunto de 16 instrucciones.\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}
...
\section{Selección del repertorio inicial}\label{selecciuxf3n-del-repertorio-inicial}}
...
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.22\columnwidth}\raggedright
Transferencia y procesamiento\strut
\end{minipage} & \begin{minipage}[t]{0.18\columnwidth}\raggedright
B: Cargar a registro\strut
\end{minipage} & \begin{minipage}[t]{0.14\columnwidth}\raggedright
---- XX00 DDDDDDDD\strut
\end{minipage} & \begin{minipage}[t]{0.35\columnwidth}\raggedright
XX = Registro destino, M = Dirección de memoria\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.22\columnwidth}\raggedright
\strut
\end{minipage} & \begin{minipage}[t]{0.18\columnwidth}\raggedright
...
---- 00YY DDDDDDDD\strut
\end{minipage} & \begin{minipage}[t]{0.35\columnwidth}\raggedright
YY = Registro fuente, M = Dirección de memoria\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.22\columnwidth}\raggedright
Control\strut
\end{minipage} & \begin{minipage}[t]{0.18\columnwidth}\raggedright
...
---- ffff DDDDDDDD\strut
\end{minipage} & \begin{minipage}[t]{0.35\columnwidth}\raggedright
ffff = funcionalidad, M = Dirección de memoria\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}
...
\midrule
\endhead
\texttt{Rx},\texttt{Ry} & Registro a registro: operandos son registros del procesador.\tabularnewline
\texttt{{[}M{]}} & Directo: operando es el contenido de una dirección de memoria.\tabularnewline
\bottomrule
\end{longtable}
...
1 & MOV Rx, {[}M{]} & Rx ← Mem{[}Dirección{]} & 0001 00YY MMMMMMMM\tabularnewline
2 & MOV {[}M{]}, Ry & Mem{[}Dirección{]} ← Rx & 0010 XX00 MMMMMMMM\tabularnewline
3 & ADD Rx, Ry & Rx ← Rx + Ry & 0011 XXYY\tabularnewline
4 & ADD Rx, {[}M{]} & Rx ← Rx + Mem{[}Dirección{]} & 0100 00YY MMMMMMMM\tabularnewline
5 & ADD {[}M{]}, Ry & Mem{[}Dirección{]} ← Mem{[}Dirección{]} + Ry & 0101 XX00 MMMMMMMM\tabularnewline
6 & SUB Rx, Ry & Rx ← Rx - Ry & 0110 XXYY\tabularnewline
7 & SUB Rx, {[}M{]} & Rx ← Rx - Mem{[}Dirección{]} & 0111 00YY MMMMMMMM\tabularnewline
8 & SUB {[}M{]}, Ry & Mem{[}Dirección{]} ← Mem{[}Dirección{]} - Ry & 1000 XX00 MMMMMMMM\tabularnewline
9 & CMP Rx, Ry & Modifica flags de Rx - Ry & 1001 XXYY\tabularnewline
A & CMP Rx, {[}M{]} & Modifica flags de Rx - Mem{[}Dirección{]} & 1010 00YY MMMMMMMM\tabularnewline
B & CMP {[}M{]}, Ry & Modifica flags de Mem{[}Dirección{]} - Ry & 1011 XX00 MMMMMMMM\tabularnewline
C & JMP M & IP ← Dirección & 1100 0000 MMMMMMMM\tabularnewline
C & JC M & Si flag C=1 entonces IP ← Dirección & 1100 0001 MMMMMMMM\tabularnewline
C & JZ M & Si flag Z=1 entonces IP ← Dirección & 1100 0011 MMMMMMMM\tabularnewline
\bottomrule
\end{longtable}
...
\item
  ...
  La UC activa la señal de lectura (read) hacia la memoria, utilizando el valor del \texttt{MAR} como dirección. El dato leído se transfiere al rRegistro de datos de memoria \texttt{MBR} a través del bus de datos. Simultáneamente, el \texttt{IP} se incrementa en 1 para apuntar al siguiente byte.
\item
...
\tightlist
\item
  MOV \texttt{Rx}, \texttt{Ry} (Copiar entre registros)
  ...
  \end{enumerate}
\item
  MOV \texttt{Rx}, \texttt{{[}Dirección{]}} (Cargar a registro)
...
\item
  ...
  1, 2, 3. Igual que MOV \texttt{Rx}, \texttt{{[}Dirección{]}}.
...
\item
  ...
  1, 2, 3, 4. Igual que MOV \texttt{Rx}, \texttt{{[}Dirección{]}}.
...
\item
  ...
  1, 2. Igual que MOV \texttt{Rx}, \texttt{{[}Dirección{]}}.
...
\midrule
\endhead
\texttt{{[}BL{]}} & Registro a registro: operandos son registros del procesador.\tabularnewline
\texttt{m} & Inmediato: operando es un valor contenido en la instrucción.\tabularnewline
\texttt{{[}BL\ +\ m{]}} & Directo: operando es el contenido de una dirección de memoria.\tabularnewline
\bottomrule
\end{longtable}
...
B: Cargar a registro\strut
\end{minipage} & \begin{minipage}[t]{0.30\columnwidth}\raggedright
\texttt{-\/-\/-\/-\ XXff\ MMMMMMMM}\strut
\end{minipage} & \begin{minipage}[t]{0.30\columnwidth}\raggedright
\texttt{ff} = funcionalidad, \texttt{XX} = Registro destino, \texttt{M} = Dirección de memoria o Dato Inmediato\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.30\columnwidth}\raggedright
C: Almacenar en memoria\strut
\end{minipage} & \begin{minipage}[t]{0.30\columnwidth}\raggedright
\texttt{-\/-\/-\/-\ ffff\ MMMMMMMM\ dddddddd}\strut
\end{minipage} & \begin{minipage}[t]{0.30\columnwidth}\raggedright
\texttt{ffff} = funcionalidad, \texttt{M} = Dirección de memoria, \texttt{d} = Dato Inmediato\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.30\columnwidth}\raggedright
D: control de flujo\strut
\end{minipage} & \begin{minipage}[t]{0.30\columnwidth}\raggedright
\texttt{-\/-\/-\/-\ ffff\ MMMMMMMM}\strut
\end{minipage} & \begin{minipage}[t]{0.30\columnwidth}\raggedright
\texttt{ffff} = funcionalidad \texttt{M} = Dirección de memoria\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}
...
Modo direccionamiento B: Cargar a registro\strut
...
\textbf{\texttt{ff}= funcionalidad}\strut
...
2\strut
\end{minipage} & \begin{minipage}[t]{0.30\columnwidth}\centering
directo \texttt{M} = Dirección de memoria\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.30\columnwidth}\centering
01\strut
\end{minipage} & \begin{minipage}[t]{0.30\columnwidth}\centering
1\strut
\end{minipage} & \begin{minipage}[t]{0.30\columnwidth}\centering
indirecto utiliza como operando implicito el registro \texttt{BL} y no requiere operando \texttt{M}\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.30\columnwidth}\centering
10\strut
\end{minipage} & \begin{minipage}[t]{0.30\columnwidth}\centering
...
\endhead
\begin{minipage}[t]{0.30\columnwidth}\centering
\textbf{\texttt{ffff}= funcionalidad}\strut
\end{minipage} & \begin{minipage}[t]{0.30\columnwidth}\centering
\textbf{\texttt{Byte}= tamaño}\strut
\end{minipage} & \begin{minipage}[t]{0.30\columnwidth}\centering
\textbf{Interpretación}\strut
...
1 & \texttt{MOV\ Rx,\ {[}M{]}} & \texttt{Rx} \(\leftarrow\) \texttt{Mem{[}Dirección{]}} & \texttt{0001\ XX00\ MMMMMMMM}\tabularnewline
1 & \texttt{MOV\ Rx,\ {[}BL{]}} & \texttt{Rx} \(\leftarrow\) \texttt{Mem{[}BL{]}} & \texttt{0001\ XX01}\tabularnewline
1 & \texttt{MOV\ Rx,\ D} & \texttt{Rx} \(\leftarrow\) \texttt{Dato} & \texttt{0001\ XX10\ MMMMMMMM}\tabularnewline
1 & \texttt{MOV\ Rx,\ {[}BL\ +\ D{]}} & \texttt{Rx} \(\leftarrow\) \texttt{Mem{[}BL\ +\ Dato{]}} & \texttt{0001\ XX11\ MMMMMMMM}\tabularnewline
2 & \texttt{MOV\ {[}M{]},\ Ry} & \texttt{Mem{[}Dirección{]}} \(\leftarrow\) \texttt{Rx} & \texttt{0010\ 00YY\ MMMMMMMM}\tabularnewline
2 & \texttt{MOV\ {[}BL{]},\ Ry} & \texttt{Mem{[}BL{]}} \(\leftarrow\) \texttt{Rx} & \texttt{0010\ 01YY}\tabularnewline
2 & \texttt{MOV\ {[}BL\ +\ D{]},\ Ry} & \texttt{Mem{[}BL\ +\ Dato{]}} \(\leftarrow\) \texttt{Rx} & \texttt{0010\ 10YY\ MMMMMMMM}\tabularnewline
2 & \texttt{MOV\ {[}M{]},\ D} & \texttt{Mem{[}Dirección{]}} \(\leftarrow\) \texttt{Dato} & \texttt{0010\ 1100\ MMMMMMMM\ dddddddd}\tabularnewline
2 & \texttt{MOV\ {[}BL{]},\ D} & \texttt{Mem{[}BL{]}} \(\leftarrow\) \texttt{Dato} & \texttt{0010\ 1101\ MMMMMMMM}\tabularnewline
2 & \texttt{MOV\ {[}BL\ +\ D{]},\ D} & \texttt{Mem{[}BL\ +\ Dato{]}} \(\leftarrow\) \texttt{Dato} & \texttt{0010\ 1110\ MMMMMMMM}\tabularnewline
...
C & \texttt{JMP\ M} & \texttt{IP} \(\leftarrow\) \texttt{Dirección} & \texttt{1100\ 0000\ MMMMMMMM}\tabularnewline
C & \texttt{JC\ M} & Si \texttt{flag\ C=1} entonces \texttt{IP} \(\leftarrow\) \texttt{Dirección} & \texttt{1100\ 0001\ MMMMMMMM}\tabularnewline
C & \texttt{JZ\ M} & Si \texttt{flag\ Z=1} entonces \texttt{IP} \(\leftarrow\) \texttt{Dirección} & \texttt{1100\ 0011\ MMMMMMMM}\tabularnewline
...
\hypertarget{alineaciuxf3n-con-los-objetivos-pedaguxf3gicos}{%
\subsection{Alineación con los objetivos pedagógicos}\label{alineaciuxf3n-con-los-objetivos-pedaguxf3gicos}}
