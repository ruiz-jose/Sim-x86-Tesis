...
Esta etapa, común a todas las instrucciones, consiste en la lectura de la instrucción desde la memoria. El proceso se desarrolla en tres ciclos:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{\texttt{MAR} \(\leftarrow\) \texttt{IP}}\\
  La Unidad de Control (UC) transfiere el contenido del registro puntero de instrucciones (\texttt{IP}) al registro de direcciones de memoria (\texttt{MAR}). Este paso prepara el sistema para acceder a la posición de memoria donde se encuentra la próxima instrucción.
\item
...
\item
  ...
  Finalmente, el contenido del registro de datos de memoria (\texttt{MBR}) se copia en el registro de instrucciones (\texttt{IR}). Con este paso, la instrucción ha sido captada y está lista para ser decodificada y ejecutada.
\end{enumerate}
...
\item
  ...
  \item
    ADD/SUB/CMP: \textbf{\texttt{id} \(\leftarrow\) \texttt{MBR}; \texttt{MAR} \(\leftarrow\) \texttt{ri}}
  \end{enumerate}
