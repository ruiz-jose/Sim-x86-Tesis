...
VonSim \protect\hyperlink{ref-vonsim}{{[}73{]}} es una herramienta diseñada específicamente para la enseñanza y el aprendizaje de la arquitectura y organización de computadoras. Sus principales características la posicionan como una solución educativa integral:
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Entorno integrado de desarrollo y simulación:} Proporciona un entorno completo que incluye editor de código ensamblador con resaltado de sintaxis y un simulador para la ejecución de programas, facilitando el proceso de aprendizaje práctico \protect\hyperlink{ref-vonsim}{{[}73{]}}.
\item
  \textbf{Fundamento en arquitectura real:} Se basa en el procesador Intel 8088, proporcionando una referencia histórica y técnicamente relevante para el estudio de la evolución de las arquitecturas de computadoras \protect\hyperlink{ref-intel8086manual}{{[}45{]}}.
\item
  \textbf{Componentes esenciales para el estudio:} Incluye cuatro registros multipropósito de 16 bits, memoria principal de 32 kB, bus de direcciones de 16 bits y bus de datos de 8 bits, abarcando los elementos fundamentales para la comprensión de la arquitectura von Neumann \protect\hyperlink{ref-stallings_computer_2021}{{[}14{]}}.
\item
  \textbf{Gestión completa de interrupciones:} Implementa tanto interrupciones por software (entrada/salida de datos) como interrupciones por hardware mediante un controlador de interrupciones programable (PIC), cubriendo aspectos fundamentales de la operación del sistema \protect\hyperlink{ref-hennessy2017computer}{{[}13{]}}.
\item
...
\item
  \textbf{Accesibilidad y sostenibilidad:} Distribuido bajo licencia GNU Affero General Public License v3.0 con código fuente disponible en GitHub, y documentación bajo licencia CC BY-SA 4.0, facilitando su estudio, modificación y mejora continua \protect\hyperlink{ref-opensource_licensing_2024}{{[}74{]}}.
\end{enumerate}
...
Resaldo de la direccion de memoria apuntadas por los registros IP y SP.
...
\caption{Memoria principal}\label{fig:resaltadoipintsp}
\end{figure}

Se resalta el vector de interrucciones.
Se resalta el programa en memoria pudiendo ver la instruccion y su tamaño en bytes que ocupa en memoria, ademas la etiqueta asociada a los datos.
No es obligatorio la directiva org para indicar la dirección de inicio del programa, ya que el simulador asigna automáticamente la dirección 0x00 al primer byte del programa. Esto simplifica la escritura de programas y evita errores comunes relacionados con la asignación manual de direcciones.
Pero si el programa requiere una dirección de inicio específica, se puede utilizar la directiva \texttt{org} para establecerla. Por ejemplo, \texttt{org\ 0x20} indicaría que el programa debe comenzar en la dirección 0x20 de la memoria. Por compatibilidad con el simulador VonSim, se mantiene la directiva \texttt{org} para aquellos usuarios que deseen especificar una dirección de inicio diferente a la predeterminada. El simulador comienza a ejecutar el programa desde la dirección 0x00, a menos que se indique lo contrario con \texttt{org}.
En caso de tener interrupciones y no indicarse la directiva \texttt{org}, el simulador asigna automáticamente la dirección 0x20 al primer byte del programa, porque las interrupciones se gestionan a partir de la dirección \texttt{0x00}. Esto simplifica la escritura de programas y evita errores comunes relacionados con la asignación manual de direcciones.
...
Esta sección describe la estructura del simulador VonSims8, el diseño de los registros fue concebido para facilitar la comprensión de los modos de direccionamiento y del ciclo de instrucción, ambos considerados fundamentos clave en el estudio de la Arquitectura de Computadoras \protect\hyperlink{ref-stallings_computer_2021}{{[}14{]}}. Se describe en la siguiente tabla \ref{tab:estructuravonsim8} los componentes principales del simulador, junto con sus características y funcionalidades específicas. Esta tabla proporciona una visión general de la arquitectura del simulador, destacando los elementos clave que componen su estructura y su propósito pedagógico.:
