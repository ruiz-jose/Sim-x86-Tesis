...
En la instrucción de salto condicional \texttt{Jxx\ Dirección}, la indicación \texttt{xx} representa todas las combinaciones posibles de los \emph{flags} donde \texttt{NZ} indica la negacion del \emph{flags} \texttt{Z}:

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 2\tabcolsep) * \real{0.3676}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 2\tabcolsep) * \real{0.6324}}@{}}
\toprule\noalign{}
...
La etapa de captación tiene 3 ciclos para todas las instrucciones. En el primero la Unidad de Control (UC) transfiere el contenido del registro puntero de instrucciones (\texttt{IP}) al registro de direcciones de memoria (\texttt{MAR}). Este paso prepara el sistema para acceder a la posición de memoria donde se encuentra la próxima instrucción. En el segundo paso la UC activa la señal de lectura para obtener el contenido de la memoria en la dirección especificada por el (\texttt{MAR}). El dato leído se transfiere al registro de datos de memoria (\texttt{MBR}) mediante el bus de datos. Simultáneamente, el registro (\texttt{IP}) se incrementa en uno, apuntando así a la siguiente instrucción en memoria u operando de la instrucción vigente. Finalmente, el tercer paso el contenido del registro de datos de memoria (\texttt{MBR}) se copia en el registro de instrucciones (\texttt{IR}). Con este paso, la instrucción ha sido captada y está lista para ser decodificada y ejecutada.
...
\hypertarget{destino-registro}{%
\subsubsection{Destino registro}\label{destino-registro}}

\begin{itemize}
\tightlist
\item
  Entre registros \texttt{Rx}, \texttt{Ry}
  ...
  \item
    CMP: \textbf{\texttt{Rx} - \texttt{Ry}; update(Flags)}
  \end{enumerate}
\item
  Directo \texttt{Rx}, \texttt{{[}Dirección{]}}
\end{itemize}
...
\begin{itemize}
\item
  Directo \texttt{{[}Dirección{]}}, \texttt{Ry}
  ...
  \tightlist
  \item
    MOV: \textbf{\texttt{MBR} \(\leftarrow\) \texttt{Ry}}
  \item
  ...
  \tightlist
  \item
    ADD: \textbf{\texttt{MBR} \(\leftarrow\) \texttt{MBR} + \texttt{Ry}; update(Flags)}
  \item
    SUB: \textbf{\texttt{MBR} \(\leftarrow\) \texttt{MBR} - \texttt{Ry}; update(Flags)}
  \item
  ...
  \tightlist
  \item
    ADD/SUB: \textbf{\texttt{write(Memoria{[}MAR{]})} \(\leftarrow\) \texttt{MBR}}
  \end{enumerate}
\item
  Indirecto \texttt{{[}BL{]}}, \texttt{Ry}
  ...
  \item
    \textbf{\texttt{MAR} \(\leftarrow\) \texttt{BL}}
    En el paso 4 se establece la direccion de destino en memoria en base a la direccion contenida en el registro BL
  \end{enumerate}
  ...
  \item
    MOV: \textbf{\texttt{write(Memoria{[}MAR{]})} \(\leftarrow\) \texttt{MBR}}
    En el paso 5 se almacena en registro MBR el operando fuente y en el paso 6 se escribe en memoria.
  \end{enumerate}
  ...
  \item
    ADD/SUB/CMP: \textbf{\texttt{MBR} \(\leftarrow\) \texttt{read(Memoria{[}MAR{]})}}
  \item
    ADD: \textbf{\texttt{write(Memoria{[}MAR{]})} \(\leftarrow\) \texttt{MBR} + \texttt{Ry}; update(Flags)}
  \item
    SUB \texttt{{[}BL{]}}, \texttt{Ry}: \textbf{\texttt{write(Memoria{[}MAR{]})} \(\leftarrow\) \texttt{MBR} - \texttt{Ry}; update(Flags)}
  \item
    CMP \texttt{{[}BL{]}}, \texttt{Ry}: \textbf{\texttt{MBR} - \texttt{Ry}; update(Flags)}
  \end{enumerate}
...
\item
  ...
  \item
    \textbf{\texttt{MBR} \(\leftarrow\) \texttt{read(Memoria{[}MAR{]})}; \texttt{IP} \(\leftarrow\) \texttt{IP} + 1}
  \item
    \textbf{\texttt{MAR} \(\leftarrow\) \texttt{ri}}
  \end{enumerate}
  ...
  \tightlist
  \item
    ADD/SUB: \textbf{\texttt{write(Memoria{[}MAR{]})} \(\leftarrow\) \texttt{MBR}; update(Flags)}
  \end{enumerate}
\item
  ...
  \setcounter{enumi}{3}
  \item
    \textbf{\texttt{MAR} \(\leftarrow\) \texttt{BL}}: El contenido de BL se transfiere al registro MAR.
  \item
    MOV \texttt{{[}BL{]}}, \texttt{Dato}: \textbf{\texttt{write(Memoria{[}MAR{]})} \(\leftarrow\) \texttt{MBR}}: Se escribe el valor de MBR en la posición de memoria indicada por MAR.
  \item
    ADD \texttt{{[}BL{]}}, \texttt{Dato}: \textbf{\texttt{write(Memoria{[}MAR{]})} \(\leftarrow\) \texttt{MBR}; update(Flags)}
  \end{enumerate}
\end{itemize}
...
4. \textbf{\texttt{MAR} \(\leftarrow\) \texttt{IP}}
5. \textbf{\texttt{MBR} \(\leftarrow\) \texttt{read(Memoria{[}MAR{]})}; \texttt{IP} \(\leftarrow\) \texttt{IP} + 1}
Los pasos 4. 5. se obtiene la direccion del operando de la instrucción.
...
\tightlist
\item
  JMP: \textbf{\texttt{IP} \(\leftarrow\) \texttt{MBR}}: El contenido del \texttt{MBR} se transfiere al registro \texttt{IP}, estableciendo la nueva dirección de ejecución.
\end{enumerate}
...
\item
  ...
  \tightlist
  \item
    \texttt{Si\ Z} == 1: \textbf{\texttt{IP} \(\leftarrow\) \texttt{MBR}}: Si la bandera de cero (\texttt{Z}) está activa, el contenido del \texttt{MBR} se transfiere al registro \texttt{IP}.
  \end{enumerate}
\item
  ...
  \tightlist
  \item
    \texttt{Si\ C} == 1: \textbf{\texttt{IP} \(\leftarrow\) \texttt{MBR}}: Si la bandera de acarreo (\texttt{C}) está activa, el contenido del \texttt{MBR} se transfiere al registro \texttt{IP}.
  \end{enumerate}
\item
  Jxx \texttt{Dirección} (Salto condicional)
  ...
  \tightlist
  \item
    \texttt{Si\ xx} == 1: \textbf{\texttt{IP} \(\leftarrow\) \texttt{MBR}}: Si la condición especificada por \texttt{xx} se cumple, el contenido del \texttt{MBR} se transfiere al registro \texttt{IP}.
  \end{enumerate}
\end{itemize}
