...
Registros & 4 registros de propósito general (`AL`, `BL`, `DL` y `DL`) de 8 bits cada uno.\\
 & 4 registros de propósito específico: `IP` (Instruction Pointer),`IR`(Instruction Register), `SP`(Stack Pointer) y `RS` (registro de estado)\\
...
Instrucciones & Longitud variable de instrucciones (1, 2 y 3 bytes).\\
 & Modos de direccionamiento: registro a registro, directo, indirecto e inmediato.\\
\bottomrule
\end{tabular}}
\end{table}
