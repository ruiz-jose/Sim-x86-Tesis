...
El repertorio de instrucciones, conocido como ISA (Instruction Set Architecture), se define el conjunto de operaciones que la CPU puede ejecutar y cómo se codifican esas operaciones. Incluye instrucciones para operaciones aritméticas, lógicas, de control y de manipulación de datos, así como modos de direccionamiento y formatos de instrucciones. El diseño del repertorio de instrucciones tiene un impacto significativo en el rendimiento, la eficiencia y la flexibilidad de una arquitectura de computadoras {[}\protect\hyperlink{ref-stallings_computer_2013}{14}{]}.

Algunas de las características esenciales que se deben considerar al diseñar un repertorio de instrucciones incluyen:

\begin{itemize}
\item
  \textbf{Tipos de Operandos}: Este aspecto se refiere a los diferentes tipos de datos que las instrucciones pueden manipular, como enteros, números de punto flotante, caracteres o direcciones de memoria.
\item
  \textbf{Tipos de Operaciones}: Incluye las operaciones que la CPU puede realizar, como aritmética (suma, resta), lógica (AND, OR), control (saltos, llamadas a subrutinas) y operaciones de manipulación de datos (movimiento de datos, almacenamiento, carga).
\item
  \textbf{Formato de las Instrucciones}: Es la estructura de las instrucciones, que incluye aspectos como la longitud de la instrucción, el número de operandos y el modo de direccionamiento. El formato puede variar entre diferentes arquitecturas y tiene un impacto significativo en la eficiencia del procesador.
\item
  \textbf{Modos de Direccionamiento}: Definen cómo se especifican los operandos en las instrucciones. Algunos modos comunes incluyen direccionamiento inmediato, directo, indirecto, y relativo. Cada modo ofrece diferentes ventajas en términos de flexibilidad y eficiencia.
\end{itemize}

\hypertarget{funciones-del-repertorio-de-instrucciones}{%
\subsubsection{Funciones del Repertorio de Instrucciones}\label{funciones-del-repertorio-de-instrucciones}}
...
\textbf{Control de la CPU}: A través de las instrucciones de control, se gestionan aspectos como la secuencia de ejecución de instrucciones, control de interrupciones, y la ejecución de llamadas a subrutinas.

\textbf{Interacción con la Memoria}: Las instrucciones permiten que la CPU interactúe con la memoria, realizando operaciones de carga y almacenamiento. Estas interacciones son esenciales para la ejecución de programas, ya que permiten mover datos entre la CPU y la memoria principal.

\textbf{Gestión de Datos}: El repertorio incluye instrucciones para la manipulación de datos, como operaciones aritméticas, lógicas y de comparación. Estas operaciones son fundamentales para la mayoría de los cálculos y procesos dentro de una CPU.

\textbf{Optimización del Rendimiento}: El diseño del repertorio de instrucciones puede influir directamente en la eficiencia y velocidad del procesador. Un repertorio bien diseñado puede minimizar el número de instrucciones necesarias para ejecutar una tarea, reducir el consumo de energía y optimizar el uso de los recursos del sistema.

\hypertarget{filosofuxedas-cisc-y-risc}{%
\subsubsection{Filosofías CISC y RISC}\label{filosofuxedas-cisc-y-risc}}

La arquitectura x86 sigue la filosofía CISC (Complex Instruction Set Computer), caracterizada por un repertorio de instrucciones amplio y complejo. Este enfoque permite ejecutar tareas complejas mediante una única instrucción, reduciendo así la cantidad total de instrucciones en un programa. Sin embargo, esta ventaja se equilibra con una mayor complejidad en la decodificación y ejecución de cada instrucción. En contraste, la filosofía RISC (Reduced Instruction Set Computer) adopta un enfoque diferente, donde el repertorio de instrucciones es más simple y las operaciones se realizan en pasos más pequeños, lo que aumenta el número de instrucciones necesarias pero facilita el diseño y optimización del hardware. Por ejemplo, una operación como el cálculo de un desplazamiento de memoria, que en CISC podría resolverse con una sola instrucción, en RISC requeriría una secuencia de instrucciones más básicas pero más rápidas {[}\protect\hyperlink{ref-stallings_computer_2013}{14}{]}.

\hypertarget{modos-de-direccionamiento}{%
\subsubsection{Modos de Direccionamiento}\label{modos-de-direccionamiento}}
...
\caption{Formato instrucción x86}\label{fig:flujo}
\end{figure}

Estos modos se ilustran en la \ldots{} En esta sección utilizaremos la siguiente notación:
...
\def\labelenumi{\alph{enumi})}
\item
  \textbf{Inmediato}: En este modo, el operando está incrustado directamente en la instrucción. Es útil para valores constantes pequeños y proporciona una ejecución rápida ya que no requiere acceso a la memoria.
\item
  \textbf{Directo}: La instrucción contiene la dirección de memoria del operando. Es sencillo y fácil de entender, pero limita el rango de direcciones que se pueden especificar directamente.
\item
  \textbf{Indirecto}: La dirección de la memoria contenida en la instrucción apunta a otra dirección de memoria donde se encuentra el operando real. Esto permite acceder a un rango más amplio de direcciones de memoria, pero introduce un nivel adicional de indirection y, por lo tanto, puede ser más lento.
\item
  \textbf{Registro}: Similar al direccionamiento directo, pero en lugar de una dirección de memoria, la instrucción especifica un registro que contiene el operando. Este modo es rápido y eficiente porque los accesos a registros son más rápidos que los accesos a memoria.
\item
  \textbf{Registro Indirecto}: Similar al direccionamiento indirecto, pero la dirección del operando se encuentra en un registro en lugar de en la memoria. Proporciona flexibilidad y rapidez.
\item
  \textbf{Con desplazamiento}: Combina una dirección base contenida en la instrucción o en un registro con un valor de desplazamiento (offset), permitiendo acceder a elementos de estructuras de datos como arrays.
\item
  \textbf{Pila}: La dirección del operando se calcula como un desplazamiento relativo a la ubicación actual de la instrucción en ejecución. Este modo es útil para implementar saltos en el flujo de control del programa.
\end{enumerate}

\hypertarget{formato-de-las-instrucciones}{%
\subsubsection{Formato de las Instrucciones}\label{formato-de-las-instrucciones}}
