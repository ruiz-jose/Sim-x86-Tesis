...
La Unidad de Control (UC) transfiere el contenido del registro puntero de instrucciones (\texttt{IP}) al registro de direcciones de memoria (\texttt{MAR}). Este paso prepara el sistema para acceder a la posición de memoria donde se encuentra la próxima instrucción. La UC activa la señal de lectura para obtener el contenido de la memoria en la dirección especificada por el (\texttt{MAR}). El dato leído se transfiere al registro de datos de memoria (\texttt{MBR}) mediante el bus de datos. Simultáneamente, el registro (\texttt{IP}) se incrementa en uno, apuntando así a la siguiente instrucción en memoria u operando de la instrucción vigente. Finalmente, el contenido del registro de datos de memoria (\texttt{MBR}) se copia en el registro de instrucciones (\texttt{IR}). Con este paso, la instrucción ha sido captada y está lista para ser decodificada y ejecutada.
...
Todas las instrucciones de dos operandos tienen los mismos pasos de ejecución, diferenciándose en el destino de la operación y el modo de direccionamiento.
...
\item
  ...
  \tightlist
  \item
    MOV \texttt{Rx}, \texttt{Ry}: \textbf{\texttt{Rx} \(\leftarrow\) \texttt{Ry}}
  \item
    ADD \texttt{Rx}, \texttt{Ry}: \textbf{\texttt{Rx} \(\leftarrow\) \texttt{Rx} + \texttt{Ry}; update(Flags)}
  \item
    SUB \texttt{Rx}, \texttt{Ry}: \textbf{\texttt{Rx} \(\leftarrow\) \texttt{Rx} - \texttt{Ry}; update(Flags)}
  \item
    CMP \texttt{Rx}, \texttt{Ry}: \textbf{\texttt{Rx} - \texttt{Ry}; update(Flags)}
  \end{enumerate}
\item
...
Los pasos 4. 5. se obtiene el operando de la instrucción. El contenido de IP se transfiere al registro MAR. Se lee el dato de la memoria en la dirección indicada por MAR y se almacena en MBR; luego, IP se incrementa en uno. El valor de MBR se transfiere a MAR. Se lee el dato de la memoria en la nueva dirección de MAR y se almacena en MBR.
8. MOV \texttt{Rx}, \texttt{{[}Dirección{]}}: \textbf{\texttt{Rx} \(\leftarrow\) \texttt{MBR}}
8. ADD \texttt{Rx}, \texttt{{[}Dirección{]}}: \textbf{\texttt{Rx} \(\leftarrow\) \texttt{Rx} + \texttt{MBR}; update(Flags)}
8. SUB \texttt{Rx}, \texttt{{[}Dirección{]}}: \textbf{\texttt{Rx} \(\leftarrow\) \texttt{Rx} - \texttt{MBR}; update(Flags)}
8. CMP \texttt{Rx}, \texttt{{[}Dirección{]}}: \textbf{\texttt{Rx} - \texttt{MBR}; update(Flags)}

\begin{itemize}
\tightlist
\item
  MOV \texttt{Rx}, \texttt{Dato} (Inmediato)
  ...
  \item
    ...
    \end{enumerate}
  \item
    \textbf{\texttt{Rx} \(\leftarrow\) \texttt{MBR}}: El contenido de MBR se transfiere al registro Rx.
  \end{enumerate}
\item
  MOV \texttt{Rx}, \texttt{{[}BL{]}} (Indirecto)
  ...
  \item
    \textbf{\texttt{MBR} \(\leftarrow\) \texttt{read(Memoria{[}MAR{]})}}: Se lee el dato de la memoria en la dirección indicada por MAR y se almacena en MBR.
  \item
    \textbf{\texttt{Rx} \(\leftarrow\) \texttt{MBR}}: Finalmente, el contenido de MBR se copia en el registro Rx.
  \end{enumerate}

  \hypertarget{destino-memoria}{%
  \subsubsection{Destino Memoria}\label{destino-memoria}}
\item
  MOV \texttt{{[}Dirección{]}}, \texttt{Ry} (Directo)
  ...
  \item
    ...
    \end{enumerate}
  \item
    \textbf{\texttt{MAR} \(\leftarrow\) \texttt{MBR}}: El valor de MBR se transfiere al registro MAR.
  \item
    \textbf{\texttt{MBR} \(\leftarrow\) \texttt{Ry}}: El contenido de Ry se copia en MBR.
  \item
    \textbf{\texttt{write(Memoria{[}MAR{]})} \(\leftarrow\) \texttt{MBR}}: Se escribe el valor de MBR en la posición de memoria indicada por MAR.
  \end{enumerate}
\item
  MOV \texttt{{[}BL{]}}, \texttt{Ry} (Indirecto)
  ...
  \end{enumerate}
\item
  MOV \texttt{{[}Dirección{]}}, \texttt{Dato} (Inmediato)
  ...
  \item
    ...
    \end{enumerate}
  \item
    \textbf{\texttt{MAR} \(\leftarrow\) \texttt{IP};\texttt{ri} \(\leftarrow\) \texttt{MBR}}: El contenido de IP se transfiere al registro MAR y simultaneamente se transfiere el valor de MBR a un registro intermedio (ri) .\\
  \item
    \textbf{\texttt{MBR} \(\leftarrow\) \texttt{read(Memoria{[}MAR{]})}; \texttt{IP} \(\leftarrow\) \texttt{IP} + 1}: Se lee el dato de la memoria en la dirección indicada por MAR y se almacena en MBR; luego, IP se incrementa en uno.\\
  \item
    \textbf{\texttt{MAR} \(\leftarrow\) \texttt{ri}}: El contenido de ri se transfiere al registro MAR.
  \item
    \textbf{\texttt{write(Memoria{[}MAR{]})} \(\leftarrow\) \texttt{MBR}}: Se escribe el valor de MBR en la posición de memoria indicada por MAR.
  \end{enumerate}
\item
  MOV \texttt{{[}BL{]}}, \texttt{Dato} (Inmediato)
...
Las instrucciones que utilizan la Unidad Aritmético-Lógica (ALU) actualizan las banderas del registro de estado \texttt{RS}.
Tienen los mismos modos de direccionamiento que MOV, pero con una diferencia importante: la instrucción ADD almacena el resultado de la operación en el operando destino. Es decir que solo cambia el ultimo paso de la etapa de ejecución.
...
\hypertarget{destino-registro-1}{%
\subsubsection{Destino registro}\label{destino-registro-1}}

\begin{itemize}
\tightlist
\item
\item
  ADD \texttt{Rx}, \texttt{{[}Dirección{]}} (Directo)
  ...
  \tightlist
  \item
    \textbf{\texttt{Rx} \(\leftarrow\) \texttt{Rx} + \texttt{MBR}; update(Flags)}
  \end{enumerate}
\item
  ADD \texttt{Rx}, \texttt{Dato} (Inmediato)
  ...
  \end{enumerate}
\item
  ADD \texttt{Rx}, \texttt{{[}BL{]}} (Indirecto)
...
\hypertarget{destino-memoria-1}{%
\subsubsection{Destino Memoria}\label{destino-memoria-1}}

\begin{itemize}
\tightlist
\item
  ADD \texttt{{[}Dirección{]}}, \texttt{Ry} (Directo)
  ...
  \tightlist
  \item
    \textbf{\texttt{write(Memoria{[}MAR{]})} \(\leftarrow\) \texttt{MBR}; update(Flags)}
  \end{enumerate}
\item
  ADD \texttt{{[}BL{]}}, \texttt{Ry} (Indirecto)
  ...
  \end{enumerate}
\item
  ADD \texttt{{[}Dirección{]}}, \texttt{Dato} (Inmediato)
  ...
  \end{enumerate}
\item
  ADD \texttt{{[}BL{]}}, \texttt{Dato} (Inmediato)
...
\hypertarget{destino-registro-2}{%
\subsubsection{Destino registro}\label{destino-registro-2}}

\begin{itemize}
\tightlist
\item
  SUB \texttt{Rx}, \texttt{Ry} (Entre registros)
  ...
  \tightlist
  \item
    \textbf{\texttt{Rx} \(\leftarrow\) \texttt{Rx} - \texttt{Ry}; update(Flags)}
  \end{enumerate}
\item
  SUB \texttt{Rx}, \texttt{{[}Dirección{]}} (Directo)
  ...
  \end{enumerate}
\item
  SUB \texttt{Rx}, \texttt{Dato} (Inmediato)
  ...
  \end{enumerate}
\item
  SUB \texttt{Rx}, \texttt{{[}BL{]}} (Indirecto)
...
\hypertarget{destino-memoria-2}{%
\subsubsection{Destino Memoria}\label{destino-memoria-2}}

\begin{itemize}
\tightlist
\item
  SUB \texttt{{[}Dirección{]}}, \texttt{Ry} (Directo)
  ...
  \end{enumerate}
\item
  SUB \texttt{{[}BL{]}}, \texttt{Ry} (Indirecto)
  ...
  \end{enumerate}
\item
  SUB \texttt{{[}Dirección{]}}, \texttt{Dato} (Inmediato)
  ...
  \end{enumerate}
\item
  SUB \texttt{{[}BL{]}}, \texttt{Dato} (Inmediato)
...
\hypertarget{destino-registro-3}{%
\subsubsection{Destino registro}\label{destino-registro-3}}

\begin{itemize}
\tightlist
\item
  CMP \texttt{Rx}, \texttt{Ry} (Entre registros)
  ...
  \tightlist
  \item
    \textbf{\texttt{Rx} - \texttt{Ry}; update(Flags)}
  \end{enumerate}
\item
  CMP \texttt{Rx}, \texttt{{[}Dirección{]}} (Directo)
  ...
  \tightlist
  \item
    \textbf{\texttt{Rx} - \texttt{MBR}; update(Flags)}\\
  \end{enumerate}
\item
  CMP \texttt{Rx}, \texttt{Dato} (Inmediato)
  ...
  \end{enumerate}
\item
  CMP \texttt{Rx}, \texttt{{[}BL{]}} (Indirecto)
  ...
  \end{enumerate}
\item
  CMP \texttt{{[}Dirección{]}}, \texttt{Ry} (Directo)
  ...
  \end{enumerate}
\item
  CMP \texttt{{[}BL{]}}, \texttt{Ry} (Indirecto)
  ...
  \end{enumerate}
\item
  CMP \texttt{{[}Dirección{]}}, \texttt{Dato} (Inmediato)
  ...
  \end{enumerate}
\item
  CMP \texttt{{[}BL{]}}, \texttt{Dato} (Inmediato)
...
\hypertarget{destino-registro-4}{%
\subsubsection{Destino registro}\label{destino-registro-4}}
