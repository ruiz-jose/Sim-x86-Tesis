...
\begin{table}[!h]
\centering
\caption{\label{tab:comparq}Comparación pedagógica de arquitecturas}
\centering
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\fontsize{9}{11}\selectfont
\begin{tabular}[t]{>{\raggedright\arraybackslash}p{3.5cm}|>{\raggedright\arraybackslash}p{3.5cm}>{\raggedright\arraybackslash}p{3.5cm}>{\raggedright\arraybackslash}p{3.5cm}>{\raggedright\arraybackslash}p{6.5cm}}
\toprule
\multicolumn{1}{>{\centering\arraybackslash}p{3.5cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Característica}}} & \multicolumn{1}{>{\centering\arraybackslash}p{3.5cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{x86}}} & \multicolumn{1}{>{\centering\arraybackslash}p{3.5cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{ARM}}} & \multicolumn{1}{>{\centering\arraybackslash}p{3.5cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{RISC‑V}}} & \multicolumn{1}{>{\centering\arraybackslash}p{6.5cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Implicación pedagógica}}}\\
\midrule
\textbf{Eficiencia Energética} & Moderada-alta según microarquitectura & Alta (diseñada para eficiencia) & Alta (diseño RISC abierto) & Permite comparar consumo y trade‑offs; útil para labs de medición de energía.\\
\textbf{Complejidad ISA} & Alta (ISA extensa, longitud variable) & Baja (RISC, instrucciones simples) & Baja (ISA reducida y extensible) & x86 exige más tiempo en decodificación/formatos; enseñar en módulos. RISC facilita introducción al pipeline.\\
\textbf{Rendimiento (típico)} & Alto en desktops/servidores & Moderado/alto en móviles & Moderado, escalable según extensión & Diseñar ejercicios que midan CPI y tiempo real para cada ISA.\\
\textbf{Modos de direccionamiento} & Muy variados (SIB, ModR/M, desplazamientos) & Más simple (load/store) & Simple y extensible (load/store) & Usar ejemplos prácticos de EA para mostrar complejidad en x86 vs simplicidad en RISC.\\
\textbf{Flexibilidad de operandos} & Alta (reg-mem, mem-mem en algunos casos) & Moderada (load/store, registros) & Moderada (diseñable por extensiones) & Mostrar cómo operaciones reg‑mem reducen instrucciones pero aumentan complejidad de decodificación.\\
\addlinespace
\textbf{Coste de aprendizaje} & Alta (gran cantidad de conceptos) & Moderada (más sencillo que x86) & Baja-moderada (buena para enseñanza) & Secuenciar contenido: empezar por RISC (conceptos básicos), luego introducir x86 con ejemplos concretos.\\
\textbf{Relevancia docente} & Muy alta (ecosistema y software legado) & Alta en móviles/embebidos & Alta en investigación y docencia & Seleccionar ejemplos y herramientas: simuladores básicos para RISC-V; VonSim para x86 con repertorio limitado.\\
\bottomrule
\end{tabular}}
\end{table}
