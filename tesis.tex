...
La utilización de simuladores educativos permite complementar la enseñanza teórica mediante entornos interactivos que facilitan la experimentación y la visualización de procesos internos del sistema. En este contexto, los simuladores orientados a la enseñanza deben priorizar la simplicidad conceptual, la claridad visual y la interacción guiada, de modo que los estudiantes puedan concentrarse en comprender los principios arquitectónicos sin verse sobrecargados por la complejidad de la herramienta.
...
En este contexto, resulta pertinente incorporar tecnologías educativas que faciliten la representación interactiva de los procesos internos de los sistemas computacionales y promuevan estrategias de aprendizaje activo. Estas tecnologías permiten situar al estudiante en un contexto que imita aspectos de la realidad, posibilitando la detección y análisis de problemáticas semejantes a las que se presentan en entornos reales. Este enfoque promueve un mejor entendimiento a través del trabajo exploratorio, la inferencia, el aprendizaje por descubrimiento y el desarrollo de habilidades \protect\hyperlink{ref-nova_tool_2013}{{[}12{]}}, \protect\hyperlink{ref-mustafa_evaluating_2010}{{[}13{]}}, \protect\hyperlink{ref-garcia-carballeira_wepsim_2020}{{[}14{]}}, \protect\hyperlink{ref-prasad2016using}{{[}15{]}}.
...
Dentro del estudio de las arquitecturas de computadoras, la arquitectura x86 ocupa un lugar central debido a su extensa adopción en computadoras personales y servidores, así como a su capacidad de mantener compatibilidad retroactiva con generaciones previas de software. A continuación, se presenta una breve evolución de esta arquitectura, que justifica su inclusión en el diseño de herramientas de simulación para la enseñanza. Comenzó con el procesador Intel 8086 en 1978 como una arquitectura de 16 bits. Evolucionó a una arquitectura de 32 bits con el procesador Intel 80386 en 1985 (i386 o x86-32) y posteriormente a 64 bits con las extensiones de AMD (AMD64) y su adopción por Intel (Intel 64) \protect\hyperlink{ref-intel_64_2025}{{[}22{]}}, \protect\hyperlink{ref-amd_developer_2024}{{[}23{]}}.
...
Las funcionalidades principales del simulador propuesto se resumen en la Tabla \ref{tab:funcionalidadessimulador}, donde se detallan los componentes, sus funciones y el propósito didáctico de cada uno.
...
Uno de los conceptos fundamentales en esta disciplina es la distinción entre \textbf{arquitectura} y \textbf{organización} de computadoras. Mientras que la arquitectura describe los atributos del sistema visibles para el programador ---como el conjunto de instrucciones, los registros y los modos de direccionamiento---, la organización se refiere a la implementación interna del hardware que permite ejecutar dichas especificaciones. \protect\hyperlink{ref-hennessy2017computer}{{[}19{]}}, \protect\hyperlink{ref-stallings_computer_2021}{{[}20{]}}, \protect\hyperlink{ref-tanenbaum_structured_2016}{{[}31{]}}, \protect\hyperlink{ref-murdocca_principles_2000}{{[}32{]}}, \protect\hyperlink{ref-bryant2015computer}{{[}33{]}}.
...
{\centering \includegraphics[width=0.75\linewidth]{images/vonneumann} 
...
Mientras la arquitectura Von Neumann se consolidaba como el paradigma dominante, surgió en paralelo un enfoque alternativo: la arquitectura Harvard. Su origen se remonta al desarrollo del Harvard Mark I, una computadora electromecánica construida entre 1939 y 1944 en la Universidad de Harvard bajo la dirección de Howard Aiken y con el apoyo de IBM \protect\hyperlink{ref-ceruzzi_history_2003}{{[}44{]}}, \protect\hyperlink{ref-williams1998history}{{[}45{]}}. Este sistema sentó las bases para un modelo arquitectónico que se distingue por la separación física entre instrucciones y datos, implementada mediante memorias independientes y buses dedicados para cada tipo de información. Esta organización elimina la competencia por el bus entre instrucciones y datos, lo que permite un acceso paralelo y mejora significativamente la eficiencia del procesamiento, especialmente en aplicaciones críticas para el rendimiento.

La Figura \ref{fig:harvard} ilustra la estructura fundamental de la arquitectura Harvard, destacando la independencia de los canales de acceso a memoria y la disposición diferenciada de instrucciones y datos. Esta representación visual facilita la comprensión de las ventajas técnicas del modelo, que ha sido ampliamente adoptado en sistemas embebidos, microcontroladores y procesadores de señal digital (DSP) \protect\hyperlink{ref-noergaard2012embedded}{{[}46{]}}. A continuación, se presenta una comparación sistemática entre ambos modelos, con el objetivo de analizar sus implicancias técnicas y contextos de aplicación \protect\hyperlink{ref-tanenbaum_structured_2016}{{[}31{]}}.

\begin{figure}

{\centering \includegraphics[width=0.75\linewidth]{images/harvard} 
...
\begin{table}[!h]
\centering
\caption{\label{tab:comparativavonneumannharvard}Cuadro comparativo entre arquitecturas Von Neumann y Harvard}
\centering
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\fontsize{12}{14}\selectfont
...
\begin{tablenotes}
\item \textit{Nota: } 
\item AVR y PIC son familias de microcontroladores ampliamente utilizadas en sistemas embebidos. AVR (Advanced Virtual RISC) es una arquitectura desarrollada por Atmel, caracterizada por su eficiencia y simplicidad. PIC (Peripheral Interface Controller) es una línea de microcontroladores de Microchip Technology, reconocida por su flexibilidad y facilidad de programación.
\end{tablenotes}
\end{threeparttable}}
\end{table}
...
Numerosas arquitecturas contemporáneas adoptan un enfoque híbrido, también denominado arquitectura Harvard modificada. Este modelo implementa memorias separadas para datos e instrucciones a nivel microarquitectónico, frecuentemente mediante la utilización de memorias caché de nivel 1 (L1) diferenciadas para cada tipo de información. Sin embargo, desde la perspectiva del programador, el modelo de memoria se presenta como unificado, lo que facilita el desarrollo de software sin exponer la complejidad interna del diseño. Esta dualidad permite optimizar la implementación física del procesador, maximizando el rendimiento y la eficiencia energética sin comprometer la simplicidad del modelo de programación \protect\hyperlink{ref-hennessy2017computer}{{[}19{]}}, \protect\hyperlink{ref-stallings_computer_2021}{{[}20{]}}, \protect\hyperlink{ref-null_essentials_2023}{{[}36{]}}, \protect\hyperlink{ref-patterson_computer_2017}{{[}40{]}}.
...
{\centering \includegraphics[width=0.75\linewidth]{images/hibrida} 
...
\toprule
\multicolumn{1}{>{\centering\arraybackslash}p{4cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Característica}}} & \multicolumn{1}{>{\centering\arraybackslash}p{4cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{x86}}} & \multicolumn{1}{>{\centering\arraybackslash}p{4cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{ARM}}} & \multicolumn{1}{>{\centering\arraybackslash}p{4cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{RISC-V}}}\\
\midrule
\textbf{Eficiencia energética} & Moderada-alta, depende de la microarquitectura & Alta, optimizada para dispositivos móviles & Alta, diseño abierto y eficiente\\
\addlinespace[16pt]
\textbf{Complejidad del repertorio de instrucciones (ISA)} & Alta (ISA extensa, instrucciones de longitud variable) & Baja (ISA reducida, instrucciones simples y de longitud fija) & Baja (ISA modular, extensible y de longitud fija)\\
\addlinespace[16pt]
\textbf{Rendimiento típico} & Elevado en entornos de escritorio y servidores & Moderado/alto en sistemas embebidos y móviles & Moderado, escalable según extensiones implementadas\\
\addlinespace[16pt]
\textbf{Ámbito de aplicación principal} & Computadoras personales, servidores, software legado & Dispositivos móviles, sistemas embebidos, IoT & Investigación, docencia, aplicaciones personalizadas\\
\addlinespace[16pt]
\bottomrule
\end{tabular}}
\end{table}
...
Esta ilustración facilita el análisis comparativo de las ventajas y limitaciones de cada modalidad, aportando claridad conceptual al estudio de la interacción entre instrucciones, memoria y registros \protect\hyperlink{ref-stallings_computer_2021}{{[}20{]}}.

\begin{figure}

{\centering \includegraphics[width=0.7\linewidth]{images/modosdireccionamiento} 
...
\begin{tabular}[t]{>{\raggedright\arraybackslash}p{3.5cm}|>{\centering\arraybackslash}p{3cm}>{\centering\arraybackslash}p{3cm}>{\centering\arraybackslash}p{3cm}>{\centering\arraybackslash}p{4.5cm}}
\toprule
\multicolumn{1}{>{\centering\arraybackslash}p{3.5cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Arquitectura}}} & \multicolumn{1}{>{\centering\arraybackslash}p{3cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Longitud instrucción}}} & \multicolumn{1}{>{\centering\arraybackslash}p{3cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Nº operandos}}} & \multicolumn{1}{>{\centering\arraybackslash}p{3cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Tipos de operandos}}} & \multicolumn{1}{>{\centering\arraybackslash}p{4.5cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Modos de direccionamiento}}}\\
\midrule
...
{\centering \includegraphics[width=0.7\linewidth]{images/convergen} 
...
\caption{Diagrama esquemático microprocesador Intel 8086}\label{fig:esquemax86}
\end{figure}
...
\begin{tabular}[t]{>{\raggedright\arraybackslash}p{4cm}>{\centering\arraybackslash}p{4cm}>{\centering\arraybackslash}p{3cm}>{\raggedright\arraybackslash}p{5cm}}
\toprule
\multicolumn{1}{>{\centering\arraybackslash}p{4cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Procesador}}} & \multicolumn{1}{>{\centering\arraybackslash}p{4cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Año de Lanzamiento}}} & \multicolumn{1}{>{\centering\arraybackslash}p{3cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Número de Bits}}} & \multicolumn{1}{>{\centering\arraybackslash}p{5cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Extensiones de 64 bits}}}\\
\midrule
...
Una instrucción típica en la arquitectura x86 puede estar compuesta por diversos elementos, entre los que se destacan los siguientes \protect\hyperlink{ref-stallings_computer_2021}{{[}20{]}}:
...
{\centering \includegraphics[width=0.95\linewidth]{images/ensamblador} 
...
Estos principios generales se materializan en diversos sectores industriales, donde la simulación desempeña un papel fundamental en el diseño, el entrenamiento, la evaluación y la toma de decisiones. La Tabla \ref{tab:aplisimu} resume las aplicaciones más relevantes de la simulación en estos ámbitos, evidenciando su impacto en la optimización de procesos y la mejora de resultados.

\begin{table}[!h]
\centering
\caption{\label{tab:aplisimu}Aplicaciones de la simulación en distintos sectores}
\centering
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\fontsize{12}{14}\selectfont
...
El formalismo DEVS se fundamenta en la modelización de sistemas como conjuntos de componentes que interactúan mediante eventos discretos. Cada componente posee un estado interno y puede recibir eventos externos que desencadenan transiciones de estado, ya sea en instantes temporizados o ante condiciones específicas. Esta estructura modular permite representar tanto la dinámica interna como la interacción externa del sistema modelado. La Figura \ref{fig:devs} ilustra la relación entre modelos atómicos y modelos acoplados en DEVS, evidenciando cómo se integran y comunican los distintos elementos para capturar el comportamiento global del sistema.

\begin{figure}

{\centering \includegraphics[width=0.45\linewidth]{images/devs} 
...
La Tabla \ref{tab:criterios} sintetiza los criterios de evaluación, junto con sus respectivos indicadores y escalas, empleados para analizar sistemáticamente los simuladores seleccionados.
...
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\fontsize{12}{14}\selectfont
\begin{tabular}[t]{>{\raggedright\arraybackslash}p{4cm}|>{\raggedright\arraybackslash}p{6cm}>{\centering\arraybackslash}p{6cm}}
\toprule
...
El proceso de selección consideró una evaluación preliminar basada en el tiempo requerido para el análisis y el grado de cumplimiento de los criterios definidos, priorizando aquellos simuladores que ofrecieran un equilibrio adecuado entre funcionalidad, usabilidad, calidad de la documentación y alineación con los contenidos de la asignatura Arquitectura de Computadoras. De esta preselección, se eligieron tres herramientas que, en principio, cumplían con la mayor cantidad de criterios evaluativos: \textbf{Emu8086}, \textbf{VonSim} y \textbf{Simple 8-bit Assembler Simulator}.

La Tabla \ref{tab:simuladores} sintetiza el proceso de selección, mostrando el resultado de la exploración previa y la evaluación final de cada simulador. Esta presentación estructurada permite visualizar de manera clara el criterio aplicado y la justificación de la elección de las herramientas analizadas.
...
La Tabla \ref{tab:tabla-comparativa-criterios} presenta una síntesis comparativa de los simuladores seleccionados, evaluados según los criterios previamente definidos. En ella se destacan las diferencias y fortalezas de cada herramienta en relación con la usabilidad, funcionalidades del editor, calidad de la documentación, ejecución de simulación, nivel de especificación de la arquitectura x86, características técnicas y cobertura de contenidos curriculares.
...
\item
  ...
  La representación gráfica de la CPU, los buses, la memoria y los dispositivos de entrada/salida resulta fundamental para visualizar la interacción y el funcionamiento integrado de los componentes durante el ciclo de instrucción. La Figura \ref{fig:diagramasimulador} muestra la estructura general del simulador VonSim8, destacando los elementos activos en cada etapa y facilitando una comprensión sistémica del proceso de ejecución. Este recurso didáctico, respaldado por estudios sobre visualización en disciplinas técnicas \protect\hyperlink{ref-sorva2013visualizations}{{[}75{]}}, contribuye significativamente a la asimilación de conceptos abstractos y al desarrollo de competencias en arquitectura de computadoras.
\end{enumerate}

\begin{figure}

\hfill{}\includegraphics[width=1\linewidth]{images/estructurasimulador} 
...
\item
  ...
  La posibilidad de ejecutar programas escritos en lenguaje ensamblador, tanto de manera paso a paso como en ejecución continua, constituye un recurso esencial para el análisis detallado de cada instrucción. Esta funcionalidad fortalece las competencias en trazado y depuración de código ensamblador, aspectos fundamentales para comprender la interacción entre software y hardware.
  Para facilitar este proceso, se incorpora un editor de ensamblador dotado de funciones avanzadas, como el resaltado de sintaxis y el autocompletado. Estas características, alineadas con los principios de diseño de interfaces orientados a la usabilidad y accesibilidad \protect\hyperlink{ref-w3c_accessibility_2021}{{[}76{]}}, optimizan la experiencia del usuario y favorecen la escritura y comprensión del código. El editor permite al usuario escribir, editar, guardar y ejecutar programas en ensamblador directamente dentro del simulador, además de ofrecer ejemplos predefinidos como apoyo didáctico. La integración de entornos de desarrollo integrados (IDEs) en contextos educativos ha demostrado ser eficaz para la enseñanza de lenguajes de programación, según diversos estudios \protect\hyperlink{ref-mccracken2001does}{{[}77{]}}. La Figura \ref{fig:editor} ilustra la interfaz del editor ensamblador implementado en el simulador, evidenciando las funcionalidades mencionadas y su aporte al proceso de aprendizaje.
\end{enumerate}

\begin{figure}

{\centering \includegraphics[width=0.4\linewidth]{images/editor} 
...
\item
  ...
  La activación progresiva del repertorio de instrucciones promueve el desarrollo paulatino de competencias, evitando que los estudiantes se enfrenten prematuramente a la totalidad del conjunto instruccional. Este método se fundamenta en teorías de aprendizaje que destacan los beneficios de la exposición gradual a nuevos conceptos para mejorar la comprensión y la retención \protect\hyperlink{ref-sweller2010cognitive}{{[}21{]}}. La Tabla \ref{tab:activacionprogresiva} resume el proceso de activación progresiva del repertorio de instrucciones, especificando las fases de incorporación y los objetivos didácticos correspondientes a cada etapa. Esta representación facilita la comprensión de cómo la introducción gradual de instrucciones contribuye al desarrollo de competencias específicas y potencia el aprendizaje en arquitectura de computadoras.
\end{enumerate}
...
\item
  ...
  Se implementa una visualización interactiva del flujo de datos basada en el modelo de Nivel de Transferencia entre Registros (Register Transfer Level, RTL). Este enfoque permite representar con precisión el desplazamiento de datos entre registros, buses y unidades funcionales del procesador, así como las señales de control involucradas en cada fase del ciclo de instrucción \protect\hyperlink{ref-harris2015digital}{{[}35{]}}, \protect\hyperlink{ref-ASMVisualizer2025}{{[}79{]}}. Stallings \protect\hyperlink{ref-stallings_computer_2021}{{[}20{]}} propone utilizar el modelo RTL para representar el ciclo de instrucción, desde la captura (\emph{Fetch}) hasta la ejecución (\emph{Execute}), facilitando la visualización del recorrido de datos y señales de control en cada etapa del proceso. La Figura \ref{fig:cicloinstruccion} representa de manera visual y secuencial las etapas fundamentales del ciclo de instrucción, ejemplificadas mediante la operación \texttt{MOV\ AL,\ BL}. En la etapa de captación (\emph{Fetch}), se observa cómo la dirección de la instrucción es transferida del registro \texttt{IP} al \texttt{MAR}, seguida por la lectura de la memoria hacia el \texttt{MBR} y su posterior carga en el \texttt{IR}. En la etapa de ejecución (\emph{Execute}), el dato se mueve desde el registro \texttt{BL} al registro \texttt{AL}, ilustrando el flujo de información y el funcionamiento interno del procesador durante la ejecución de una instrucción. Esta representación facilita la comprensión didáctica de los procesos involucrados en el ciclo de instrucción.
\end{enumerate}

\begin{figure}

{\centering \includegraphics[width=0.95\linewidth]{images/cicloinstruccion} 
...
\item
  ...
  La incorporación de un vector de interrupciones predefinido permite simular eventos externos, como la entrada de datos mediante teclado o la salida de información a través de un monitor. Ante la recepción de una interrupción, el simulador activa la señal \texttt{INTR}, detiene la ejecución actual y transfiere el control a la rutina correspondiente, facilitando la exploración interactiva de conceptos clave como la asincronía, el manejo de eventos y la interrupción del flujo secuencial. Estos aspectos, característicos de arquitecturas modernas, resultan fundamentales para el entendimiento de sistemas reales y se alinean con las recomendaciones de Null y Lobur \protect\hyperlink{ref-null_essentials_2023}{{[}36{]}}, quienes destacan la importancia de abordar estos conceptos en etapas tempranas de la formación.
  Adicionalmente, se implementa un módulo genérico de entrada/salida programada (Programmed Input/Output, PIO), que actúa como interfaz entre la CPU y los dispositivos periféricos. Este módulo posibilita la simulación de operaciones mediante instrucciones como IN y OUT, permitiendo al estudiante interactuar con dispositivos representados gráficamente, como interruptores y teclas. De esta manera, se favorece una comprensión más tangible de los mecanismos subyacentes al intercambio de información entre el procesador y los dispositivos externos. La Figura \ref{fig:perifericos} muestra el módulo PIO y su interacción con los periféricos, ilustrando visualmente la estructura y funcionamiento de este componente dentro del simulador.
\end{enumerate}

\begin{figure}

{\centering \includegraphics[width=0.9\linewidth]{images/perifericos} 

}

\caption{Módulo genérico de entrada/salida programada (PIO)}\label{fig:perifericos}
\end{figure}
...
\toprule
\multicolumn{1}{>{\centering\arraybackslash}p{5cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Requisito}}} & \multicolumn{1}{>{\centering\arraybackslash}p{12cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Fundamento pedagógico / técnico}}}\\
\midrule
...
En esta sección se describe la estructura del simulador VonSim8. El diseño de los registros se concibió con un propósito pedagógico: facilitar la comprensión de los modos de direccionamiento y del ciclo de instrucción, elementos fundamentales en el estudio de la Arquitectura de Computadoras \protect\hyperlink{ref-stallings_computer_2021}{{[}20{]}}.

La Tabla \ref{tab:estructuravonsim8} ofrece una descripción detallada de los componentes principales del simulador, especificando sus características y funcionalidades. Este panorama estructural permite identificar los elementos esenciales de la arquitectura implementada, facilitando la comprensión integral del diseño y su correspondencia con los objetivos pedagógicos del proyecto.
...
Los registros de propósito general son aquellos que el programador puede utilizar libremente para operaciones aritméticas, lógicas y de transferencia de datos. En el simulador, se dispone de cuatro registros de propósito general de 8 bits: \texttt{AL}, \texttt{BL}, \texttt{CL} y \texttt{DL}. Estos registros pueden emplearse como operandos en las instrucciones del repertorio.
...
\tightlist
\item
  \textbf{\texttt{SP} (\emph{Stack Pointer})}: Gestiona la pila, permitiendo rastrear las direcciones de memoria involucradas en las operaciones de apilamiento y desapilamiento.
\item
  \textbf{\texttt{Flags} (\emph{Flags Register})}: Almacena las banderas de estado, que reflejan los resultados de las operaciones aritméticas y lógicas, y permiten controlar el flujo de ejecución del programa.
\item
  \textbf{\texttt{IP} (\emph{Instruction Pointer})}: Guarda la dirección de memoria de la próxima instrucción a ejecutar, garantizando la correcta secuenciación del programa.
\item
  \textbf{\texttt{IR} (\emph{Instruction Register})}: Contiene el byte correspondiente a la instrucción que está siendo decodificada y ejecutada en ese instante.
\end{itemize}
...
\tightlist
\item
  \textbf{\texttt{MAR} (\emph{Memory Address Register})}: Almacena las direcciones de memoria a las que se desea acceder.\\
\item
  \textbf{\texttt{MBR} (\emph{Memory Buffer Register})}: Retiene temporalmente el byte de datos que se transfiere entre la memoria principal y la CPU a través del bus de datos.
\end{itemize}

Finalmente, se incorporan dos registros auxiliares: \texttt{ri}, utilizado para el almacenamiento temporal de direcciones, e \texttt{id}, destinado al almacenamiento temporal de datos. Ambos cumplen funciones de apoyo durante la ejecución de instrucciones, optimizando la gestión interna de la CPU.
...
El registro \texttt{Flags} almacena banderas de estado resultantes de operaciones aritméticas, lógicas, de control y de gestión del flujo. Estas banderas permiten a la Unidad de Control modificar el flujo mediante saltos condicionales o habilitar/deshabilitar servicios (interrupciones).

La Tabla \ref{tab:flags} presenta una síntesis de las banderas de estado implementadas en esta versión del simulador, detallando su función y relevancia en el procesamiento de instrucciones. Esta información resulta fundamental para comprender el mecanismo de control y la gestión de eventos internos durante la ejecución de programas en arquitectura x86.
...
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\fontsize{12}{14}\selectfont
\begin{tabular}[t]{>{\centering\arraybackslash}p{1.5cm}|>{\centering\arraybackslash}p{2.5cm}>{\raggedright\arraybackslash}p{10cm}}
\toprule
\multicolumn{1}{>{\centering\arraybackslash}p{1.5cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Bit N°}}} & \multicolumn{1}{>{\centering\arraybackslash}p{2.5cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Abreviatura}}} & \multicolumn{1}{>{\centering\arraybackslash}p{10cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Descripción}}}\\
\midrule
...
\fontsize{12}{14}\selectfont
\begin{threeparttable}
\begin{tabular}[t]{>{\raggedright\arraybackslash}p{3cm}|>{\centering\arraybackslash}p{2.3cm}>{\centering\arraybackslash}p{2.3cm}>{\centering\arraybackslash}p{2.3cm}>{\centering\arraybackslash}p{2.3cm}>{\centering\arraybackslash}p{2.3cm}}
\toprule
\multicolumn{1}{>{\centering\arraybackslash}p{3cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Clase}}} & \multicolumn{1}{>{\centering\arraybackslash}p{2.3cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Z}}} & \multicolumn{1}{>{\centering\arraybackslash}p{2.3cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{S}}} & \multicolumn{1}{>{\centering\arraybackslash}p{2.3cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{C}}} & \multicolumn{1}{>{\centering\arraybackslash}p{2.3cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{O}}} & \multicolumn{1}{>{\centering\arraybackslash}p{2.3cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{I}}}\\
\midrule
\textbf{MOV} & -- & -- & -- & -- & Preserva\\
\addlinespace[1pt]
...
La Figura \ref{fig:memoria} muestra la estructura visual de la memoria principal, destacando su disposición matricial y la accesibilidad de cada celda para el análisis y la manipulación de datos durante la simulación.
...
La Figura \ref{fig:buses} ilustra la estructura y disposición de los buses en el simulador, evidenciando su función como elementos clave para la interconexión y el flujo de información en la arquitectura implementada.
...
La Figura \ref{fig:banderas} muestra la representación visual del registro Flags, evidenciando la disposición de las banderas y su configuración dinámica según el nivel de avance del estudiante.

\begin{figure}

{\centering \includegraphics[width=0.9\linewidth]{images/flags} 
...
El flag de interrupción I se visualiza únicamente cuando el programa lo requiere, como ocurre al ejecutar instrucciones específicas de interrupción, tales como \texttt{INT} o \texttt{IRET}. Esta funcionalidad permite que los estudiantes observen de manera directa la activación y desactivación del flag, comprendiendo su impacto en el flujo de ejecución y en la gestión de eventos asincrónicos dentro del simulador

La Figura \ref{fig:banderaI} muestra la representación gráfica del registro de estado I, evidenciando su comportamiento dinámico y su relevancia en el manejo de interrupciones.

\begin{figure}

{\centering \includegraphics[width=0.2\linewidth]{images/flagi} 
...
La Figura \ref{fig:controles} presenta la nueva disposición del menú de controles, destacando las opciones disponibles y su relevancia para el aprendizaje práctico en arquitectura de computadoras.

\begin{figure}

{\centering \includegraphics[width=0.9\linewidth]{images/controles} 
...
La Figura \ref{fig:registros} muestra la disposición y estructura de los registros de 8 bits implementados en el simulador, evidenciando su funcionalidad y su aporte a la didáctica del modelo.

\begin{figure}

{\centering \includegraphics[width=0.8\linewidth]{images/registros} 
...
La supresión de los registros temporales \texttt{left}, \texttt{right} y \texttt{result} en la ALU responde al objetivo de simplificar el flujo de datos y disminuir la carga cognitiva de los estudiantes. En lugar de emplear registros intermedios, el simulador representa de manera directa las operaciones realizadas sobre los operandos, lo que facilita la comprensión del funcionamiento aritmético y lógico de la unidad. Esta estrategia didáctica permite al usuario observar de forma más transparente el procesamiento interno, favoreciendo el aprendizaje de los mecanismos fundamentales de la ALU.

La Figura \ref{fig:leftrigth} ilustra la eliminación de estos registros temporales, mostrando la simplificación lograda en la visualización y el flujo de datos dentro del simulador.

\begin{figure}

{\centering \includegraphics[width=0.9\linewidth]{images/leftrigth} 
...
Por motivos pedagógicos, los registros \texttt{SP}, \texttt{ri} e \texttt{id} permanecen ocultos en la interfaz del simulador y solo se habilitan de manera automática cuando una instrucción específica requiere su utilización. Esta estrategia contribuye a simplificar la visualización y a reducir la carga cognitiva durante las etapas iniciales del aprendizaje, permitiendo que el estudiante se enfoque en los componentes esenciales del ciclo de instrucción. La Figura \ref{fig:idrisp} muestra la representación gráfica de estos registros, evidenciando su activación dinámica y su función de apoyo en la ejecución de instrucciones.

\begin{figure}

{\centering \includegraphics[width=0.22\linewidth]{images/idrisp} 
...
La memoria principal del simulador se representa mediante una matriz de 16×16 celdas en formato hexadecimal, lo que posibilita el almacenamiento de hasta 256 bytes de datos. Este diseño visual facilita la identificación de cada posición de memoria y promueve una comprensión intuitiva de la organización y gestión de datos en la arquitectura implementada. La Figura \ref{fig:memoriacomp} ilustra la estructura matricial de la memoria, destacando su accesibilidad y utilidad para el análisis y la manipulación durante la simulación.
...
El simulador incorpora un mecanismo visual que destaca, dentro de la memoria principal, las posiciones apuntadas por los registros \texttt{IP} y \texttt{SP}. Esta funcionalidad permite al estudiante identificar de manera inmediata tanto la dirección de la próxima instrucción a ejecutar como la ubicación actual de la cima de la pila, facilitando el seguimiento del flujo de ejecución y la gestión de la memoria durante el ciclo de instrucción. La Figura \ref{fig:resaltadoipsp} muestra el resaltado simultáneo de estas direcciones, evidenciando su importancia para la comprensión de la dinámica interna del procesador.

\begin{figure}

{\centering \includegraphics[width=0.8\linewidth]{images/resaltadoipsp} 

}

\caption{Resaltado registro IP y registro SP}\label{fig:resaltadoipsp}
\end{figure}

El simulador VonSim8 destaca visualmente, mediante el uso de color naranja, el espacio reservado en la memoria principal para el vector de interrupciones, que abarca las ocho posiciones comprendidas entre las direcciones \texttt{0x00h} a \texttt{0x07h}. Esta representación facilita la identificación del área destinada al manejo de interrupciones, permitiendo al estudiante comprender su ubicación y relevancia dentro de la arquitectura. La Figura \ref{fig:resaltadoint} muestra el resaltado específico de este segmento, evidenciando su función en la gestión de eventos externos y en la organización interna del sistema.

\begin{figure}

{\centering \includegraphics[width=0.45\linewidth]{images/resaltadoint} 

}

\caption{Resaltado vector de interrupciones}\label{fig:resaltadoint}
\end{figure}
...
Con el objetivo de facilitar el análisis y la comprensión del funcionamiento interno del simulador, se incorporó un visor de instrucciones y datos del programa en memoria. Esta herramienta permite al estudiante visualizar, para cada celda de memoria, la instrucción almacenada, el tamaño en bytes que ocupa y la etiqueta asociada a los datos. De este modo, se promueve una interpretación detallada del contenido de la memoria y se favorece el seguimiento del flujo de ejecución durante la simulación. La Figura \ref{fig:visorprog} muestra la interfaz del visor, evidenciando su utilidad para el estudio y la manipulación de instrucciones y datos en el entorno educativo.
...
Con el fin de asegurar la compatibilidad con programas desarrollados para VonSim, el simulador VonSim8 permite cargar programas utilizando la directiva ORG, aunque adapta la dirección inicial de ejecución. En lugar de emplear la dirección\texttt{0x2000h}, característica de VonSim, se utiliza la dirección \texttt{0x20h} en VonSim8, lo que facilita la ejecución de programas en el entorno simplificado. Esta flexibilidad permite a los usuarios definir direcciones personalizadas y mantener la interoperabilidad entre ambos simuladores. La Figura \ref{fig:org} muestra ejemplos de programas con la directiva ORG y END, evidenciando la compatibilidad y la adaptación de las direcciones de inicio en cada caso.

\begin{figure}

{\centering \includegraphics[width=0.8\linewidth]{images/org} 
...
\begin{tabular}[t]{>{\raggedright\arraybackslash}p{4cm}|>{\raggedright\arraybackslash}p{9cm}>{\raggedright\arraybackslash}p{9cm}}
\toprule
\multicolumn{1}{>{\centering\arraybackslash}p{4cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Característica}}} & \multicolumn{1}{>{\centering\arraybackslash}p{9cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{VonSim}}} & \multicolumn{1}{>{\centering\arraybackslash}p{9cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{VonSim8}}}\\
\midrule
...
En el simulador VonSim8, el parámetro \texttt{xx} de las instrucciones \texttt{Jxx} permite realizar saltos condicionales en función de distintas combinaciones de banderas de estado, como cero (Z), acarreo (C), signo (S) y desbordamiento (O). La negación de una bandera se indica mediante la letra N, lo que otorga mayor flexibilidad para controlar el flujo de ejecución en programas ensamblador. La Tabla \ref{tab:saltoscondicionales} sintetiza las instrucciones de salto condicional implementadas, detallando la condición asociada a cada bandera y su efecto sobre el flujo del programa.
...
\tightlist
\item
  \textbf{Transferencia y procesamiento de datos}: este grupo de instrucciones permite mover datos entre registros y memoria, así como realizar operaciones aritméticas. Resultan fundamentales para comprender el flujo de información en una arquitectura computacional. Por ejemplo, la siguiente operación aritmética en Python:
\end{itemize}
...
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\fontsize{16}{18}\selectfont
\begin{tabular}[t]{>{\raggedright\arraybackslash}p{4cm}|>{\centering\arraybackslash}p{3cm}>{\centering\arraybackslash}p{3cm}>{\centering\arraybackslash}p{2.5cm}>{\raggedright\arraybackslash}p{10cm}}
\toprule
\multicolumn{1}{>{\centering\arraybackslash}p{4cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Categoría}}} & \multicolumn{1}{>{\centering\arraybackslash}p{3cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Instrucción}}} & \multicolumn{1}{>{\centering\arraybackslash}p{3cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Código operación}}} & \multicolumn{1}{>{\centering\arraybackslash}p{2.5cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Operandos}}} & \multicolumn{1}{>{\centering\arraybackslash}p{10cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Acción}}}\\
\midrule
...
\fontsize{12}{14}\selectfont
\begin{threeparttable}
\begin{tabular}[t]{>{\centering\arraybackslash}p{2cm}|>{\raggedright\arraybackslash}p{3.5cm}>{\centering\arraybackslash}p{3.5cm}>{\raggedright\arraybackslash}p{8cm}}
\toprule
\multicolumn{1}{>{\centering\arraybackslash}p{2cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{CodOp}}} & \multicolumn{1}{>{\centering\arraybackslash}p{3.5cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Instrucción}}} & \multicolumn{1}{>{\centering\arraybackslash}p{3.5cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Byte}}} & \multicolumn{1}{>{\centering\arraybackslash}p{8cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Codificación}}}\\
\midrule
...
La Tabla \ref{tab:tablaregistros} muestra los códigos binarios y decimales asignados a los registros de propósito general AL, BL, CL y DL, habilitados para su uso como Rx y Ry en las instrucciones del simulador.

\begin{table}[!h]
\centering
\caption{\label{tab:tablaregistros}Tabla de registros de propósito general}
\centering
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\fontsize{11}{13}\selectfont
\begin{threeparttable}
...
Las microoperaciones que conforman el ciclo de instrucción se describen utilizando la notación de transferencia entre registros: \texttt{destino} \(\leftarrow\) \texttt{origen}.

La Figura \ref{fig:flujoCicloInstruccion} esquematiza el flujo general del ciclo de instrucción en VonSim8, distinguiendo claramente las dos etapas fundamentales: captación (\emph{Fetch}) y ejecución (\emph{Execute}). Esta representación visual facilita la comprensión de la secuencia lógica y la interacción entre componentes durante la ejecución de cada instrucción, aportando claridad didáctica al análisis del funcionamiento interno del procesador.
...
La Tabla \ref{tab:componentesvonsim8} sintetiza los modelos y componentes fundamentales del simulador VonSim8, especificando para cada uno sus funciones, señales de entrada y salida, así como los retardos asociados. Esta presentación estructurada permite comprender la organización interna del sistema y el comportamiento de sus elementos durante la simulación.
...
La Tabla \ref{tab:microcaptacionfetch} expone la secuencia de microoperaciones y eventos DEVS que modelan la fase de captación (Fetch) en el simulador. Esta representación permite analizar en detalle el flujo de acciones internas durante la obtención de instrucciones, aportando rigor al estudio del ciclo de ejecución.
...
La Tabla \ref{tab:microoperacioneseje} detalla la secuencia de microoperaciones que conforman la ejecución de la instrucción modelada en el simulador. Esta información permite analizar el proceso interno paso a paso, facilitando la comprensión de la dinámica operativa durante la fase de ejecución.
...
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\fontsize{14}{16}\selectfont
\begin{tabular}[t]{>{\raggedright\arraybackslash}p{1cm}|>{\raggedright\arraybackslash}p{7cm}>{\raggedright\arraybackslash}p{5cm}>{\raggedright\arraybackslash}p{5cm}}
\toprule
\multicolumn{1}{>{\centering\arraybackslash}p{1cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Paso}}} & \multicolumn{1}{>{\centering\arraybackslash}p{7cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Microoperación}}} & \multicolumn{1}{>{\centering\arraybackslash}p{5cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Descripción}}} & \multicolumn{1}{>{\centering\arraybackslash}p{5cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Funciones DEVS}}}\\
\midrule
...
La Figura \ref{fig:dispositivos} ofrece una representación integral de la arquitectura del simulador, ilustrando la disposición y conectividad de los distintos dispositivos y módulos. Esta visualización facilita la comprensión de la estructura interna y la interacción entre componentes, aportando claridad al análisis de los mecanismos de comunicación y gestión de recursos implementados en el sistema.
...
La Tabla \ref{tab:tisa} presenta las categorías de instrucciones diseñadas para la interacción con los módulos de entrada y salida, junto con sus códigos de operación y las acciones correspondientes. Esta síntesis facilita la comprensión de los mecanismos de comunicación y gestión de interrupciones implementados en el simulador.
...
El simulador VonSim8 integra un teclado y una pantalla como periféricos básicos de entrada y salida, cuya interacción se realiza a través de rutinas de sistema invocadas mediante interrupciones. Esta implementación permite al estudiante experimentar con la gestión de datos y la comunicación entre el procesador y los dispositivos externos, facilitando la comprensión de los mecanismos fundamentales de E/S en una arquitectura computacional. La Figura \ref{fig:tecladopantalla} muestra la representación visual de estos periféricos, evidenciando su funcionalidad y su integración en el entorno de simulación.

\begin{figure}

{\centering \includegraphics[width=0.6\linewidth]{images/tecladopantalla} 
...
\tightlist
\item
  \texttt{OUT\ puerto,\ AL}: envía el contenido de \texttt{AL} al puerto indicado (dirección de 8 bits, de \texttt{00h} a \texttt{FFh}), permitiendo transmitir datos a dispositivos externos.
\item
  \texttt{IN\ AL,\ puerto}: recibe un byte desde el puerto especificado y lo almacena en \texttt{AL}.
\end{itemize}
...
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\fontsize{11}{13}\selectfont
\begin{tabular}[t]{>{\centering\arraybackslash}p{2cm}|>{\centering\arraybackslash}p{7cm}}
\toprule
...
La Tabla \ref{tab:tablaaspectos} presenta los promedios de calificación obtenidos en la evaluación del material teórico y de las actividades propuestas, evidenciando una valoración positiva por parte de los estudiantes. Esta síntesis permite identificar los aspectos más destacados y las áreas de mejora en el proceso de enseñanza.
...
La Tabla \ref{tab:tablaevaluacion} sintetiza los resultados obtenidos en la evaluación del simulador VonSim8, reflejando una valoración positiva por parte de los estudiantes y señalando áreas de mejora. Esta presentación permite identificar los aspectos más destacados y las oportunidades de optimización en el uso pedagógico del simulador.
...
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\fontsize{12}{14}\selectfont
\begin{tabular}[t]{>{\raggedright\arraybackslash}p{4cm}|>{\centering\arraybackslash}p{3cm}>{\raggedright\arraybackslash}p{9cm}}
\toprule
\multicolumn{1}{>{\centering\arraybackslash}p{4cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Aspecto evaluado}}} & \multicolumn{1}{>{\centering\arraybackslash}p{3cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Promedio}}} & \multicolumn{1}{>{\centering\arraybackslash}p{9cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Interpretación}}}\\
\midrule
...
La Tabla \ref{tab:tablaevaluaciondocentes} sintetiza la valoración realizada por los docentes sobre el material didáctico vinculado al simulador VonSim8, destacando los aspectos considerados más relevantes para la enseñanza. Esta presentación permite identificar los elementos que contribuyen a la calidad pedagógica y a la efectividad del recurso educativo.
...
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\fontsize{12}{14}\selectfont
\begin{tabular}[t]{>{\raggedright\arraybackslash}p{4cm}|>{\centering\arraybackslash}p{5.5cm}>{\raggedright\arraybackslash}p{5.5cm}}
\toprule
...
La Tabla \ref{tab:evasimuedu} presenta la evaluación realizada por los docentes sobre el simulador educativo VonSim8, destacando su impacto positivo en la enseñanza de arquitectura de computadoras. Los resultados evidencian que el simulador facilita la comprensión de conceptos complejos y aporta valor pedagógico, aunque se identifican oportunidades de mejora en aspectos técnicos y de interfaz.

\begin{table}[!h]
\centering
\caption{\label{tab:evasimuedu}Evaluación del simulador educativo}
\centering
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\fontsize{12}{14}\selectfont
\begin{tabular}[t]{>{\raggedright\arraybackslash}p{6cm}|>{\centering\arraybackslash}p{2cm}>{\raggedright\arraybackslash}p{8cm}}
...
\item
  \textbf{Evaluación empírica del impacto pedagógico}\\
