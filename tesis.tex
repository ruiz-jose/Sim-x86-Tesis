...
\hypertarget{implementaciuxf3n-del-ciclo-de-instrucciuxf3n-para-mov-al-bl-en-xdevs}{%
\section{Implementación del ciclo de instrucción para MOV AL, BL en xDEVS}\label{implementaciuxf3n-del-ciclo-de-instrucciuxf3n-para-mov-al-bl-en-xdevs}}

La instrucción MOV AL, BL realiza la transferencia del contenido del registro BL al registro AL:

En VonSim8, esta operación implica una lectura del registro fuente (BL) y una escritura en el registro destino (AL), coordinadas por la Unidad de Control (UC).
El ciclo completo se compone de dos fases generales, que se modelan como secuencia de eventos en el entorno xDEVS:

Captación (Fetch) --- común a todas las instrucciones.

Ejecución (Execute) --- específica del tipo MOV.

\hypertarget{fase-de-captaciuxf3n-fetch}{%
\subsection{Fase de Captación (Fetch)}\label{fase-de-captaciuxf3n-fetch}}

Ya definida en tu texto, se conserva igual:

MAR ← IP

MBR ← read(Mem{[}MAR{]}) \textbar{} IP ← IP + 1

IR ← MBR

Una vez completada esta fase, el registro IR contiene el código binario correspondiente a MOV AL, BL.
...
Para simular esta instrucción, intervienen los siguientes modelos Atómicos:
...
\hypertarget{flujo-de-eventos-y-microoperaciones-rtl}{%
\subsubsection{Flujo de eventos y microoperaciones (RTL)}\label{flujo-de-eventos-y-microoperaciones-rtl}}
...
\hypertarget{definiciuxf3n-devs-simplificada-de-cada-modelo}{%
\subsection{Definición DEVS simplificada de cada modelo}\label{definiciuxf3n-devs-simplificada-de-cada-modelo}}

\hypertarget{modelo-atuxf3mico-registro-ej.-al-o-bl}{%
\subsubsection{Modelo Atómico: Registro (ej. AL o BL)}\label{modelo-atuxf3mico-registro-ej.-al-o-bl}}
...
Durante la simulación en VonSim8 (siguiendo los tiempos y funciones DEVS):
...
El formalismo xDEVS permite representar de forma modular, jerárquica y temporizada el comportamiento del ciclo de instrucción.
Para MOV AL, BL, la simulación reproduce la dinámica de señales y transferencias de datos propias del nivel RTL, mostrando cómo la Unidad de Control coordina la comunicación entre registros mediante eventos discretos.
