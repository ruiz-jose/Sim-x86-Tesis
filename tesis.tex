...
\item
  ...
  \item
    ADD/SUB/CMP: \textbf{\texttt{MBR} \(\leftarrow\) \texttt{read(Memoria{[}MAR{]})}}
  \item
    ADD: \textbf{\texttt{write(Memoria{[}MAR{]})} \(\leftarrow\) \texttt{MBR} + \texttt{Ry}\textbar{} update(Flags)}
  \item
    SUB \texttt{{[}BL{]}}, \texttt{Ry}: \textbf{\texttt{write(Memoria{[}MAR{]})} \(\leftarrow\) \texttt{MBR} - \texttt{Ry} \textbar{} update(Flags)}
  \item
    CMP \texttt{{[}BL{]}}, \texttt{Ry}: \textbf{\texttt{MBR} - \texttt{Ry} \textbar{} update(Flags)}
  \end{enumerate}
