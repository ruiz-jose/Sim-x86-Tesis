...
La fase de captación tiene como objetivo obtener la instrucción almacenada en la memoria unificada, utilizando el valor del Instruction Pointer (IP) como dirección base, y transferirla al Instruction Register (IR) para su posterior decodificación. Este proceso marca el inicio del ciclo de instrucción y establece la secuencia temporal de acceso a memoria en la arquitectura Von Neumann.

En el simulador VonSim8, esta fase reproduce la restricción estructural propia de la arquitectura Von Neumann, al modelar el uso exclusivo y secuencial del bus y de la memoria compartida durante la captación de la instrucción.
...
Durante esta fase, el bus permanece bloqueado para otros accesos hasta que finaliza la lectura completa de la instrucción, garantizando la exclusividad del recurso compartido.
...
Los resultados de la simulación realizados en xdevs.py se presentan en el Anexo D (\ref{anexoD}), donde se incluyen las trazas temporales de los eventos generados durante la ejecución de la instrucción MOV AL, BL, evidenciando la coordinación correcta entre la UC, el bus y los registros involucrados.

\hypertarget{conclusiuxf3n}{%
\subsection{Conclusión}\label{conclusiuxf3n}}

El modelado en la plataforma xDEVS, implementado mediante xdevs.py, permite describir con precisión, modularidad y extensibilidad el comportamiento temporal de la instrucción MOV AL, BL en el simulador VonSim8.

La simulación refleja cómo la UC coordina los accesos secuenciales al bus y a la memoria compartida, respetando la restricción estructural característica de la arquitectura de Von Neumann.

Además, este enfoque posibilita instrumentar métricas de rendimiento ---como el número de ciclos por instrucción (CPI), la latencia media de memoria y el tiempo de retención del bus--- que sirven como base cuantitativa para la validación del simulador.
Desde una perspectiva pedagógica, el modelo facilita la comprensión de las dependencias funcionales entre los componentes internos del procesador y permite visualizar el impacto del bus compartido sobre el rendimiento global del sistema.
...
En este anexo se presentan los resultados obtenidos al simular el comportamiento del VonSim8 utilizando la herramienta xdevs.
Incluye el codigo y sus resultados.
...
class Register(Atomic):
    # Modelo atómico genérico para un registro de 8 bits.
