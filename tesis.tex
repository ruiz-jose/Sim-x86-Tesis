...
El espacio destinado al vector de interrupciones en el VonSim8 es de 8 posiciones de memoria desde \texttt{0x00} a \texttt{0x07} y solamente implementa las interrupciones por software:
...
Resaltado vector de interrupciones en memoria:
...
Se incorporó la opción de ampliación de fuente en el editor, así como ejemplos prácticos que permiten al estudiante probar y experimentar directamente con el simulador.

\begin{figure}

{\centering \includegraphics[width=0.85\linewidth]{images/editor} 

}

\caption{Resaltado vector de interrupciones y registro SP}\label{fig:editorvonsim}
\end{figure}
...
\addlinespace[10pt]
\textbf{Interrupciones implementadas} & Completo soporte de interrupciones por hardware y software, con PIC. & Solo interrupciones por software: `INT 0` (fin), `INT 6` (entrada de teclado), `INT 7` (salida de cadena).\\
\addlinespace[10pt]
\addlinespace
\textbf{Temporales de la ALU} & Registros temporales `left` y `rigth` visibles. & Eliminados, para simplificar la representación del ciclo de instrucción.\\
\addlinespace[10pt]
...
La ALU (\emph{Arithmetic Logic Unit}) permite realizar operaciones aritméticas y lógicas de 8 bits. Las operaciones disponibles son: \texttt{ADD}, \texttt{INC}, \texttt{SUB}, \texttt{DEC}, \texttt{NEG}, \texttt{NOT}, \texttt{AND}, \texttt{OR} y \texttt{XOR}. Todas estas operaciones modifican el registro \texttt{FLAGS}.
...
\begin{table}[!h]
\centering
\caption{\label{tab:tisaredu}Categorías de instrucciones y códigos de operación en VonSim8}
\centering
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\fontsize{10}{12}\selectfont
...
\item
  ...
  \item
    ...
    \item
      \textbf{\texttt{MBR} \(\leftarrow\) \texttt{read(Memoria{[}MAR{]})} \textbar{} \texttt{IP} \(\leftarrow\) \texttt{IP} + 1} -- Leer la dirección e incrementar IP.
    \item
      \textbf{\texttt{ri} \(\leftarrow\) \texttt{MBR} \textbar{} \texttt{SP} \(\leftarrow\) \texttt{SP} - 1}
    \item
      \textbf{\texttt{MAR} \(\leftarrow\) \texttt{SP} \textbar{} \texttt{MBR} \(\leftarrow\) \texttt{IP}}
    \item
      \textbf{\texttt{write(Memoria{[}MAR{]})} \(\leftarrow\) \texttt{MBR} \textbar{} \texttt{IP} \(\leftarrow\) \texttt{ri}}
    \end{enumerate}
  \item
    \textbf{RET}
    ...
    \tightlist
    \item
      \textbf{\texttt{MAR} \(\leftarrow\) \texttt{SP}}
    \item
      \textbf{\texttt{MBR} \(\leftarrow\) \texttt{read(Memoria{[}MAR{]})}}
    \item
      \textbf{\texttt{IP} \(\leftarrow\) \texttt{MBR}\textbar{} \texttt{SP} \(\leftarrow\) \texttt{SP} + 1}
    \end{enumerate}
  \end{itemize}
\item
  ...
  \item
    ...
    \item
      \textbf{\texttt{ri} \(\leftarrow\) \texttt{MBR} \textbar{} \texttt{SP} \(\leftarrow\) \texttt{SP} - 1}
    \item
      \textbf{\texttt{MAR} \(\leftarrow\) \texttt{SP} \textbar{} \texttt{MBR} \(\leftarrow\) \texttt{Flags}}
    \item
      \textbf{\texttt{write(Memoria{[}MAR{]})} \(\leftarrow\) \texttt{MBR} \textbar{} update(Flags I=0)}
    \item
      \textbf{\texttt{MBR} \(\leftarrow\) \texttt{IP} \textbar{} \texttt{SP} \(\leftarrow\) \texttt{SP} - 1}
    \item
      \textbf{\texttt{MAR} \(\leftarrow\) \texttt{SP}}
    \item
      \textbf{\texttt{write(Memoria{[}MAR{]})} \(\leftarrow\) \texttt{MBR} \textbar{} \texttt{IP} \(\leftarrow\) \texttt{ri}}
  ...
  \item
    ...
    \item
      \textbf{\texttt{MBR} \(\leftarrow\) \texttt{read(Memoria{[}MAR{]})}}
    \item
      \textbf{\texttt{Flags} \(\leftarrow\) \texttt{MBR} \textbar{} \texttt{SP} \(\leftarrow\) \texttt{SP} - 1}
    \end{enumerate}
  \item
    \textbf{CLI}
...
\item
  ...
  \item
    ...
    \item
      ...
      \tightlist
      \item
        \textbf{\texttt{MAR} \(\leftarrow\) \texttt{DL}} -- Dirección destino.
      \item
        \textbf{\texttt{MBR} \(\leftarrow\) \texttt{AL}} -- Contenido a escribir.
      \item
        \textbf{\texttt{write(E-S{[}MAR{]})} \(\leftarrow\) \texttt{MBR}}.
      \end{enumerate}
    \item
      ...
      \item
        \textbf{\texttt{MBR} \(\leftarrow\) \texttt{read(Memoria{[}MAR{]})} \textbar{} \texttt{IP} \(\leftarrow\) \texttt{IP} + 1} -- Leer el dato e incrementar IP.
      \item
        \textbf{\texttt{MAR} \(\leftarrow\) \texttt{MBR}} -- Dirección destino.
      \item
        \textbf{\texttt{MBR} \(\leftarrow\) \texttt{AL}} -- Contenido a escribir.
      \item
        \textbf{\texttt{write(E-S{[}MAR{]})} \(\leftarrow\) \texttt{MBR}}
  ...
  \item
    ...
    \item
      ...
      \item
        \textbf{\texttt{MAR} \(\leftarrow\) \texttt{DL}} -- Dirección destino.
      \item
        \textbf{\texttt{MBR} \(\leftarrow\) \texttt{read(E-S{[}MAR{]})}} -- Contenido a leer.
      \item
        \textbf{\texttt{AL} \(\leftarrow\) \texttt{MBR}}
      \end{enumerate}
    \item
...
\item
  ...
  \item
    ...
    \item
      \textbf{\texttt{MBR} \(\leftarrow\) \texttt{read(Memoria{[}MAR{]})}}
    \item
      \textbf{\texttt{Rx} \(\leftarrow\) \texttt{MBR}} \textbar{} \texttt{SP} \(\leftarrow\) \texttt{SP} + 1
    \end{enumerate}
  \item
    \textbf{PUSH Ry}
    ...
    \item
      \textbf{\texttt{MAR} \(\leftarrow\) \texttt{SP}}
    \item
      \textbf{\texttt{MBR} \(\leftarrow\) \texttt{Ry}} \textbar{} \texttt{SP} \(\leftarrow\) \texttt{SP} - 1
    \item
      \textbf{\texttt{write(Memoria{[}MAR{]})} \(\leftarrow\) \texttt{MBR}}
    \end{enumerate}
  \end{itemize}
...
- \textbf{INC \texttt{Rx}}
4 - \textbf{\texttt{Rx} \(\leftarrow\) \texttt{Rx} + 1 \textbar{} update(Flags)}
...
\item
  \textbf{NOT \texttt{Rx}}
  4 - \textbf{\texttt{Rx} \(\leftarrow\) NOT \texttt{Rx} \textbar{} update(Flags)}
\item
  \textbf{NEG \texttt{Rx}}
  4 - \textbf{\texttt{Rx} \(\leftarrow\) CA2 \texttt{Rx} \textbar{} update(Flags)}
\end{itemize}
