...
En las arquitecturas \textbf{RISC}, todas las instrucciones tienen la misma longitud, lo que simplifica la decodificación y mantiene un flujo continuo de instrucciones a través del pipeline. Esto no solo reduce la latencia, sino que también mejora la predictibilidad del rendimiento, esencial para arquitecturas escalables y eficientes. Además, el formato simplificado de \textbf{RISC} permite una mejor \textbf{utilización de la memoria caché}, dado que las instrucciones ocupan menos espacio y permiten un acceso más rápido.

Por el contrario, las arquitecturas \textbf{CISC}, como \textbf{x86}, utilizan un \textbf{formato de longitud variable} para ofrecer flexibilidad en las instrucciones. Esto permite a \textbf{CISC} proporcionar una gama más amplia de operaciones con menos líneas de código. Sin embargo, esta flexibilidad tiene un costo: las instrucciones de longitud variable requieren más tiempo para decodificarse y complican la implementación del pipeline, lo que puede afectar negativamente al rendimiento {[}\protect\hyperlink{ref-tanenbaum_structured_2013}{24}{]}.

Por ejemplo, en un procesador \textbf{x86}, la decodificación de instrucciones de longitud variable puede ser un cuello de botella, especialmente cuando el pipeline se interrumpe debido a errores de predicción de ramas. Aunque se pueden mitigar estos problemas mediante técnicas avanzadas como la predicción dinámica de saltos y el prefetching, el impacto en el rendimiento sigue siendo significativo.

En las arquitecturas \textbf{RISC} como \textbf{ARM} o \textbf{MIPS}, se priorizan los modos de direccionamiento simples, permitiendo un acceso más rápido a los operandos y reduciendo la latencia del pipeline {[}\protect\hyperlink{ref-stallings_computer_2013}{14}{]}. En contraste, los modos de direccionamiento en \textbf{CISC} como \textbf{x86} añaden flexibilidad a costa de un mayor tiempo de acceso. Este impacto puede mitigarse parcialmente mediante el uso de cachés y técnicas de prefetching, pero sigue siendo un factor clave en el rendimiento general del sistema.

\hypertarget{comparativa-entre-cisc-y-risc-1}{%
\subsection{Comparativa entre CISC y RISC}\label{comparativa-entre-cisc-y-risc-1}}
