...
\addlinespace[10pt]
\textbf{Gestión de interrupciones} & Simulación de asincronía real\\
\addlinespace[10pt]
\bottomrule
\end{tabular}}
\end{table}
...
Considerando los contenidos y objetivos pedagógicos de la asignatura Arquitectura de Computadoras, se desarrolló VonSim8\footnote{VonSim8: \url{https://ruiz-jose.github.io/VonSim8/}} un simulador concebido en este trabajo como adaptación de VonSim, pero adaptado para operar con registros y memoria de 8 bits, con una interfaz simplificada orientada al aprendizaje progresivo.
...
\addlinespace[10pt]
\textbf{Tamaño de registros} & 8 y 16 bis & 8 bits\\
\addlinespace[10pt]
...
Esta sección describe la estructura del simulador VonSim8, el diseño de los registros fue concebido para facilitar la comprensión de los modos de direccionamiento y del ciclo de instrucción, ambos considerados fundamentos clave en el estudio de la Arquitectura de Computadoras \protect\hyperlink{ref-stallings_computer_2021}{{[}14{]}}. Se describe en la siguiente tabla \ref{tab:estructuravonsim8} los componentes principales del simulador, junto con sus características y funcionalidades específicas. Esta tabla proporciona una visión general de la arquitectura del simulador, destacando los elementos clave que componen su estructura y su propósito pedagógico.:
...
En base a las entrevistas con los docentes y el análisis de los contenidos del curso, se definieron las siguientes categorías de instrucciones:

\begin{table}[!h]
\centering
\caption{\label{tab:ISAdidacticos}Tipo de instrucción y uso pedagógicos esperados}
\centering
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\fontsize{10}{12}\selectfont
\begin{tabular}[t]{>{\centering\arraybackslash}p{6cm}|>{\centering\arraybackslash}p{9cm}}
\toprule
\multicolumn{1}{>{\centering\arraybackslash}p{6cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Tipo de instruccion}}} & \multicolumn{1}{>{\centering\arraybackslash}p{9cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Uso pedagógico esperado}}}\\
\midrule
\textbf{Transferencia y procesamiento de datos} & Instrucciones que permiten mover datos entre registros y memoria, realizar operaciones aritméticas, y manipular el contenido de los registros. Estas instrucciones son fundamentales para la comprensión del flujo de datos en una arquitectura computacional\\
\addlinespace[10pt]
\textbf{Control de flujo} & Instrucciones que permiten alterar el flujo de ejecución del programa mediante saltos condicionales e incondicionales, así como la detención del procesador. Estas instrucciones son esenciales para comprender cómo se controlan las decisiones y el flujo de ejecución en un programa.\\
\addlinespace[10pt]
\bottomrule
\end{tabular}}
\end{table}
...
\centering
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\begin{tabular}[t]{>{\raggedright\arraybackslash}p{3cm}>{\raggedright\arraybackslash}p{8cm}>{\raggedright\arraybackslash}p{8cm}}
\toprule
Tipo & Operandos & Ejemplo\\
\midrule
Transferencia y procesamiento & 2 operandos (fuente y destino) & MOV operando-destino, operando-fuente\\
Control & Con 1 o sin operando & JMP operando o HLT\\
\bottomrule
\end{tabular}}
\end{table}
...
\addlinespace[8pt]
\textbf{Transferencia y procesamiento} & B: Cargar a registro & Operando destino registro Rx y operando fuente puede ser:\begin{itemize} \item dirección [M]\item dirección registro [BL] \item valor en la instrucción d \end{itemize}\\
\addlinespace[8pt]
\textbf{} & C: Almacenar en memoria & Operando destino puede ser: \begin{itemize} \item dirección [M]\item dirección registro [BL] \end{itemize} y operando fuente puede ser:\begin{itemize} \item Registro: Ry\item valor en la instrucción d \end{itemize}\\
\addlinespace[8pt]
...
A: Entre registros & MOV Rx, Ry & Rx ← Ry\\
 & ADD Rx, Ry & Rx ← Rx + Ry\\
 & SUB Rx, Ry & Rx ← Rx - Ry\\
 & CMP Rx, Ry & Rx - Ry: no almacena resultado en operando destino\\
...
\addlinespace
 ...
 & SUB [M], Ry & B: Cargar a registro\\
\addlinespace
 & SUB [M], --` & C: Almacenar en memoria\\
 & CMP [M], -- & B: Cargar a registro\\
 &  & C: Almacenar en \vphantom{1} memoria\\
 &  & B: Cargar a registro\\
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.8194}}@{}}
\toprule\noalign{}
...
01 & 1 & Indirecto utiliza como operando implícito el registro \texttt{BL} \\
...
\bottomrule\noalign{}
\endlastfoot
00YY & 2 & directo \texttt{M} = Dirección de memoria, \texttt{YY} = Registro fuente \\
