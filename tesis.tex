...
\toprule
\multicolumn{1}{>{\centering\arraybackslash}p{2cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Paso}}} & \multicolumn{1}{>{\centering\arraybackslash}p{5cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Microoperación}}} & \multicolumn{1}{>{\centering\arraybackslash}p{8cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Descripción}}} & \multicolumn{1}{>{\centering\arraybackslash}p{8cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Modelos y funciones DEVS}}}\\
\midrule
...
\addlinespace[8pt]
\textbf{2} & UC → BL.enable\_out = 1 & La UC activa la señal del muxregout para leer el registro fuente BL. & $\lambda$ (UC) genera evento de salida hacia BL.\\
\addlinespace[8pt]
\textbf{3} & BUS ← BL & La UC activa la señal del muxregin para leer el registro destino AL. & $\lambda$ (BL) emite valor actual al BUS.\\
\addlinespace[8pt]
\textbf{4} & UC → AL.enable\_in = 1 & La UC habilita la escritura en AL. & $\lambda$ (UC) envía evento de control a AL.\\
\addlinespace[8pt]
\textbf{5} & AL ← BUS & El registro AL captura el dato del bus y actualiza su estado interno. & $\delta_{\text{ext}}$ (AL) recibe valor del BUS y actualiza su estado S.\\
\addlinespace[8pt]
\bottomrule
\end{tabular}}
\end{table}
...
\item
  \(\tau\):\\
  Duración del micropaso (instantánea o tiempo simbólico \(t_{\text{reg}}\)).
\end{itemize}
...
De esta forma, la secuencia de microoperaciones RTL se traduce directamente al formalismo DEVS.
...
Durante la simulación en VonSim8, los tiempos y funciones DEVS determinan la secuencia de eventos y el intercambio de datos entre modelos:
...
El formalismo xDEVS permite representar con precisión el comportamiento modular, jerárquico y temporizado del ciclo de instrucción MOV AL, BL.
La simulación reproduce la dinámica de señales y las transferencias de datos propias del nivel RTL (Register Transfer Level), evidenciando cómo la Unidad de Control coordina la comunicación entre los registros mediante eventos discretos y funciones de transición bien definidas.
