...
\subsection{xDEVS en la enseñanza de la Arquitectura de Computadoras}\label{xdevs-en-la-enseuxf1anza-de-la-arquitectura-de-computadoras}}
...
\hypertarget{implementaciuxf3n-del-ciclo-de-instrucciuxf3n-mov-al-bl-mediante-el-formalismo-xdevs}{%
\section{Implementación del ciclo de instrucción MOV AL, BL mediante el Formalismo xDEVS}\label{implementaciuxf3n-del-ciclo-de-instrucciuxf3n-mov-al-bl-mediante-el-formalismo-xdevs}}

La simulación de la instrucción \textbf{MOV AL, BL} es esencial para validar la arquitectura de VonSim8. Esta instrucción de transferencia de registro a registro, que copia el contenido del registro fuente BL al registro destino AL, se modela como una secuencia de eventos discretos y temporizados que representan el funcionamiento a Nivel de Transferencia de Registros (RTL).

El formalismo xDEVS (eXtended Discrete Event System Specification) se emplea para definir rigurosamente la estructura modular y la dinámica de eventos del simulador. Cada componente del microprocesador (registros, UC) se representa como un modelo atómico, y su interacción mediante el bus se define en un modelo acoplado jerárquico.
...
El ciclo completo se descompone en estados bien definidos de la Unidad de Control (UC), que actúan como transiciones internas (\(\delta_{\text{int}}\)) en el formalismo \(\text{xDEVS}\).

\hypertarget{fase-de-captaciuxf3n-fetch}{%
\subsection{Fase de Captación (Fetch)}\label{fase-de-captaciuxf3n-fetch}}

Esta fase, común a cualquier instrucción, se conserva tal como se detalla en la definición general del ciclo de instrucción:
...
Al finalizar, el registro \textbf{IR} contiene el código binario correspondiente a la instrucción \texttt{MOV\ AL,\ BL}.
...
\hypertarget{modelos-atuxf3micos-y-sus-interacciones}{%
\subsubsection{Modelos Atómicos y sus Interacciones}\label{modelos-atuxf3micos-y-sus-interacciones}}
...
\begin{table}[!h]
\centering
\caption{\label{tab:componentesprincipales}Componentes principales del simulador VonSim8}
\centering
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\fontsize{10}{12}\selectfont
\begin{tabular}[t]{>{\raggedright\arraybackslash}p{6cm}|>{\raggedright\arraybackslash}p{4cm}>{\raggedright\arraybackslash}p{8cm}}
\toprule
\multicolumn{1}{>{\centering\arraybackslash}p{6cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Componente}}} & \multicolumn{1}{>{\centering\arraybackslash}p{4cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Tipo}}} & \multicolumn{1}{>{\centering\arraybackslash}p{8cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Descripción}}}\\
\midrule
\textbf{UC (Unidad de Control)} & Atómico & Motor del simulador. Decodifica IR, genera señales de control para lectura/escritura.\\
\addlinespace[8pt]
\textbf{BL (Registro Fuente)} & Atómico (Registro) & Emite su valor al BUS tras recibir la señal de control (evento) de la UC.\\
\addlinespace[8pt]
\textbf{AL (Registro Destino)} & Atómico (Registro) & Captura y actualiza su estado interno con el dato del BUS tras recibir la señal de control de la UC.\\
\addlinespace[8pt]
\textbf{BUS de datos} & Modelo Acoplado (Canal) & Representa el medio de comunicación. Su estado transfiere el dato desde la salida $\lambda$ de BL hasta la entrada $\delta_{\text{ext}}$ de AL.\\
\addlinespace[8pt]
\bottomrule
\end{tabular}}
\end{table}
...
\hypertarget{secuencia-de-microoperaciones-y-eventos-xdevs}{%
\subsubsection{Secuencia de Microoperaciones y Eventos xDEVS}\label{secuencia-de-microoperaciones-y-eventos-xdevs}}
...
\begin{table}[!h]
\centering
\caption{\label{tab:microoperacionesMOV}Secuencia de microoperaciones para MOV AL, BL en VonSim8}
\centering
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\fontsize{10}{12}\selectfont
\begin{tabular}[t]{>{\raggedright\arraybackslash}p{2cm}|>{\raggedright\arraybackslash}p{5cm}>{\raggedright\arraybackslash}p{8cm}>{\raggedright\arraybackslash}p{8cm}}
\toprule
\multicolumn{1}{>{\centering\arraybackslash}p{2cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Paso}}} & \multicolumn{1}{>{\centering\arraybackslash}p{5cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Microoperación}}} & \multicolumn{1}{>{\centering\arraybackslash}p{8cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Descripción}}} & \multicolumn{1}{>{\centering\arraybackslash}p{8cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Modelos y funciones xDEVS}}}\\
\midrule
\textbf{1} & UC: decodificar(IR) & La UC lee el valor del IR, determina el opcode $ ext{MOV}$ y los operandos. & $\delta_{\text{ext}}$ (UC) recibe evento del IR; actualiza su estado (S) con la instrucción actual.\\
\addlinespace[8pt]
\textbf{2} & UC → BL.enable\_out = 1 & La UC activa la señal de lectura del registro fuente BL. & $\lambda$ (UC) genera evento de salida hacia BL.\\
\addlinespace[8pt]
\textbf{3} & BUS ← BL & El registro BL (modelo atómico) emite su valor al BUS. & $\lambda$ (BL) emite valor actual al BUS.\\
\addlinespace[8pt]
\textbf{4} & UC → AL.enable\_in = 1 & La UC habilita la escritura en el registro destino AL. & $\lambda$ (UC) envía evento de control a AL.\\
\addlinespace[8pt]
\textbf{5} & AL ← BUS & El registro AL captura el dato del bus y actualiza su valor. & $\delta_{\text{ext}}$ (AL) recibe valor del BUS y actualiza su estado S.\\
\addlinespace[8pt]
\bottomrule
\end{tabular}}
\end{table}
...
\hypertarget{definiciuxf3n-xdevs-simplificada-de-cada-modelo}{%
\subsection{Definición XDEVS simplificada de cada modelo}\label{definiciuxf3n-xdevs-simplificada-de-cada-modelo}}

\hypertarget{modelo-atuxf3mico-registro-reg}{%
\subsubsection{Modelo Atómico: Registro (REG)}\label{modelo-atuxf3mico-registro-reg}}

La definición formal del modelo atómico de registro se presenta en la ecuación \eqref{eq:modreg}:
...
\hypertarget{modelo-atuxf3mico-unidad-de-control-uc}{%
\subsubsection{Modelo Atómico: Unidad de Control (UC)}\label{modelo-atuxf3mico-unidad-de-control-uc}}

La definición formal del modelo atómico de la unidad de control se presenta en la ecuación \eqref{eq:moduc}:
...
\hypertarget{diagrama-de-interacciuxf3n-resumen-conceptual}{%
\subsection{Diagrama de interacción (resumen conceptual)}\label{diagrama-de-interacciuxf3n-resumen-conceptual}}

En el entorno \textbf{xDEVS}, cada una de estas microoperaciones se modela como una \textbf{transición de estado} o un \textbf{evento} entre modelos atómicos.\\
Por ejemplo, la operación \texttt{MOV\ AL,\ BL} se descompone en:
...
\item
  \(\lambda(UC)\) → activa \texttt{BL.enable\_out}.\\
\item
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.4297}}
...
Microoperación RTL
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Evento o transición xDEVS
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Modelo atómico involucrado
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
Fetch 1 & \texttt{MAR\ ←\ IP} & \(\delta_{\text{ext(UC)}}\) activa señal hacia MAR & UC, MAR, IP \\
Fetch 2 & \texttt{MBR\ ←\ read(Mem{[}MAR{]})} / \texttt{IP\ ←\ IP\ +\ 1} & \(\lambda_{\text{(Mem)}}\) emite dato → \(\delta_{\text{ext(MBR)}}\) lo recibe & Memoria, MBR, UC \\
Fetch 3 & \texttt{IR\ ←\ MBR} & \(\lambda(MBR)\) → \(\delta_{\text{ext}}(IR)\) & MBR, IR \\
Execute 1 & \texttt{AL\ ←\ BL} & \(\lambda(UC)\) genera señal de control → \(\lambda(BL)\) → \(\delta_{\text{ext}}(AL)\) & UC, BL, BUS, AL \\
\end{longtable}

IR → UC → {[} señales de control {]}
↓
+------------+
\textbar{} BL \textbar{} -- \(\lambda\) --\textgreater{} (dato) --\textgreater{} BUS --\textgreater{} (dato) --\textgreater{} AL
+------------+

\emph{Nota:} La UC garantiza el acceso \textbf{mutuamente exclusivo} al bus de datos, activando las señales \texttt{enable\_out} y \texttt{enable\_in}
...
\hypertarget{resultados-de-simulaciuxf3n-esperados-en-vonsim8}{%
\subsection{Resultados de simulación esperados (en VonSim8)}\label{resultados-de-simulaciuxf3n-esperados-en-vonsim8}}
...
\begin{table}[!h]
\centering
\caption{\label{tab:cicloMOV}Ciclo de ejecución de la instrucción MOV AL, BL}
\centering
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\fontsize{10}{12}\selectfont
\begin{tabular}[t]{>{\raggedright\arraybackslash}p{3cm}|>{\raggedright\arraybackslash}p{5cm}>{\raggedright\arraybackslash}p{4cm}>{\raggedright\arraybackslash}p{8cm}}
\toprule
\multicolumn{1}{>{\centering\arraybackslash}p{3cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Tiempo (t)}}} & \multicolumn{1}{>{\centering\arraybackslash}p{5cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Evento}}} & \multicolumn{1}{>{\centering\arraybackslash}p{4cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Registro afectado}}} & \multicolumn{1}{>{\centering\arraybackslash}p{8cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Acción}}}\\
\midrule
\textbf{$t_{\text{0}}$} & Captación completada & IR & Contiene MOV AL, BL\\
\addlinespace[8pt]
\textbf{$t_{\text{1}}$} & Decodificación & UC & Genera señales de control\\
\addlinespace[8pt]
\textbf{$t_{\text{2}}$} & Lectura de BL & BL & Emite valor al bus\\
\addlinespace[8pt]
\textbf{$t_{\text{3}}$} & Escritura en AL & AL & Guarda el valor de BL\\
\addlinespace[8pt]
\textbf{$t_{\text{4}}$} & Fin de instrucción & UC & Pasa al siguiente ciclo (fetch siguiente instrucción)\\
\addlinespace[8pt]
\bottomrule
\end{tabular}}
\end{table}
