...
Se elimino el uso de los registros temporales de la ALU (\texttt{left}, \texttt{right} y \texttt{result}).
...
Entre los registros específicos, no accesibles directamente por el programador, se encuentran: \texttt{SP} (\emph{Stack Pointer}), utilizado para la gestión de la pila; el registro \texttt{Flags} (\emph{flags register}); el \texttt{IP} (\emph{Instruction Pointer}), que contiene la dirección de la próxima instrucción a ejecutar; y el \texttt{IR} (\emph{Instruction Register}), que almacena el byte de la instrucción actualmente en proceso de decodificación y ejecución. Además, existen dos registros dedicados a la transferencia de información entre la CPU y la memoria principal: el \texttt{MAR} (\emph{Memory Address Register}) encargado de almacenar direcciones de memoria, y el \texttt{MBR} (\emph{Memory Buffer Register}) que almacena el byte que se quiere propagar o se ha recibido por el bus de datos.
...
El registro \texttt{Flags} (banderas de estado) es un registro de 8 bits que contiene las \emph{flags} mostradas en la siguiente tabla. Este registro no es directamente accesible por el programador, pero puede ser modificado por las operaciones de la ALU y pueden realizarse saltos condicionales en base a sus valores.
...
El simulador permite configurar la conexión de diversos módulos de entrada/salida y otros dispositivos al bus principal. Estos módulos se agrupan en las siguientes categorías:
...
\item
  Teclado y pantalla.
\item
  Un PIO, que puede conectarse a LEDs e interruptores.
\item
  Un PIC, que interactúa con la tecla F10 para generar interrupciones, junto con un temporizador (timer) y su reloj.
\item
  Handshake, que puede conectarse tanto a un PIO como a un PIC.
\end{itemize}

La Figura \ref{fig:dispositivos} presenta una visión de los dispositivos que pueden conectarse al simulador.
...
\hypertarget{ejemplo-de-uso-del-muxf3dulo-handshake}{%
\subsubsection{Ejemplo de uso del módulo Handshake}\label{ejemplo-de-uso-del-muxf3dulo-handshake}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{; Imprime el string "Hola" en la impresora usando sondeo}

\NormalTok{dato DB "Hola", 0         ; String a imprimir, terminado en 0 (carácter nulo)}

\NormalTok{HS\_DATA   EQU 40h         ; Dirección del registro de datos del Handshake}
\NormalTok{HS\_STATUS EQU 41h         ; Dirección del registro de estado del Handshake}

\NormalTok{; {-}{-}{-} Deshabilita las interrupciones del Handshake (bit 7 en 0) {-}{-}{-}}
\NormalTok{IN  AL, HS\_STATUS}
\NormalTok{AND AL, 01111111b         ; Fuerza el bit 7 a 0 (sin interrupciones)}
\NormalTok{OUT HS\_STATUS, AL}

\NormalTok{; {-}{-}{-} Inicializa el puntero al string {-}{-}{-}}
\NormalTok{MOV BL, OFFSET dato       ; BL apunta al primer carácter del string}

\NormalTok{; {-}{-}{-} Bucle principal: espera espacio en el buffer e imprime {-}{-}{-}}
\NormalTok{Sondeo:}
\NormalTok{    IN  AL, HS\_STATUS}
\NormalTok{    AND AL, 00000001b     ; Lee el flag busy (bit 0): 1=lleno, 0=libre}
\NormalTok{    JZ  ImprimirCadena   ; Si busy=0, hay espacio y puede imprimir}
\NormalTok{    JMP Sondeo            ; Si busy=1, espera hasta que haya espacio}

\NormalTok{ImprimirCadena:}
\NormalTok{    MOV AL, [BL]          ; Carga el siguiente carácter del string}
\NormalTok{    CMP AL, 0             ; ¿Es el final del string? (carácter nulo)}
\NormalTok{    JZ fin                ; Si sí, termina el programa}

\NormalTok{    OUT HS\_DATA, AL       ; Envía el carácter al registro de datos del Handshake}

\NormalTok{    INC BL                ; Avanza al siguiente carácter del string}
\NormalTok{    JMP Sondeo            ; Repite el proceso para el próximo carácter}

\NormalTok{fin:}
\NormalTok{    HLT   ; Detiene la ejecución}
\end{Highlighting}
\end{Shaded}
...
El PIC dispone de 8 líneas de interrupción, denominadas INT0 a INT7 (aunque no todas son utilizadas). Cada línea está asociada a un registro de 8 bits en la memoria de E/S. Por ejemplo, la línea INT0 corresponde a la dirección 24h, la línea INT1 a la dirección 25h, y así sucesivamente hasta la línea INT7, que utiliza la dirección 2Bh. En estos registros se almacena el número de interrupción asociado a cada línea.
...
\bottomrule\noalign{}
\endlastfoot
\texttt{INT0} & \protect\hyperlink{tecla-f10}{Tecla F10} \\
\texttt{INT1} & \protect\hyperlink{timer}{Timer} \\
\texttt{INT2} & {[}Handshake{]} \\
\texttt{INT3} & -- \\
\texttt{INT4} & -- \\
\texttt{INT5} & -- \\
\texttt{INT6} & -- \\
...
El PIC cuenta con tres registros adicionales para gestionar las interrupciones. Cada bit de estos registros corresponde a una línea de interrupción, donde el bit menos significativo representa la línea da bit corresponde con una línea de interrupción: el bit menos significativo corresponde a la línea y el más significativo la línea \texttt{INT7}.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{; Programa: Imprime "Hola" a los 3 segundos de iniciado y luego termina.}
\NormalTok{; Utiliza la interrupción del TIMER (ID = 5).}

\NormalTok{org 10h}
\NormalTok{mensaje db "hola"        ; Mensaje a imprimir}
\NormalTok{imprimio db 0            ; Flag para saber si ya imprimió}

\NormalTok{; Definición de direcciones de registros de dispositivos}
\NormalTok{CONT equ 10h             ; Registro de conteo del timer}
\NormalTok{COMP equ 11h             ; Registro de comparación del timer}

\NormalTok{EOI  equ 20h             ; End Of Interrupt (para PIC)}
\NormalTok{IMR  equ 21h             ; Interrupt Mask Register (PIC)}
\NormalTok{INT1 equ 25h             ; Registro de vector de interrupción 1}

\NormalTok{org 20h                  ; Comienzo del código principal}

\NormalTok{; {-}{-}{-} Habilitar interrupciones del timer {-}{-}{-}}
\NormalTok{; IMR = 1111 1101b (solo habilita interrupciones del timer y teclado)}
\NormalTok{mov al, 11111101b        ; Habilita interrupciones del timer (bit 1 en 0)}
\NormalTok{out IMR, al}

\NormalTok{; {-}{-}{-} Configurar vector de interrupción del timer {-}{-}{-}}
\NormalTok{mov al, 5                ; ID de interrupción del timer}
\NormalTok{out INT1, al             ; Asigna rutina de atención a la posición 5}

\NormalTok{; {-}{-}{-} Instalar rutina de interrupción en el vector {-}{-}{-}}
\NormalTok{mov bl, 5                ; Vector de interrupción 5}
\NormalTok{mov [bl], imp\_msj        ; Apunta a la rutina imp\_msj}

\NormalTok{; {-}{-}{-} Configurar timer para 3 segundos {-}{-}{-}}
\NormalTok{mov al, 3                ; Valor de comparación (3 segundos)}
\NormalTok{out COMP, al}

\NormalTok{mov al, 0                ; Reinicia el contador del timer}
\NormalTok{out CONT, al}

\NormalTok{; {-}{-}{-} Esperar a que se imprima el mensaje {-}{-}{-}}
\NormalTok{loopinf: cmp imprimio, 0 ; ¿Ya imprimió?}
\NormalTok{         jz loopinf      ; Si no, sigue esperando}

\NormalTok{hlt                      ; Termina el programa}

\NormalTok{; {-}{-}{-} Rutina de interrupción del timer {-}{-}{-}}
\NormalTok{org 50h}
\NormalTok{imp\_msj:}
\NormalTok{         mov bl, offset mensaje ; Dirección del mensaje}
\NormalTok{         mov al, 4             ; Servicio de impresión}
\NormalTok{         int 7                 ; Llama a la interrupción de impresión}
\NormalTok{         mov imprimio, 1       ; Marca que ya imprimió}
\NormalTok{         mov al, 20h           ; Señal de fin de interrupción}
\NormalTok{         out EOI, al           ; Notifica al PIC}
\NormalTok{         iret                  ; Retorna de la interrupción}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{; Programa: Contador de pulsaciones de la tecla F10 usando interrupciones}

\NormalTok{;{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{; 1) Definiciones y variables}
\NormalTok{;{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}

\NormalTok{org 10h                ; Dirección de inicio de datos}
\NormalTok{cantidad db 0          ; Variable: almacena la cantidad de veces que se presionó F10}

\NormalTok{ID   EQU 1             ; ID de la interrupción para F10 (puede ser 0{-}7)}
\NormalTok{IMR  EQU 21h           ; Dirección del registro IMR (máscara de interrupciones)}
\NormalTok{EOI  EQU 20h           ; Dirección para enviar End Of Interrupt al PIC}
\NormalTok{INT0 EQU 24h           ; Dirección para configurar la línea INT0 (F10)}

\NormalTok{;{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{; 2) Inicialización del PIC y vector de interrupción}
\NormalTok{;{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}

\NormalTok{org 20h                ; Dirección de inicio del código principal}

\NormalTok{; 2.1) Habilitar solo la interrupción de F10 (INT0)}
\NormalTok{mov al, 11111110b      ; Habilita solo INT0 (bit 0 en 0), el resto deshabilitado}
\NormalTok{out IMR, al}

\NormalTok{; 2.2) Configurar el ID de la interrupción para INT0}
\NormalTok{mov al, ID             ; Cargar el ID elegido para F10}
\NormalTok{out INT0, al}

\NormalTok{; 2.3) Asociar el vector de interrupción con la subrutina atenderf10}
\NormalTok{mov bl, ID             ; BL = ID de la interrupción}
\NormalTok{mov [bl], atenderf10   ; Vector de interrupción: dirección de la rutina}

\NormalTok{;{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{; 3) Bucle principal (espera activa)}
\NormalTok{;{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}

\NormalTok{loop: jmp loop         ; Espera indefinida (el programa queda esperando interrupciones)}

\NormalTok{hlt                    ; (Opcional) Detiene la CPU si sale del bucle}

\NormalTok{;{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{; 4) Rutina de atención de la interrupción F10}
\NormalTok{;{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}

\NormalTok{org 50h                ; Dirección de la subrutina de atención}

\NormalTok{atenderf10:}
\NormalTok{    inc cantidad       ; Incrementa el contador cada vez que se presiona F10}
\NormalTok{    mov al, 20h        ; Código de End Of Interrupt (EOI)}
\NormalTok{    out EOI, al        ; Notifica al PIC que terminó la atención}
\NormalTok{    iret               ; Retorna de la interrupción}
\end{Highlighting}
\end{Shaded}
