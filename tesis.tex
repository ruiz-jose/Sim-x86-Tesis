...
\begin{itemize}
\item
  ALU (Arithmetic Logic Unit): Unidad aritmético-lógica. Componente del procesador encargado de realizar operaciones aritméticas y lógicas sobre los datos.
\item
  CPU (Central Processing Unit): Unidad central de procesamiento. Es el componente principal de una computadora, responsable de ejecutar instrucciones y coordinar el funcionamiento de los demás componentes.
\item
  CPI (Cycles Per Instruction): Ciclos por instrucción. Métrica que indica el número promedio de ciclos de reloj requeridos para ejecutar una instrucción.
\item
  DMA (Direct Memory Access): Acceso directo a memoria. Mecanismo que permite transferir datos entre la memoria y dispositivos periféricos sin intervención directa de la CPU.
\item
  E/S (Entrada/Salida): Conjunto de operaciones y dispositivos que permiten la comunicación entre la computadora y el entorno externo.
\item
  ISA (Instruction Set Architecture): Arquitectura del conjunto de instrucciones. Define la interfaz entre el hardware y el software, especificando las operaciones que puede ejecutar el procesador.
\item
  PIC (Programmable Interrupt Controller): Controlador programable de interrupciones. Dispositivo que gestiona y prioriza las señales de interrupción recibidas por la CPU.
\item
  PIO (Programmed Input/Output): Entrada/salida programada. Método de transferencia de datos entre la CPU y los periféricos mediante instrucciones explícitas de programa.
\item
...
\item
  RISC (Reduced Instruction Set Computing): Computación con conjunto reducido de instrucciones. Filosofía de diseño de procesadores que prioriza instrucciones simples y de longitud fija.
\item
  CISC (Complex Instruction Set Computing): Computación con conjunto complejo de instrucciones. Filosofía de diseño de procesadores que incluye instrucciones multifuncionales y de longitud variable.
\item
  Von Neumann: Arquitectura de computadoras en la que datos e instrucciones comparten la misma memoria y bus de comunicación.
\item
  Harvard: Arquitectura de computadoras que separa físicamente la memoria y los buses para datos e instrucciones.
\item
...
\item
  RISC-V: Arquitectura de procesadores de código abierto basada en el paradigma RISC, destacada por su flexibilidad y modularidad.
\item
...
\item
  DEVS (Discrete Event System Specification): Formalismo para modelar sistemas de eventos discretos, utilizado en simulación y análisis de sistemas complejos.
\item
  Pipeline: Técnica de procesamiento que permite ejecutar múltiples instrucciones de manera solapada, mejorando el rendimiento del procesador.
\item
  Memoria caché: Memoria de alta velocidad utilizada para almacenar temporalmente datos e instrucciones frecuentemente utilizados, optimizando el acceso a la memoria principal.
\item
  Bus: Canal de comunicación que permite la transferencia de datos entre los diferentes componentes de la computadora.
\item
...
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\fontsize{10}{12}\selectfont
\begin{tabular}[t]{>{\raggedright\arraybackslash}p{4cm}|>{\raggedright\arraybackslash}p{4cm}>{\raggedright\arraybackslash}p{4cm}l}
\toprule
\multicolumn{1}{>{\centering\arraybackslash}p{4cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{Característica}}} & \multicolumn{1}{>{\centering\arraybackslash}p{4cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{x86}}} & \multicolumn{1}{>{\centering\arraybackslash}p{4cm}}{\cellcolor[HTML]{D3D3D3}{\textbf{ARM}}} & \multicolumn{1}{c}{\cellcolor[HTML]{D3D3D3}{\textbf{RISC-V}}}\\
\midrule
\textbf{Eficiencia Energética} & Moderada & Alta & Alta\\
\addlinespace[10pt]
\textbf{Complejidad ISA} & Alta & Baja & Baja\\
\addlinespace[10pt]
\textbf{Rendimiento} & Alto & Moderado & Moderado\\
\addlinespace[10pt]
\textbf{Áreas de Aplicación} & Escritorio, servidores & Dispositivos móviles & Investigación, embebidos\\
\addlinespace[10pt]
\bottomrule
\end{tabular}}
\end{table}
