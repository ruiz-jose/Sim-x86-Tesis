...
```{r aplicacionessimulacion, echo=FALSE, message=FALSE}
knitr::kable(
  data.frame(
...
Un modelo atómico DEVS se define formalmente como una séxtupla:
...
- \(\delta_{ext}: Q \times X \rightarrow S\) es la función de transición externa, con \(Q = \{ (s, e) | s \in S, 0 \leq e \leq ta(s) \}\)
- \(\lambda: S \rightarrow Y\) es la función de salida
- \(ta: S \rightarrow \mathbb{R}^+_0 \cup \{\infty\}\) es la función de tiempo de permanencia
...
La siguiente figura \@ref(fig:ensambla) ilustra la organización modular del formalismo DEVS, mostrando cómo se integran modelos atómicos dentro de modelos acoplados:
