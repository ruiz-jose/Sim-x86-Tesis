...
kable(criterios, format = "latex", booktabs = TRUE, caption = "Criterios e indicadores de evaluación de simuladores") %>%
...
simuladores <- data.frame(
  ...
  `Exploración Previa` = c("1", "1", "1", "1", "1", "1", "1", "1"),
  `Evaluación Final` = c("0", "1", "0", "0", "1", "1", "0", "0")
)

# Generar la tabla con formato LaTeX o HTML según el tipo de salida
kable(simuladores, format = ifelse(knitr::is_latex_output(), "latex", "html"),
      caption = "Proceso de selección de simuladores",
      booktabs = TRUE, escape = FALSE) %>%
  kable_styling(
    latex_options = c("hold_position", "scale_down"), # Mantén la tabla en su posición y ajusta el tamaño
    full_width = FALSE, # Evita que la tabla ocupe todo el ancho
    position = "center" # Centra la tabla
  ) %>%
  column_spec(1, bold = TRUE, border_right = TRUE) %>% # Negrita en la primera columna
  column_spec(2:3, width = "4cm") %>% # Ajusta el ancho de las columnas
  row_spec(0, bold = TRUE, align = "c", background = "#D3D3D3") # Estilo para el encabezado
```
