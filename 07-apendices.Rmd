...
Este anexo documenta la implementación del modelo DEVS del simulador VonSim8 desarrollado con la biblioteca xdevs.py (Python), así como los resultados experimentales obtenidos durante la ejecución de la instrucción MOV AL, BL. El objetivo es proporcionar evidencia empírica de la validación del modelo teórico presentado en el Capítulo 5, sección "Modelado con xDEVS del ciclo de instrucción MOV AL, BL".

Este código está disponible en el repositorio xdevs-vonim8^[xdevs-vonim8: https://github.com/ruiz-jose/xdevs-vonsim8].
...
class Register(Atomic):
    ...
    def lambdaf(self):
        if self.pending_read:
            print(f"  [{self.name}] lambda: Emitiendo dato={self.value:02X} por data_out")
...
class InstructionPointer(Atomic):
    ...
    def lambdaf(self):
        if self.pending_output:
            print(f"  [IP] lambda: Emitiendo dirección IP={self.value:02X} por addr_out")
...
class MemoryAddressRegister(Atomic):
    ...
    def deltint(self):
        if self.pending_addr is not None:
            ...
            print(f"  [MAR] delta_int: Dirección almacenada. MAR={self.address:02X}")
            self.pending_addr = None
        self.passivate()
    
    def lambdaf(self):
        if self.pending_addr is not None:
            print(f"  [MAR] lambda: Emitiendo dirección MAR={self.pending_addr:02X} hacia MEM")
...
class Memory(Atomic):
    ...
    def deltext(self, e: float):
        ...
        if self.pending_addr is not None and self.pending_read:
            ...
            print(f"  [MEM] delta_ext: Solicitud de lectura en dirección {self.pending_addr:02X}")
    ...
    def deltint(self):
        if self.pending_operation:
            ...
            print(f"  [MEM] delta_int: Lectura completada. MEM[{addr_val:02X}]={data:02X}")
        self.pending_operation = None
        self.passivate()
    ...
    def lambdaf(self):
        if self.pending_operation and self.pending_operation[0] == "read":
            ...
            data = self.storage.get(addr_val, 0x00)
            print(f"  [MEM] lambda: Emitiendo dato={data:02X} por data_out")
