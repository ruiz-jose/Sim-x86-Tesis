...
Para contrastar RISC (RISC-V) y CISC (x86) se muestra una operación: leer dos palabras desde memoria, sumarlas y almacenar el resultado donde estaba el primer operando.

Ejemplo RISC-V (instrucciones de longitud fija; uso explícito de registros base y desplazamientos):
...
Notas:
- la es pseudo instrucción (ensamblador expande a auipc + lw/addi según el caso).
- lw/sw usan formato lw rd, offset(rs1).
- Separación clara entre obtención de direcciones y acceso a datos reduce ambigüedad respecto a 0(mem1).

Ejemplo x86 (instrucciones de longitud variable; ALU con operando en memoria):
...
Notas:
- x86 permite ALU con un operando en memoria (reg–mem), a diferencia del load/store puro en RISC‑V.
- Instrucciones de longitud variable; el tamaño del operando se declara con dd/dword (32 bits).
- Sintaxis estilo NASM/Intel; en otros ensambladores pueden variar directivas y etiquetas.
