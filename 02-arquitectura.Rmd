...
El repertorio de instrucciones, conocido como **ISA (Instruction Set Architecture)**, define el conjunto de operaciones que un procesador puede ejecutar y cómo se codifican dichas operaciones. Este conjunto incluye instrucciones aritméticas, lógicas, de control y manipulación de datos, además de los modos de direccionamiento y formatos de las instrucciones. El diseño de un ISA influye considerable en el rendimiento, la eficiencia energética y la flexibilidad de la arquitectura de un procesador [@hennessy_computer_2012; @null_essentials_2014; @stallings_computer_2013].
...
Entre las características clave que deben considerarse en el diseño de un repertorio de instrucciones están las siguientes [@hennessy_computer_2012]:
  ...
  - **Tipos de operandos**: los tipos de datos que las instrucciones pueden manipular, tales como enteros, números en punto flotante, caracteres y direcciones de memoria. Un ISA bien diseñado soporta eficientemente diversos tipos de operandos.
  - **Tipos de operaciones**: las operaciones que el procesador es capaz de realizar, incluyendo aritmética (suma, resta), lógica (AND, OR), control (saltos, llamadas a subrutinas) y manipulación de datos (movimiento de datos, almacenamiento, carga).
  - **Modos de direccionamiento**: los métodos para especificar operandos en las instrucciones, como inmediato, directo, indirecto y relativo, cada uno con ventajas en términos de flexibilidad y eficiencia.
  - **Formato de las instrucciones**: la estructura de una instrucción, que define su longitud, número de operandos y modos de direccionamiento. Este aspecto impacta directamente en la complejidad y eficiencia del procesador.
...
# Mostrar la tabla en formato adecuado según el tipo de salida
if (knitr::is_latex_output()) {
  kable(ensambladores, format = "latex", booktabs = TRUE, caption = "Comparativa de Ensambladores x86") %>%
    kable_styling(latex_options = "scale_down") %>%
    column_spec(1, width = "6cm") %>%  # Ajustar el ancho de la columna "Característica"
    column_spec(2:4, width = "5cm") %>%  # Ajustar el ancho de las columnas "TASM", "MASM", "NASM"
    row_spec(0, bold = TRUE, background = "#D3D3D3")
} else {
  kable(ensambladores, format = "html", table.attr = "class='table table-striped'", caption = "Comparativa de Ensambladores x86") %>%
    kable_styling(full_width = FALSE, position = "center") %>%
    column_spec(1, width = "20em") %>%  # Ajustar el ancho de la columna "Característica"
    column_spec(2:4, width = "15em") %>%  # Ajustar el ancho de las columnas "TASM", "MASM", "NASM"
    row_spec(0, bold = TRUE, background = "#D3D3D3")
}
```
