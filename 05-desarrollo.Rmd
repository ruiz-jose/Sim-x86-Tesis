...
Entre los registros específicos, no accesibles directamente por el programador, se encuentran: `SP` (_Stack Pointer_), utilizado para la gestión de la pila; el registro `Flags` (_flags register_); el `IP` (_Instruction Pointer_), que contiene la dirección de la próxima instrucción a ejecutar; y el `IR` (_Instruction Register_), que almacena el byte de la instrucción actualmente en proceso de decodificación y ejecución. Además, existen dos registros dedicados a la transferencia de información entre la CPU y la memoria principal: el `MAR` (_Memory Address Register_) encargado de almacenar direcciones de memoria, y el `MBR` (_Memory Buffer Register_) que almacena el byte que se quiere propagar o se ha recibido por el bus de datos.
...
#### Flags
El registro `Flags` (banderas de estado) es un registro de 8 bits que contiene las _flags_ mostradas en la siguiente tabla. Este registro no es directamente accesible por el programador, pero puede ser modificado por las operaciones de la ALU y pueden realizarse saltos condicionales en base a sus valores.
...
tpuertos <- data.frame(
  ...
  Direccion = c(
    ...
    "28h", "29h", "2Ah", "2Bh", "30h", "31h", "32h", "33h", "40h", "41h"
  ),
  Nombre = c(
    "CONT", "COMP", "EOI", "IMR", "IRR", "ISR", "INT0", "INT1", "INT2", "INT3",
    "INT4", "INT5", "INT6", "INT7", "PA", "PB", "CA", "CB", "DATA", "STATE"
  ),
  stringsAsFactors = FALSE
)
...
if (knitr::is_latex_output()) {
  tabla_puertos %>%
    ...
    row_spec(c(1, 3, 15, 19), background = "#F0F0F0")
} else {
  tabla_puertos %>%
    kable_styling(
      bootstrap_options = c("striped", "hover", "condensed", "responsive"),
...
#### Impresora con PIO
El simulador permite conectar una impresora al módulo PIO, emulando el protocolo de comunicación utilizado por las impresoras paralelas estándar tipo Centronics. La configuración de los puertos para esta conexión se organiza de la siguiente manera:
...
Donde:
- Puerto B (PB): almacena el carácter a imprimir codificado en ASCII (8 bits de datos: DDDD DDDD)
- Puerto A (PA): controla las señales de comunicación, donde:
  - Los seis bits más significativos no se utilizan
  - Bit S (strobe): señal de control que indica a la impresora cuándo debe capturar los datos
  - Bit B (busy): señal de estado que indica si la impresora está ocupada

El protocolo de comunicación se basa en el strobe, una señal de control que notifica a la impresora cuándo debe leer los datos del puerto B. Las impresoras compatibles con el estándar Centronics capturan el valor presente en PB al detectar un flanco ascendente en la señal strobe, es decir, cuando esta transiciona de S=0 a S=1.


Secuencia para imprimir un carácter:

  1. Verificar disponibilidad: comprobar que la bandera busy esté en 0, indicando que la impresora no está ocupada
  2. Cargar datos: escribir el carácter deseado (código ASCII) en el puerto PB
  3. Preparar strobe: establecer la señal strobe en 0
  4. Activar impresión: cambiar la señal strobe a 1 para generar el flanco ascendente que iniciará la impresión

Esta implementación proporciona una comprensión práctica de los protocolos de comunicación paralela y el control de dispositivos periféricos en sistemas computacionales.
