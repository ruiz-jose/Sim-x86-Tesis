...
| **Control de flujo**      | `JMP / Jxx / CALL / RET` | {12}           | Salto incondicional `JMP`, condicionales `Jxx`, llamada `CALL`, retorno `RET` |

| **Manejo de pila y E/S**  | `PUSH / POP / OUT / IN` | {13}           | `PUSH`: poner en la pila<br>`POP`: retirar de la pila<br>`OUT`: enviar a puerto<br>`IN`: recibir desde puerto |

| **Manejo de interrupción**| `INT / IRET / NOP / HLT`| {14}           | `INT`: llamar rutina de interrupción<br>`IRET`: retornar de interrupción<br>`NOP`: no opera<br>`HLT`: detiene el CPU  |

| **Miscelánea**            | `AND / OR / XOR / NOT`  | {15}           | Operaciones lógicas                               |
