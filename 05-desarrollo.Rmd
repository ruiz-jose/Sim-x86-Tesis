...
if (knitr::is_latex_output()) {
  kable(bloquesfuncionales, format = "latex", booktabs = TRUE, caption = "Bloques funcionales principales") %>% # nolint: line_length_linter.
    kable_styling(latex_options = "scale_down") %>%
    column_spec(1, width = "5cm") %>%
    column_spec(2, width = "10cm")
} else {
  kable(bloquesfuncionales, format = "html", table.attr = "class='table table-striped'", caption = "Bloques funcionales principales") %>% # nolint
...
arq <- data.frame(
  `Componente` = c("Arquitectura",
                   ...
                   "Instrucciones",
                   ""),
  `Características` = c("- Von Neumann, con memoria compartida para datos e instrucciones.", # nolint
                        "- 4 registros de propósito general (`AL`, `BL`, `DL` y `DL`) de 8 bits cada uno.", # nolint
                        "- 3 registros de propósito específico: `IP` (Instruction Pointer), `IR`(Instruction Register) y `SP`(Stack Pointer).", # nolint
...
if (knitr::is_latex_output()) {
  kable(arq, format = "latex", booktabs = TRUE, caption = "Componentes del CPU x86") %>% # nolint
    kable_styling(latex_options = "scale_down") %>%
    column_spec(1, width = "5cm") %>%
    column_spec(2, width = "10cm")
} else {
  kable(arq, format = "html", table.attr = "class='table table-striped'", caption = "Componentes del CPU x86") %>% # nolint: line_length_linter.
    kable_styling(full_width = FALSE, position = "center") %>%
    column_spec(1, width = "20em") %>%
    column_spec(2, width = "40em")
}
...
# Convertir a HTML
kable(setreducido, format = "markdown", align = "l", col.names = c("Instrucciones", "Código operación", "nemónico", "Acción"), # nolint: line_length_linter. # nolint: line_length_linter.
      caption = "Tabla de Instrucciones y Códigos de Operación de la Arquitectura x86") # nolint: line_length_linter.
```
...
codtraducido <- data.frame(
  ...
  Operacion = c("A: entre registros", "B: Cargar a registro", "C: Almacenar en memoria", "D: control de flujo"), # nolint: line_length_linter.
...
if (knitr::is_latex_output()) {
  kable(codtraducido, format = "latex", booktabs = TRUE, align = "l", col.names = c("Tipo", "Operación", "Codificación", "Parámetros"), # nolint: line_length_linter.
    ...
    column_spec(2, width = "5cm") %>%  # Ajustar el ancho de la columna "Operación" # nolint: line_length_linter, line_length_linter.
    column_spec(3, width = "5cm") %>%  # Ajustar el ancho de la columna "Codificación" # nolint: line_length_linter, line_length_linter.
    column_spec(4, width = "6cm") %>%  # Ajustar el ancho de la columna "Parámetros" # nolint
    row_spec(3, extra_latex_after = "\\midrule")  # Agregar una línea después de la tercera fila # nolint: line_length_linter.
} else {
  kable(codtraducido, format = "html", table.attr = "class='table table-striped'", align = "l", col.names = c("Tipo", "Operación", "Codificación", "Parámetros"), # nolint: line_length_linter.
    ...
    column_spec(2, width = "25em") %>%  # Ajustar el ancho de la columna "Operación" # nolint
    column_spec(3, width = "25em") %>%  # Ajustar el ancho de la columna "Codificación" # nolint: line_length_linter.
    column_spec(4, width = "30em") %>%  # Ajustar el ancho de la columna "Parámetros" # nolint: line_length_linter.
    row_spec(3, extra_css = "border-top: 2px solid #000;")  # Agregar una línea después de la tercera fila # nolint: line_length_linter.
}
```
...
# Imprimir la tabla en formato markdown
kable(tabla_instrucciones, format = "markdown", col.names = c("#", "Instrucción", "Acción", "Codificación")) # nolint: line_length_linter.
```
...
formatoampliado <- data.frame(
  Tipo = c("Transferencia y procesamiento", "", "", "Control"),
  Operacion = c("A: entre registros", "B: Cargar a registro", "C: Almacenar en memoria", "D: control de flujo"), # nolint: line_length_linter.
  Codificación = c("____XXYY", "____XX00 MMMMMMMM", "____00YY MMMMMMMM dddddddd", "____ffff MMMMMMMM"), # nolint: line_length_linter, line_length_linter.
  Parámetros = c(
    "XX = Registro destino, YY = Registro fuente",
