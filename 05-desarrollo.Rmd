...
```{r diagramasimulador, echo=FALSE, fig.cap="Estructura general del simulador VonSim8", fig.align = 'center', out.width = "90%"}
...
```{r org, echo=FALSE, fig.cap="Compatibilitad directiva ORG y END", fig.align = 'center', out.width = "85%"}
...
```{r dispositivos, echo=FALSE, fig.cap="Arquitectura general del simulador", fig.align = 'center', out.width = "85%"}
...
tisa <- data.frame(
  ...
  Acción = c(
    ...
    "Operación aritmética: operando1 ← operando1 - operando2",
    "Comparación: operando1 - operando2 (no actualiza el destino)",
    "Salto incondicional JMP, condicionales Jxx, subrutina CALL, llamar rutina de interrupción INT",
    "Retorno subrutina RET, retornar de interrupción IRET,  deshabilita interrupciones CLI, habilita interrupciones STI, detener CPU HLT",
    "Poner en la pila PUSH, retirar de la pila POP, enviar a puerto OUT, recibir desde puerto IN",
...
- **Interrupciones**
  - **INT `Dirección`**
    ...
    11. **`write(Memoria[MAR])` $\leftarrow$ `MBR` | `IP` $\leftarrow$ `ri`** - Guardar en la pila y saltar a la rutina de interrupción.
  - **IRET**
    4. **`MAR` $\leftarrow$ `SP`** – Obtener dirección de retorno desde la pil
    5. **`MBR` $\leftarrow$ `read(Memoria[MAR])`** – Leer la dirección de retorno.
    6. **`IP` $\leftarrow$ `MBR` | `SP` $\leftarrow$ `SP` + 1** – Restaurar IP y actualizar SP.
  ...
  - **CLI**
    4. **update(Flags I=1)** – Desactivar interrupciones.
  - **STI**
    4. **update(Flags I=0)** – Activar interrupciones.
- **E/S**
  - **OUT**
...
Con el propósito de evaluar la efectividad del simulador VonSim8 en la enseñanza de arquitectura de computadoras, se llevó a cabo una prueba piloto durante el segundo cuatrimestre de 2025, dirigida a estudiantes de segundo año de la Licenciatura en Sistemas. El objetivo principal fue medir el grado de comprensión de conceptos esenciales, como el ciclo de instrucción, la manipulación de registros y la gestión de interrupciones.

### Objetivos de la validación
- Analizar el impacto del simulador en la comprensión de los conceptos fundamentales de arquitectura de computadoras.
- Identificar mejoras en la capacidad de los estudiantes para analizar y depurar programas en ensamblador.
- Evaluar la percepción de usabilidad y utilidad didáctica de la herramienta.

### Población de estudio
La prueba piloto se realizó sobre una muestra de 30 estudiantes matriculados en la asignatura Arquitectura de Computadoras, correspondiente al segundo año de la carrera. La participación fue voluntaria, asegurando la representatividad respecto al perfil típico de la materia.

### Instrumentos de recolección de datos

Para la validación se emplearon instrumentos diseñados para medir tanto la calidad didáctica como captar opiniones cualitativas de docentes y estudiantes. A continuación se detallan los instrumentos utilizados, adaptados al contexto universitario y alineados con los objetivos pedagógicos del capítulo:

- **Cuestionario de conocimiento:** Aplicado a estudiantes, utiliza una escala tipo Likert (por ejemplo, 1-5) para evaluar la satisfacción y percepción de eficacia en aspectos clave del simulador, como visualización, editor, progresividad de instrucciones, métricas, documentación y manejo de periféricos. Incluye además un espacio para comentarios abiertos.

Ejemplo de ítems para estudiantes:
  - La visualización gráfica del simulador me ayudó a comprender cómo interactúan los componentes de la CPU y la memoria.
  - El editor de ensamblador facilitó mi aprendizaje comparado con otros entornos conocidos.
  - La activación progresiva de instrucciones es útil para no sentirme sobrecargado.
  - Las métricas de rendimiento (CPI, ciclos, tiempo de CPU) me permitieron comparar distintas implementaciones.
  - La documentación/tutorial responde a mis dudas para trabajar autónomamente.

- **Entrevistas semiestructuradas a docentes:** Este instrumento permite recoger opiniones expertas sobre el potencial pedagógico del simulador, comparaciones con herramientas previas y oportunidades de mejora. Las preguntas se organizan en categorías como usabilidad, claridad conceptual, impacto en la motivación estudiantil y utilidad de la documentación.

Ejemplo de preguntas para docentes:
  - ¿Qué elementos del simulador considera más efectivos para la enseñanza del ciclo de instrucción y los modos de direccionamiento?
  - ¿Qué ventajas o desventajas observa respecto a otros simuladores utilizados previamente?
  - ¿Cómo evalúa la progresividad en la activación de instrucciones para evitar sobrecarga cognitiva?
  - ¿El simulador facilita la exploración práctica de conceptos como interrupciones y periféricos? ¿Por qué?
  - ¿Recomendaría ajustes en algún módulo (editor, visualizador, métricas, documentación)?

### Análisis de datos

El análisis de los datos recolectados se realizará mediante técnicas cuantitativas y cualitativas, en función de los instrumentos aplicados:

- **Cuestionario de conocimiento (escala Likert y comentarios abiertos):**
  - Se calcularán estadísticas descriptivas (media, mediana, desviación estándar) para cada ítem del cuestionario, permitiendo identificar tendencias generales en la percepción y satisfacción de los estudiantes respecto a las distintas funcionalidades del simulador.
  - Los comentarios abiertos serán analizados mediante codificación temática, identificando patrones recurrentes, sugerencias y aspectos destacados por los estudiantes.

- **Entrevistas semiestructuradas a docentes:**
  - Las respuestas serán transcritas y analizadas mediante análisis temático, agrupando las opiniones en categorías como usabilidad, claridad conceptual, impacto en la motivación estudiantil y utilidad de la documentación.
  ...
  - Las sugerencias y recomendaciones de los docentes se utilizarán para orientar futuras mejoras en el desarrollo del simulador.

Esta combinación de análisis cuantitativo y cualitativo permitirá obtener una visión integral del impacto pedagógico del simulador VonSim8, fundamentando las conclusiones y propuestas de mejora en la evidencia recogida durante la validación.
