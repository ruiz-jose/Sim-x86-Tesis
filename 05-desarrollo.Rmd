...
estructuravonsim8 <- data.frame(
  ...
  `Características` = c(
    ...
    if (knitr::is_latex_output()) {
      "6 registros de propósito específico:\\begin{itemize}
      \\item `IP` (Instruction Pointer)
      \\item `IR` (Instruction Register)
      \\item `SP` (Stack Pointer)
      \\item `RS` (registro de estado)
    ...
    if (knitr::is_latex_output()) {
      "Puertos de la CPU:\\begin{itemize}
      \\item Bus de direcciones de 8 bits, gestionado por el buffer \\texttt{MAR}.
      \\item Bus de datos de 8 bits, gestionado por el buffer \\texttt{MBR}.
      \\item Señal de lectura (\\texttt{rd}) y escritura (\\texttt{wr}), cada una de 1 bit.
      \\item Señal \\texttt{IO/M} (1 bit) para distinguir acceso a memoria o E/S.
      \\item Señal de petición de interrupción (\\texttt{INTR}, 1 bit).
      \\item Señal de reconocimiento de interrupción (\\texttt{INTA}, 1 bit).
      \\end{itemize}"
    } else {
...
1. **`MAR` $\leftarrow$ `IP`**: La UC transfiere el contenido del Puntero de Instrucción (`IP`) al registro de direcciones de memoria (`MAR`). El IP contiene la dirección de la siguiente instrucción a ejecutar, por lo que este paso prepara el sistema para acceder a esa ubicación en la memoria.

2. **`MBR` $\leftarrow$ `read(Memoria[MAR])` | `IP` $\leftarrow$ `IP` + 1**: La UC activa la señal de lectura para obtener la instrucción en la dirección especificada por el `MAR`. Este dato se almacena en el registro de datos de memoria (`MBR`) a través del bus de datos. Simultáneamente, el  `IP` se incrementa en 1, apuntando así a la siguiente instrucción u operando en memoria.

3. **`IR` $\leftarrow$ `MBR`**: El contenido del MBR se transfiere al registro de instrucciones (`IR`). En este punto, la instrucción ha sido captada y está lista para ser ejecutada.

### Etapa 2: Ejecución:
En esta etapa, las operaciones varían según el tipo de instrucción. A continuación, se detallan los pasos para las instrucciones de dos operandos, como `MOV`, `ADD`, `SUB` y `CMP`, las cuales pueden tener como destino un registro o una dirección de memoria.

**Instrucciones con destino en un registro (Rx)** 

Modo de direccionamiento: Entre registros (`Rx`, `Ry`)
  4. MOV: **`Rx` $\leftarrow$ `Ry`** 
  4. ADD: **`Rx` $\leftarrow$ `Rx` + `Ry`; update(Flags)** 
  4. SUB: **`Rx` $\leftarrow$ `Rx` - `Ry`; update(Flags)**
  4. CMP: **`Rx` - `Ry`; update(Flags)**

Modo de direccionamiento: Directo (`Rx`, `[Dirección]`)
  4. **`MAR` $\leftarrow$ `IP`**: El IP se transfiere al MAR para obtener la dirección del operando fuente.
  5. **`MBR` $\leftarrow$ `read(Memoria[MAR])` | `IP` $\leftarrow$ `IP` + 1**: Se lee la dirección de memoria y se almacena en MBR; el IP se incrementa.
  6. **`MAR` $\leftarrow$ `MBR`**: La dirección del operando fuente se mueve de MBR al MAR.
  7. **`MBR` $\leftarrow$ `read(Memoria[MAR])`**: Se lee el dato del operando fuente de la memoria y se almacena en MBR.

  8. A continuación, se realiza la operación específica de la instrucción:
    - MOV: **`Rx`  $\leftarrow$ `MBR`**
    - ADD: **`Rx`  $\leftarrow$ `Rx` + `MBR`; update(Flags)**
    - SUB: **`Rx`  $\leftarrow$ `Rx` - `MBR`; update(Flags)**
    - CMP: **`Rx` - `MBR`; update(Flags)**

Modo de direccionamiento: Inmediato (`Rx`, `Dato`)
  4. **`MAR` $\leftarrow$ `IP`**: Se obtiene la dirección del dato (operando fuente).
  5. **`MBR` $\leftarrow$ `read(Memoria[MAR])` | `IP` $\leftarrow$ `IP` + 1**: Se lee el dato de la memoria y se almacena en MBR; el IP se incrementa.
...
Modo de direccionamiento: Indirecto (`Rx`, `[BL]`)
  4. **`MAR` $\leftarrow$ `BL`**: El contenido del registro BL, que contiene la dirección del operando fuente, se transfiere al MAR.
  5. **`MBR` $\leftarrow$ `read(Memoria[MAR])`**: Se lee el dato de la memoria en la dirección de BL y se almacena en MBR.
  6. A continuación, se realiza la operación específica de la instrucción:
    - MOV: **`Rx`  $\leftarrow$ `MBR`**
    - ADD: **`Rx`  $\leftarrow$ `Rx` + `MBR` | update(Flags)**
    - SUB: **`Rx`  $\leftarrow$ `Rx` - `MBR` | update(Flags)**
...
Modo de direccionamiento: Directo (`[Dirección]`, `Ry`)
  4. **`MAR` $\leftarrow$ `IP`**: Se obtiene la dirección del operando destino.
  5. **`MBR` $\leftarrow$ `read(Memoria[MAR])` | `IP` $\leftarrow$ `IP` + 1**: Se lee la dirección de memoria y se almacena en MBR; el IP se incrementa.
  6. **`MAR` $\leftarrow$ `MBR`**: La dirección de memoria se mueve de MBR a MAR.  

  Si la instrucción es MOV:
  7. MOV: **`MBR` $\leftarrow$ `Ry`**: El contenido del registro Ry se mueve a MBR.
  8. MOV: **`write(Memoria[MAR])` $\leftarrow$ `MBR`**: El dato de MBR se escribe en la memoria.

  Si la instrucción es ADD, SUB, CMP:
  7. ADD/SUB/CMP: **`MBR` $\leftarrow$ `read(Memoria[MAR])`**: Se lee el dato de la memoria en la dirección del operando destino y se almacena en MBR.
  Luego se realiza la operación:
  8. ADD: **`MBR` $\leftarrow$ `MBR` + `Ry` | update(Flags)**
  8. SUB: **`MBR` $\leftarrow$ `MBR` - `Ry` | update(Flags)**
  8. CMP: **`MBR` - `Ry`; update(Flags)**

  Si la instrucción es ADD o SUB, se escribe el resultado en memoria: 
  9. ADD/SUB: **`write(Memoria[MAR])` $\leftarrow$ `MBR`**


Modo de direccionamiento: Indirecto (`[BL]`, `Ry`)

  4. **`MAR` $\leftarrow$ `BL`**: El contenido de BL, que es la dirección del operando destino, se transfiere a MAR.
  Si la instrucción es MOV:
  5. MOV: **`MBR` $\leftarrow$ `Ry`**: El contenido del registro Ry se mueve a MBR.
  6. MOV: **`write(Memoria[MAR])` $\leftarrow$ `MBR`**: El dato de MBR se escribe en la memoria.
  
  Si la instrucción es ADD, SUB, CMP:
  6 MBR ← read(Memoria[MAR]): Se lee el dato del operando destino en la memoria y se almacena en MBR.
  5. ADD/SUB/CMP: **`MBR` $\leftarrow$ `read(Memoria[MAR])`**
  6. ADD: **`write(Memoria[MAR])` $\leftarrow$ `MBR` + `Ry`| update(Flags)**
  6. SUB `[BL]`, `Ry`: **`write(Memoria[MAR])` $\leftarrow$ `MBR` - `Ry` | update(Flags)**
  6. CMP `[BL]`, `Ry`: **`MBR` - `Ry` | update(Flags)**

  La instrucción CMP no realiza el paso 9 de escritura en memoria.
  7. ADD/SUB: **`write(Memoria[MAR])` $\leftarrow$ `MBR`**



**Instrucciones de 2 operandos**, como `MOV`, `ADD`, `SUB` y `CMP`, se ejecutan siguiendo un conjunto de pasos que varían según el destino de operando y el modo de direccionamiento. Estas instrucciones pueden tener como destino un registro o una posición de memoria, y pueden involucrar direccioniento entre registros, directo, indirecto o inmediato.
Las instrucciones que utilizan la Unidad Aritmético-Lógica (ALU) actualizan las banderas del registro de estado  `FLAGS`.
...
* Inmediato `[Dirección]`, `Dato` 
  4. **`MAR` $\leftarrow$ `IP`**
  5. **`MBR` $\leftarrow$ `read(Memoria[MAR])` | `IP` $\leftarrow$ `IP` + 1**
  Los pasos 4. 5. se obtiene la direccion del operando destino de de la instrucción.

  6. **`MAR` $\leftarrow$ `IP`;`ri` $\leftarrow$ `MBR`**
  7. **`MBR` $\leftarrow$ `read(Memoria[MAR])` | `IP` $\leftarrow$ `IP` + 1**
  8. MOV:**`MAR` $\leftarrow$ `ri`**
  8. ADD/SUB/CMP: **`MAR` $\leftarrow$ `ri` | `id` $\leftarrow$ `MBR`**
  ...
  9. MOV: **`write(Memoria[MAR])` $\leftarrow$ `MBR`**

  Para la instrucción ADD/SUB/CMP los pasos son: 
  9. **`MBR` $\leftarrow$ `read(Memoria[MAR])`**
  10. ADD: **`MBR` $\leftarrow$ `MBR` + `id`; update(Flags)**
  10. SUB: **`MBR` $\leftarrow$ `MBR` - `id`; update(Flags)**
  10. CMP: **`MBR` - `id`; update(Flags)**

  La instrucción CMP no realiza el paso 10 de escritura en memoria.
  11. ADD/SUB: **`write(Memoria[MAR])` $\leftarrow$ `MBR` | update(Flags)**
...
* Inmediato `[BL]`, `Dato`
  ...
  Para la instrucción MOV los pasos son:
  6. **`MAR` $\leftarrow$ `BL`**
  7. MOV: **`write(Memoria[MAR])` $\leftarrow$ `MBR`**

  Para la instrucción ADD/SUB/CMP los pasos son: 
  6. **`MAR` $\leftarrow$ `BL` | `id` $\leftarrow$ `MBR`**
  7. ADD/SUB/CMP: **`MBR` $\leftarrow$ `read(Memoria[MAR])`**
  8. ADD: **`MBR` $\leftarrow$ `MBR` + `id`; update(Flags)**
  8. SUB: **`MBR` $\leftarrow$ `MBR` - `id`; update(Flags)**
  8. CMP: **`MBR` - `id`; update(Flags)**

  La instrucción CMP no realiza el paso 10 de escritura en memoria.
  9. ADD/SUB: **`write(Memoria[MAR])` $\leftarrow$ `MBR` | update(Flags)**
...
* Salto incondicional y condicionales:  `Dirección` 
  ...
  6. JMP: **`IP` $\leftarrow$ `MBR`** (Salto incondicional)
  6. Jxx: `Si ` xx== 1: **`IP` $\leftarrow$ `MBR`** (Salto condicional)
  Si la bandera esta en 1 el contenido del `MBR` se transfiere al registro `IP`.
  Si la bandera esta en 0 no se realiza ninguna acción y se continua con la siguiente instrucción.
:

En la instrucción de salto condicional `Jxx Dirección`, el parámetro `xx` representa todas las combinaciones posibles de los _flags_. En este contexto, `NZ` indica la negación del _flag_ `Z`. A continuación se detallan las instrucciones y su acción correspondiente:
