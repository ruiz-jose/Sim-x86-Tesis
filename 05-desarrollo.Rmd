...
3. **Repertorio de instrucciones reducido y habilitación progresiva:**
    - Seleccionar un subconjunto esencial de instrucciones x86, que se habiliten progresivamente según avance la asignatura, adaptándose al ritmo de aprendizaje de los estudiantes. Esta estrategia didáctica facilita la asimilación gradual del conjunto de instrucciones (ISA) y es consistente con enfoques pedagógicos en el área de organización y arquitectura de computadores [@null2018essentials; @stallings2021computer].

4. **Simulación visual e interactiva de micropasos de instrucciones:**
    - Visualizar el flujo de datos mediante el nivel de transferencia entre registros (RTL, por sus siglas en inglés: Register Transfer Level), resaltando cada componente involucrado en las distintas etapas del ciclo de instrucción. Es común en los libros de Hennessy y Patterson [@hennessy2017computer] el uso de RTL para representar el ciclo de instruccion desde el fetch (recuperación de la instrucción) hasta el execute (ejecución). Este enfoque permite a los estudiantes observar cómo se transfieren los datos entre registros y cómo se activan las señales de control en cada etapa del ciclo de instrucción. 

    ```{r cicloinstruccion, echo=FALSE, fig.cap="Ciclo de instrucción", fig.align = 'center', out.width = "85%"}
...
## Diseño conceptual
A partir de los requisitos funcionales y pedagógicos previamente establecidos, se desarrolla el diseño conceptual del simulador, que traduce dichas especificaciones en una estructura técnica coherente con los objetivos educativos. Este diseño busca asegurar la coherencia entre la funcionalidad de la herramienta y los objetivos pedagógicos definidos para la enseñanza de arquitectura de computadoras.

El diseño conceptual del simulador se construye a partir de los requisitos funcionales y pedagógicos definidos previamente, y se plasma en una arquitectura operativa articulada por bloques funcionales con responsabilidades bien delimitadas.
...
El modelo propuesto adopta una arquitectura simplificada de 8 bits, inspirada en los principios de la arquitectura x86, y diseñada con un repertorio reducido de instrucciones. Se opta por una arquitectura de 8 bits como estrategia didáctica, ya que al simplificar el sistema se favorece la comprensión conceptual sin comprometer los aspectos esenciales del repertorio x86.

#### Principales bloques funcionales
La elección de estos bloques funcionales responde tanto a la necesidad de modelar componentes esenciales de una computadora real como a criterios pedagógicos de modularidad y claridad conceptual. Se describe los siguientes bloques funcionales:
