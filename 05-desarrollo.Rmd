...
3. **Repertorio de instrucciones reducido y habilitación progresiva:**
    - Se propone seleccionar un subconjunto esencial de instrucciones x86 y habilitarlo de manera progresiva, siguiendo recomendaciones pedagógicas que sugieren que el aprendizaje efectivo se potencia cuando los contenidos se introducen de forma escalonada. Tal como afirman las National Academies of Sciences, Engineering, and Medicine [@nationalacademies2018how], esta estrategia facilita la construcción gradual de esquemas mentales complejos y mejora la comprensión de conceptos técnicos. Esta estrategia didáctica facilita la asimilación gradual del conjunto de instrucciones (ISA) y es consistente con enfoques pedagógicos orientados a reducir la complejidad inicial del aprendizaje  [@hasan_survey_2012; @null_essentials_2023; @stallings_computer_2021].

4. **Simulación visual e interactiva de micropasos de instrucciones:**
    - Visualizar el flujo de datos mediante una representación en el nivel de transferencia entre registros (Register Transfer Level, RTL), que describe cómo los datos se mueven entre componentes internos como registros y buses durante cada etapa del ciclo de instrucción. En la obra de Stallings [@stallings_computer_2021], es común el uso del nivel de transferencia entre registros (RTL) para representar cada etapa del ciclo de instrucción, desde la captura (‘fetch’) hasta la ejecución (‘execute’). Este enfoque facilita la comprensión del procesador, ya que permite visualizar el recorrido de los datos entre registros y la activación de señales de control en cada etapa. 
...
## Fundamentación de la arquitectura del simulador
