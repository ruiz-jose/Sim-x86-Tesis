...
## Implementación del ciclo de instrucción MOV AL, BL en xDEVS

La instrucción **MOV AL, BL** transfiere el contenido del registro fuente **BL** al registro destino **AL**.  
En el simulador **VonSim8**, esta operación se modela como una secuencia de *eventos discretos* coordinados por la **Unidad de Control (UC)**, que emite señales de lectura y escritura sobre los registros involucrados.  

A continuación se describe la implementación de este ciclo en el entorno **xDEVS**, donde cada componente se representa como un **modelo atómico**, y su interacción como un **modelo acoplado jerárquico**.
...
- **Fase de captación (Fetch)** — común a todas las instrucciones.  
- **Fase de ejecución (Execute)** — específica del tipo de instrucción (en este caso, MOV).
...
componentes <- data.frame(
  Componente = c(
    ...
    "AL (Registro Acumulador bajo)",
    "BL (Registro base bajo)",
    "BUS (Bus de datos)"
  ),
  Tipo = c(
    "Atómico",
    ...
    "Atómico",
    "Atómico",
    "Atómico o variable compartida"
  ),
  Descripción = c(
    "Decodifica IR, genera señales de control para lectura/escritura.",
    "Contiene el código binario de la instrucción.",
    "Registro destino.",
    "Registro fuente.",
    "Transporta el dato desde el origen al destino."
  )
)
...
#### Flujo de eventos y microoperaciones (nivel RTL)
...
microoperaciones <- data.frame(
  ...
  Descripción = c(
    "La UC lee el valor del IR y reconoce el opcode MOV con operandos AL, BL.",
    "La UC activa la señal de lectura del registro fuente.",
    "El registro BL coloca su valor en el bus de datos.",
