...
- **Subrutinas**
  - **CALL `Dirección`**
    4. Se obtiene el operando de la instrucción.
    5. Se obtiene el operando de la instrucción.
...
- **Interrupciones**
  - **INT `Dirección`**
    4. Obtener segundo byte de memoria
    5. Obtener segundo byte de memoria
    6. `ri` $\leftarrow$ `MBR`; `SP` $\leftarrow$ `SP` - 1
    7. `MAR` $\leftarrow$ `SP`
    8. `MBR` $\leftarrow$ `Ry`
    9. `write(Memoria[MAR])` $\leftarrow$ `MBR`
