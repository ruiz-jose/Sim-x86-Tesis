...
Considerando los contenidos y objetivos pedagógicos de la asignatura Arquitectura de Computadoras, se desarrolló VonSim8^[VonSim8: https://ruiz-jose.github.io/VonSim8/] un simulador concebido en este trabajo como adaptación de VonSim, pero adaptado para operar con registros y memoria de 8 bits, con una interfaz simplificada orientada al aprendizaje progresivo.
...
```{r comparativavonsim, echo=FALSE, out.width = "100%", fig.cap="Comparación entre VonSim y VonSim8"}
library(knitr)
library(kableExtra)

comparativavonsim <- data.frame(
  Característica = c("Arquitectura", "Tamaño de registros", "Interfaz gráfica", "Repertorio de instrucciones", "Objetivo didáctico"),
  VonSim = c("Von Neumann", "8 y 16 bis", "Completa, detallada", "Amplio", "Demostración de conceptos avanzados"),
  VonSim8 = c("Von Neumann", "8 bits", "Simplificada, esquemática", "Reducido y adaptado", "Comprensión progresiva de conceptos básicos")
)

tabla_kable <- kable(comparativavonsim,
               format = ifelse(knitr::is_latex_output(), "latex", "html"),
...
if (knitr::is_latex_output()) {
  tabla_kable %>%
    ...
    row_spec(0, bold = TRUE, align = "c", background = "#D3D3D3") %>%
    row_spec(seq_len(nrow(comparativavonsim)),
             extra_latex_after = "\\addlinespace[10pt]")
} else {
  tabla_kable %>%
    kable_styling(
...
Esta sección describe la estructura del simulador VonSim8, el diseño de los registros fue concebido para facilitar la comprensión de los modos de direccionamiento y del ciclo de instrucción, ambos considerados fundamentos clave en el estudio de la Arquitectura de Computadoras [@stallings_computer_2021]. Se describe en la siguiente tabla \@ref(tab:estructuravonsim8) los componentes principales del simulador, junto con sus características y funcionalidades específicas. Esta tabla proporciona una visión general de la arquitectura del simulador, destacando los elementos clave que componen su estructura y su propósito pedagógico.:
...
```{r ISAdidacticos, echo=FALSE, message=FALSE, warning=FALSE}
library(knitr)
library(kableExtra)

# Crear los datos de la tabla
ISAdidacticos<- data.frame(
  `tipoisa` = c(
    "Transferencia y procesamiento de datos",
    "Control de flujo"
  ),
  `Uso pedagógico esperado` = c(
    "Instrucciones que permiten mover datos entre registros y memoria, realizar operaciones aritméticas, y manipular el contenido de los registros. Estas instrucciones son fundamentales para la comprensión del flujo de datos en una arquitectura computacional",
    "Instrucciones que permiten alterar el flujo de ejecución del programa mediante saltos condicionales e incondicionales, así como la detención del procesador. Estas instrucciones son esenciales para comprender cómo se controlan las decisiones y el flujo de ejecución en un programa."
  )
)

tabla <- kable(ISAdidacticos,
               format = ifelse(knitr::is_latex_output(), "latex", "html"),
               caption = "Tipo de instrucción y uso pedagógicos esperados",
               col.names = c("Tipo de instruccion", "Uso pedagógico esperado"),
               booktabs = TRUE,
               escape = FALSE,
               align = c("c", "c"))
...
if (knitr::is_latex_output()) {
  tabla %>%
    ...
    column_spec(2, width = "9cm") %>%
    row_spec(0, bold = TRUE, align = "c", background = "#D3D3D3") %>%
    row_spec(seq_len(nrow(ISAdidacticos)),
             extra_latex_after = "\\addlinespace[10pt]")
} else {
  tabla %>%
    kable_styling(
...
formatoinst <- data.frame(
  ...
  operandos = c("2 operandos (fuente y destino)", "Con 1 o sin operando"), # nolint: line_length_linter.
  Ejemplo = c("MOV operando-destino, operando-fuente", "JMP operando o HLT"), # nolint: line_length_linter.
...
# Mostrar la tabla en formato adecuado según el tipo de salida
if (knitr::is_latex_output()) {
  kable(formatoinst, format = "latex", booktabs = TRUE, align = "l", col.names = c("Tipo", "Operandos", "Ejemplo"), # nolint: line_length_linter.
  ...
  kable(formatoinst, format = "html", table.attr = "class='table table-striped'", align = "l", col.names = c("Tipo", "Operandos", "Ejemplo"), # nolint: line_length_linter.
...
modosdireccionamiento <- data.frame(
  ...
  Parámetros = c(
    ...
    if (knitr::is_latex_output()) {
      "Operando destino registro Rx y operando fuente puede ser:\\begin{itemize} \\item dirección [M]\\item dirección registro [BL] \\item valor en la instrucción d \\end{itemize}" # nolint: line_length_linter.
    } else {
      "Operando destino registro `Rx` y operando fuente puede ser:<ul><li>dirección `[M]`</li><li>dirección registro `[BL]`</li><li>valor en la instrucción `d`</li></ul>" # nolint: line_length_linter.
    },
     if (knitr::is_latex_output()) {
...
if (knitr::is_latex_output()) {
  tabla %>%
    ...
    column_spec(3, width = "8cm") %>%
    row_spec(0, bold = TRUE, align = "c", background = "#D3D3D3") %>%
    row_spec(seq_len(nrow(modosdireccionamiento)), extra_latex_after = "\\addlinespace[8pt]")
} else {
  tabla %>%
    kable_styling(
      bootstrap_options = c("striped", "hover", "condensed"),
    ...
    row_spec(0, bold = TRUE, color = "white", background = "#0073C2") %>%
    footnote(
      general = "Las instrucciones de transferencia y de procesamiento de datos tienen los tres tipos de combinaciones en los operandos.",
      general_title = "Nota: ",
      threeparttable = TRUE
    )
}
...
instaccion <- data.frame(
  ...
  Ejemplo = c(
    ...
    "SUB [M], Ry", "SUB [M], --`", "CMP [M], --", "", "", "",
    "JMP M", "JC M", "JZ M", "HLT"
  ),
  Acción = c(
    "Rx ← Ry", "Rx ← Rx + Ry", "Rx ← Rx - Ry", "Rx - Ry: no almacena resultado en operando destino",
    ...
    "B: Cargar a registro", "C: Almacenar en memoria", "B: Cargar a registro", "C: Almacenar en memoria", "B: Cargar a registro", "C: Almacenar en memoria",
...
dirampliado <- data.frame(
  ...
  `Interpretación` = c(
    ...
    "Indirecto utiliza como operando implícito el registro `BL`", # nolint: line_length_linter.
...
dirampliado2 <- data.frame(
  `Función` = c("00YY", "01YY", "1100", "1101"),
  `Byte` = c("2", "1", "3", "2"),
  `Interpretación` = c(
    "directo `M` = Dirección de memoria, `YY` = Registro fuente",
