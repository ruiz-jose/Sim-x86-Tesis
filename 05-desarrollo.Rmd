...
## Implementación del ciclo de instrucción MOV AL, BL mediante el Formalismo xDEVS

La simulación de la instrucción **MOV AL, BL** es esencial para validar la arquitectura de VonSim8. Esta instrucción de transferencia de registro a registro, que copia el contenido del registro fuente BL al registro destino AL, se modela como una secuencia de eventos discretos y temporizados que representan el funcionamiento a Nivel de Transferencia de Registros (RTL).
...
Esta fase, común a cualquier instrucción, se conserva tal como se detalla en la definición general del ciclo de instrucción:
  ...
  - **`MAR` $\leftarrow$ `IP`**
  - **`MBR` $\leftarrow$ `read(Memoria[MAR])` | `IP` $\leftarrow$ `IP` + 1**
  - **`IR` $\leftarrow$ `MBR`**

Al finalizar, el registro **IR** contiene el código binario correspondiente a la instrucción `MOV AL, BL`.
