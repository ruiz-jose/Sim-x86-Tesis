...
| --------------------------------- | ----------------------------- | ------------------------------------------------------------------ |
| **UC** (Unidad de Control)        | Atómico                       | Decodifica `IR`, genera señales de control para lectura/escritura. |
| **IR** (Registro de Instrucción)  | Atómico                       | Contiene el código binario de la instrucción.                      |
| **AL** (Registro Acumulador bajo) | Atómico                       | Registro destino.                                                  |
| **BL** (Registro base bajo)       | Atómico                       | Registro fuente.                                                   |
| **BUS** (Bus de datos)            | Atómico o variable compartida | Transporta el dato desde el origen al destino.                     |
...
El modelo atómico del **Registro** se define como:

| Paso | Microoperación           | Descripción                                                                  | Modelos y funciones DEVS                                                                |
| ---- | ------------------------ | ---------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| 1    | `UC: decodificar(IR)`    | La UC lee el valor del IR y reconoce el opcode `MOV` con operandos `AL, BL`. | `/delta_ext` (UC) recibe evento del IR; actualiza su estado (`S`) con la instrucción actual. |
| 2    | `UC → BL.enable_out = 1` | La UC activa la señal de lectura del registro fuente.                        | `/lambda` (UC) genera evento de salida hacia BL.                                              |
| 3    | `BUS ← BL`               | El registro BL coloca su valor en el bus de datos.                           | `/lambda` (BL) emite valor actual al BUS.                                                     |
| 4    | `UC → AL.enable_in = 1`  | La UC habilita la escritura en AL.                                           | `/lambda` (UC) envía evento de control a AL.                                                  |
| 5    | `AL ← BUS`               | El registro AL captura el dato del bus y actualiza su estado interno.        | `/delta_ext` (AL) recibe valor del BUS y actualiza su estado `S`.                            |
...
| Tiempo (t) | Evento               | Registro afectado | Acción                                                |
| ---------- | -------------------- | ----------------- | ----------------------------------------------------- |
| t         | Captación completada | IR                | Contiene `MOV AL, BL`                                 |
