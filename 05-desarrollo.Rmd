...
componentes_vonsim8 <- data.frame(
  ...
  `Entradas / Salidas` = c(
    "\\texttt{salida: addr\\_out; entrada: ip\\_write, branch\\_target}",
...
if (knitr::is_latex_output()) {
  tabla_componentes_vonsim8 %>%
    ...
    column_spec(2, width = "7cm") %>%
    column_spec(3, width = "6cm") %>%
    column_spec(4, width = "2.5cm", bold = TRUE) %>%
    row_spec(0, bold = TRUE, align = "c", background = "#D3D3D3")
} else {
  tabla_componentes_vonsim8 %>%
    ...
    column_spec(4, width = "8em", bold = TRUE) %>%
    row_spec(0, bold = TRUE, color = "white", background = "#0073C2")
}
```
...
microoperaciones_fetch <- data.frame(
  ...
  Descripción = c(
    ...
    "La UC activa la señal mem\\_read para iniciar la lectura de la instrucción y, de forma concurrente, genera una señal de incremento sobre el IP +1 .",
    "La memoria unificada entrega la instrucción solicitada tras su retardo interno.",
    "El MBR transfiere la instrucción al registro de instrucción.",
    "El IR genera un evento indicando que la instrucción está lista para decodificación."
  ),
  `Modelos y funciones DEVS` = c(
    "$\\lambda(UC) \\rightarrow \\delta_{ext}(IP)$",
    "$\\lambda(IP) \\rightarrow \\delta_{ext}(MAR)$",
    "$\\lambda(UC) \\rightarrow \\delta_{ext}(MEM);\\lambda(UC) \\rightarrow \\delta_{ext}(IP)$",
...
Durante la fase de ejecución, la UC decodifica el contenido del IR y activa las señales necesarias para que el Banco de Registros (REG_BANK) permita la transferencia del dato desde BL hacia AL a través del bus compartido.

El REG_BANK es un modelo acoplado que contiene cuatro modelos atómicos: AL, BL, CL y DL. Cada registro posee sus propias entradas (enable_in, enable_out, data_in, data_out), pero el banco actúa como intermediario, gestionando la habilitación del registro fuente y del registro destino según las señales de la UC.
...
microoperaciones <- data.frame(
  ...
  Microoperación = c(
    "UC: decodificar(IR)",
    "UC $\\rightarrow$ REG\\_BANK.enable\\_out(BL) = 1",
    "BUS $\\leftarrow$ BL",
    "UC $\\rightarrow$ REG\\_BANK.enable\\_in(AL) = 1",
    "AL $\\leftarrow$ BUS"
  ),
  Descripción = c(
    "La UC analiza el opcode MOV y determina los operandos AL y BL.",
    "El Banco de Registros activa la salida del registro BL.",
    "El valor del registro BL se transmite por el bus.",
    "La UC habilita la entrada del registro AL dentro del Banco de Registros.",
    "El registro AL captura el dato del bus y actualiza su valor interno."
  ),
  `Modelos y funciones xDEVS` = c(
    "$\\delta_{\\text{ext}}(UC)$ recibe evento del IR y actualiza su estado con la instrucción actual.",
    "$\\lambda_{UC}$ genera evento hacia REG\\_BANK(BL).",
    "$\\lambda_{BL}$ emite su valor al bus.",
    "$\\lambda_{UC}$ activa señal de escritura hacia REG\\_BANK(AL).",
    "$\\delta_{\\text{ext}}(AL)$ recibe el dato desde el bus y actualiza su estado $S$."
  )
)
...
tabla_microop <- kable(
  microoperaciones,
  format = ifelse(knitr::is_latex_output(), "latex", "html"),
  caption = "Secuencia de microoperaciones para MOV AL, BL utilizando el Banco de Registros",
