...
Cada uno se modela como un **modelo atómico o acoplado** dentro del entorno xDEVS, con entradas, salidas y retardos propios.
...
componentes_vonsim8 <- data.frame(
  ...
  Descripción = c(
    ...
    "Banco de registros que agrupa a AL, BL, CL y DL. Cada registro es un modelo atómico interno que puede leer o escribir a través del bus. El banco gestiona las señales de control (\\texttt{enable\\_in}, \\texttt{enable\\_out}) y coordina el acceso a los registros.",
...
La secuencia de eventos principales es:
  ...
  1. UC → IP: la UC inicia la lectura de la dirección actual del IP.
  ...
  4. MEM → MBR: la memoria devuelve la instrucción tras un retardo $\tau_{mem}$.

  5. MBR → IR: el MBR transfiere la instrucción al IR.

  6. UC → IR → UC: el IR notifica a la UC que la instrucción está lista para decodificación.
...
El REG_BANK es un modelo acoplado que contiene cuatro modelos atómicos: AL, BL, CL y DL.
Cada registro posee sus propias entradas (enable_in, enable_out, data_in, data_out), pero el banco actúa como intermediario, gestionando la habilitación del registro fuente y del registro destino según las señales de la UC.
