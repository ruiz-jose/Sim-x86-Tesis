...
* Inmediato `[Dirección]`, `Dato` 
  ...
  Para la instrucción ADD/SUB/CMP los pasos son:
  9. ADD/SUB/CMP: **`id` $\leftarrow$ `MBR`; `MAR` $\leftarrow$ `ri`**
  10. **`MBR` $\leftarrow$ `read(Memoria[MAR])`**
  11. ADD: **`MBR` $\leftarrow$ `MBR` + `id`; update(Flags)**
  11. SUB: **`MBR` $\leftarrow$ `MBR` - `id`; update(Flags)**
  11. CMP: **`MBR` - `id`; update(Flags)**

  La instrucción CMP no realiza el paso 10 de escritura en memoria.
  12. ADD/SUB: **`write(Memoria[MAR])` $\leftarrow$ `MBR`; update(Flags)**
