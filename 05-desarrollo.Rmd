...
arq <- data.frame(
  ...
  `Características` = c("- von Neumann, con memoria compartida para datos e instrucciones.",
                        ...
                        "- 2 registros de propósito específico: `IP` (Instruction Pointer) y `SP`(Stack Pointer).",
...
El ciclo de la instrucción describe las secuencias de microoperaciones que ocurren en la computadora durante la ejecución de una instrucción. Para expresar estas operaciones, se utiliza un lenguaje de transferencia entre registros:
...
#### **Etapa de Ejecución**:
La etapa de ejecución depende del tipo de instrucción. A continuación, se detalla cómo se ejecutan algunas instrucciones específicas:
