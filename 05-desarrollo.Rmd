...
En el VonSim8 el registro `FLAGS` es un registro de 8 bits que contiene las _flags_ o banderas mostradas en la siguiente tabla. Este registro no es directamente accesible por el usuario, pero puede ser modificado por las operaciones de la ALU y pueden realizarse saltos condicionales en base a sus valores.
...
tabla_flags <- kable(flags,
              format = ifelse(knitr::is_latex_output(), "latex", "html"),
              caption = "Registro FLAGS: descripción de las banderas",
              col.names = c("Bit #", "Abreviatura", "Descripción"),
              booktabs = TRUE,
              escape = FALSE,
              align = c("c", "c", "c"))
...
if (knitr::is_latex_output()) {
  tabla_flags %>%
    ...
    row_spec(0, bold = TRUE, align = "c", background = "#D3D3D3") %>%
    row_spec(seq_len(nrow(flags)), extra_latex_after = "\\addlinespace[10pt]")
} else {
  tabla_flags %>%
    kable_styling(
      bootstrap_options = c("striped", "hover", "condensed", "responsive"),
