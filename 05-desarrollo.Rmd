...
La etapa de captación tiene 3 ciclos para todas las instrucciones. En el primero la Unidad de Control (UC) transfiere el contenido del registro puntero de instrucciones (`IP`) al registro de direcciones de memoria (`MAR`). Este paso prepara el sistema para acceder a la posición de memoria donde se encuentra la próxima instrucción. En el segundo paso la UC activa la señal de lectura para obtener el contenido de la memoria en la dirección especificada por el (`MAR`). El dato leído se transfiere al registro de datos de memoria (`MBR`) mediante el bus de datos. Simultáneamente, el registro (`IP`) se incrementa en uno, apuntando así a la siguiente instrucción en memoria u operando de la instrucción vigente. Finalmente, el tercer paso el contenido del registro de datos de memoria (`MBR`) se copia en el registro de instrucciones (`IR`). Con este paso, la instrucción ha sido captada y está lista para ser decodificada y ejecutada. 
...
*  Directo `Rx`, `[Dirección]`
  ...
  8. MOV: **`Rx`  $\leftarrow$ `MBR`**
  8. ADD: **`Rx`  $\leftarrow$ `Rx` + `MBR`; update(Flags)**
  8. SUB: **`Rx`  $\leftarrow$ `Rx` - `MBR`; update(Flags)**
  8. CMP: **`Rx` - `MBR`; update(Flags)**
...
* Inmediato `Rx`, `Dato`
  ...
  6. MOV: **`Rx`  $\leftarrow$ `MBR`**
  6. ADD: **`Rx`  $\leftarrow$ `Rx` + `MBR`; update(Flags)**
  6. SUB: **`Rx`  $\leftarrow$ `Rx` - `MBR`; update(Flags)**
  6. CMP: **`Rx` - `MBR`; update(Flags)**
...
* Directo `[Dirección]`, `Ry`     
  ...
  7. MOV: **`MBR` $\leftarrow$ `Ry`** 
  8. MOV: **`write(Memoria[MAR])` $\leftarrow$ `MBR`**
...
* Indirecto `[BL]`, `Ry`    
  ...
  5. MOV: **`MBR` $\leftarrow$ `Ry`**
  6. MOV: **`write(Memoria[MAR])` $\leftarrow$ `MBR`**
...
* Inmediato `[Dirección]`, `Dato` 
  ...
  Para la instrucción ADD/SUB/CMP los pasos son:
  9. ADD/SUB/CMP: **`id` $\leftarrow$ `MBR`**
...
* Inmediato `[BL]`, `Dato`  
  ...
  7. MOV `[BL]`, `Dato`: **`write(Memoria[MAR])` $\leftarrow$ `MBR`**: Se escribe el valor de MBR en la posición de memoria indicada por MAR.
  7. ADD `[BL]`, `Dato`: **`write(Memoria[MAR])` $\leftarrow$ `MBR`; update(Flags)**
