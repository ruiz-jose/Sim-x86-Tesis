...
* Inmediato `[Dirección]`, `Dato` 
  ...
  Para la instrucción ADD/SUB/CMP los pasos son:
  9. ADD: **`MBR` $\leftarrow$ `MBR` + `Ry`; update(Flags)**
  9. SUB: **`MBR` $\leftarrow$ `MBR` - `Ry`; update(Flags)**
  9. CMP: **`MBR` - `Ry`; update(Flags)**

  La instrucción CMP no realiza el paso 10 de escritura en memoria.
  10. ADD/SUB: **`write(Memoria[MAR])` $\leftarrow$ `MBR`; update(Flags)**
