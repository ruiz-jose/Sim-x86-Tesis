...
La Unidad de Control (UC) transfiere el contenido del registro puntero de instrucciones (`IP`) al registro de direcciones de memoria (`MAR`). Este paso prepara el sistema para acceder a la posición de memoria donde se encuentra la próxima instrucción. La UC activa la señal de lectura para obtener el contenido de la memoria en la dirección especificada por el (`MAR`). El dato leído se transfiere al registro de datos de memoria (`MBR`) mediante el bus de datos. Simultáneamente, el registro (`IP`) se incrementa en uno, apuntando así a la siguiente instrucción en memoria u operando de la instrucción vigente. Finalmente, el contenido del registro de datos de memoria (`MBR`) se copia en el registro de instrucciones (`IR`). Con este paso, la instrucción ha sido captada y está lista para ser decodificada y ejecutada. 
...
* Entre registros `Rx`, `Ry`
  4. MOV `Rx`, `Ry`:  **`Rx` $\leftarrow$ `Ry`** 
  4. ADD `Rx`, `Ry`:  **`Rx` $\leftarrow$ `Rx` + `Ry`; update(Flags)** 
  4. SUB `Rx`, `Ry`:  **`Rx` $\leftarrow$ `Rx` - `Ry`; update(Flags)**
  4. CMP `Rx`, `Ry`:  **`Rx` - `Ry`; update(Flags)**
...
Los pasos 4. 5. se obtiene el operando de la instrucción. El contenido de IP se transfiere al registro MAR. Se lee el dato de la memoria en la dirección indicada por MAR y se almacena en MBR; luego, IP se incrementa en uno.  El valor de MBR se transfiere a MAR. Se lee el dato de la memoria en la nueva dirección de MAR y se almacena en MBR.
  8. MOV `Rx`, `[Dirección]`: **`Rx`  $\leftarrow$ `MBR`**
  8. ADD `Rx`, `[Dirección]`: **`Rx`  $\leftarrow$ `Rx` + `MBR`; update(Flags)**
  8. SUB `Rx`, `[Dirección]`: **`Rx`  $\leftarrow$ `Rx` - `MBR`; update(Flags)**
  8. CMP `Rx`, `[Dirección]`: **`Rx` - `MBR`; update(Flags)**
...
* ADD `[Dirección]`, `Ry` (Directo)     
  8. **`write(Memoria[MAR])` $\leftarrow$ `MBR`; update(Flags)**

* ADD `[BL]`, `Ry` (Indirecto)     
  8. **`write(Memoria[MAR])` $\leftarrow$ `MBR`; update(Flags)**

* ADD `[Dirección]`, `Dato` (Inmediato)     
  9. **`write(Memoria[MAR])` $\leftarrow$ `MBR`; update(Flags)**

* ADD `[BL]`, `Dato` (Inmediato)     
  7. **`write(Memoria[MAR])` $\leftarrow$ `MBR`; update(Flags)**
...
* SUB `Rx`, `[Dirección]` (Directo)
  8. **`Rx`  $\leftarrow$ `Rx` - `MBR`; update(Flags)**

* SUB `Rx`, `Dato` (Inmediato)
  6. **`Rx`  $\leftarrow$ `Rx` - `MBR`; update(Flags)**
...
#### Destino Memoria
* SUB `[Dirección]`, `Ry` (Directo)     
  8. **`write(Memoria[MAR])` $\leftarrow$ `MBR`; update(Flags)**
* SUB `[BL]`, `Ry` (Indirecto)     
  8. **`write(Memoria[MAR])` $\leftarrow$ `MBR`; update(Flags)**
* SUB `[Dirección]`, `Dato` (Inmediato) 
  9. **`write(Memoria[MAR])` $\leftarrow$ `MBR`; update(Flags)**
* SUB `[BL]`, `Dato` (Inmediato)     
  7. **`write(Memoria[MAR])` $\leftarrow$ `MBR`; update(Flags)**  

#### Destino registro
* CMP `Rx`, `Ry` (Entre registros)
  4. **`Rx` - `Ry`; update(Flags)**
* CMP `Rx`, `[Dirección]` (Directo)
  8. **`Rx` - `MBR`; update(Flags)**  
* CMP `Rx`, `Dato` (Inmediato)
  6. **`Rx` - `MBR`; update(Flags)**
* CMP `Rx`, `[BL]` (Indirecto)        
  6. **`Rx` - `MBR`; update(Flags)**  
#### Destino Memoria
* CMP `[Dirección]`, `Ry` (Directo)     
  8. **`write(Memoria[MAR])` $\leftarrow$ `MBR`; update(Flags)**

* CMP `[BL]`, `Ry` (Indirecto)     
  8. **`write(Memoria[MAR])` $\leftarrow$ `MBR`; update(Flags)**

* CMP `[Dirección]`, `Dato` (Inmediato) 
  9. **`write(Memoria[MAR])` $\leftarrow$ `MBR`; update(Flags)**

* CMP `[BL]`, `Dato` (Inmediato)     
  7. **`write(Memoria[MAR])` $\leftarrow$ `MBR`; update(Flags)**
